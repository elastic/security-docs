...
To view Linux session data from your Kubernetes infrastructure, you'll need to set up the <DocLink id="serverlessSecurityKubernetesDashboard">Kubernetes dashboard</DocLink>.
</DocCallOut>
...
1. Go to **Manage** -> **Policies**, and edit one or more of your ((elastic-defend)) integration policies.
1. Select the **Policy settings** tab, then scroll down to the Linux event collection section near the bottom.
...
1. Go to **Manage** -> **Policies**, then select one or more of your ((elastic-defend)) integration policies to edit.
1. On the **Policy settings** tab, scroll down to the Linux event collection section near the bottom of the page
