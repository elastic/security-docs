...
You can set up CSPM for Azure either by by enroling an Azure organization containing multiple subscriptions, or by enroling a single subscription. Either way, first you will add the CSPM integration, then enable cloud account access.
...
The first method is simpler, but requires you to deploy {agent} to a VM in the Azure subscription you want to monitor with CSPM; the other methods allows {agent} to be deployed anywhere, including a VM outside Azure or a personal laptop.
...
. Fill in the values you copied earlier for **Client ID**, **Tenant ID**, and **Client Secret**.
. Under **Where to add this integration**, select **New hosts**.
...
. Upload the certificate bundle to a VM running cloudbeat.
...
. Fill in the values you copied earlier for **Client ID** and **Tenant ID**.
. For **Client Certificate Path**, enter the fullpath to the certificate you uploaded to your Cloudbeat host. 
