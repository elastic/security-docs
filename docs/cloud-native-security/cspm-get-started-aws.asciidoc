...
. (Optional) click **Advanced settings** to deploy the integration using agentless technology (beta). 
...
NOTE: Agentless deployments (beta) support two authentication methods: <<cspm-use-temp-credentials, temporary keys>> and <<cspm-use-keys-directly, direct access keys>>.
