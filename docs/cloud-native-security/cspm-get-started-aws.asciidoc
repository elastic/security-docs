...
If you choose not to use the CloudFormation setup method, you'll need to provide the necessary access using one of the manual options below. When using manual authentication to onboard at the organization level, some preliminary steps are required:

. Go to the AWS Console and log in to the organization where you want to deploy.
. In the organization's management account, create an IAM role called `cloudbeat-root` (the name is important). The role needs several policies:
...
After creating the necessary roles, authenticate using one of the <<cspm-set-up-manual, Manual options>>.
