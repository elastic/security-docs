---
id: serverlessSecurityD4cGetStarted
...
1. Optional — make any desired changes to the integration's policy by adjusting the **Selectors** and **Responses** sections. (For more information, refer to the <DocLink id="serverlessSecurityD4cPolicyGuide">Defend for Containers policy guide</DocLink>). You can also change these later.
...
One of the <DocLink id="serverlessSecurityD4cPolicyGuide" section="default-policies">default D4C policies</DocLink> sends process telemetry events (`fork` and `exec`) to ((es)).

In order to detect threats using this data, you'll need active <DocLink id="serverlessSecurityDetectionEngineOverview">detection rules</DocLink>. Elastic has prebuilt detection rules designed for this data. (You can also create your own <DocLink id="serverlessSecurityRulesUiCreate">custom rules</DocLink>.)
