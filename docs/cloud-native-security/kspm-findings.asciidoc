...
. Click **Group findings by:** to view a list of fields by which you can group findings. Select one of the suggested group-by fields, or click **Custom field** to choose your own group-by field. 
. When grouping is enabled, click a group to expand it and view all findings within that group.
. To disable grouping, click **Group findings by:** and select **None**.
...
* **Fields**: Select which fields to display for each finding. Fields which are checked here appear in the table and the **Columns** menu.

TIP: You can also click a column's name to open a menu that allows you to remove the column, sort by the column, move the column left or right, copy the column's name, or copy its contents as a string.
