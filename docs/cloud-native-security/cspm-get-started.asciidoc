...
For most users, the simplest option is to use AWS CloudFormation to automatically provision the necessary resources and permissions in your AWS account. Several manual options are also described below.

[discrete]
[[cspm-set-up-cloudformation]]
=== CloudFormation (recommended)
. Name your integration.
. Under *Setup Access*, select CloudFormation.
. In a new browser tab, log in as an admin to the AWS account you want to onboard.
. Return to your Kibana tab. Click *Save and continue* at the bottom of the page.
. Review the information modal, then click *Launch CloudFormation*.
. A CloudFormation template appears in a new tab. You don't need to modify its configuration.
