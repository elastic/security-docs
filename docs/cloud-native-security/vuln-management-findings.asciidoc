...
When *Group by* is set to `None`, you can sort the Findings table by multiple fields. To do so either click the column headings, or click the *Sort fields* button to the upper left of the table. When sorting is active, the *Sort fields* button changes to *X fields sorted* (where _X_ is the number of fields sorting your data), and can be used to modify or clear sorting.
