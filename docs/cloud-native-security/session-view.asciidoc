...
Session View can only display data that was collected by {elastic-defend} when *Include session data* was enabled. When this setting is enabled, {agent} captures additional process context data in process, file, and network events. For more information about the additional
...
of the process that caused the alert. This alert is a `process` alert, as indicated by the gear icon.
...
* The alert badge for multiple alerts. If a single process event or its children cause alerts of multiple types, multiple icons appear on the alert badge (a gear for `process`, a file for `file`, and a globe for `network`):
+
[role="screenshot"]
