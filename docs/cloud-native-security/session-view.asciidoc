...
. To include file and network alerts in Session View, check the boxes for *Network* and *File* events.
...
* The alert badge for multiple alerts. If a single event causes alerts of multiple types, multiple icons appear on the alert badge (a gear, a page, and a globe for `process`, `file`, and `network` alerts, respectively):
+
[role="screenshot"]
