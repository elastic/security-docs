...
* The alert badge for multiple alerts. If a single process event or its children cause alerts of multiple types, multiple icons appear on the alert badge (a gear, a page, and a globe for `process`, `file`, and `network` alerts, respectively):
+
[role="screenshot"]
