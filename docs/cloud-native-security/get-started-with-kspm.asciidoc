...
Levarege IRSA (IAM role to Kubenretes service-account) feature to get temporary credentials and scoped permission.
...
To use this, leave all options empty and follow the https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[IRSA] guide.
...
To use the assigned role of the instance, leave all the options empty.
This is important to validate that the assigned has all the required permissions.
