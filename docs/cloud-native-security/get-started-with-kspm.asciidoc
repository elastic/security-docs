...
A patch is required to https://www.elastic.co/guide/en/cloud-on-k8s/current/k8s-elastic-agent-configuration.html[Elastic Agent CRD] and https://www.elastic.co/guide/en/cloud-on-k8s/current/k8s-elastic-agent-configuration.html#k8s-elastic-agent-role-based-access-control[Elastic Agent Cluster-Role]

.Patch Elastic Agent
[%collapsible]
====
Add `volumes` and `volumeMonunts` to `podTemplate`
```yaml
podTemplate:
  spec:
    containers:
