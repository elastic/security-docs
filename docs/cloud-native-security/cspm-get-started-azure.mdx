...
1. Give your integration a name that matches the purpose or team of the Azure subscription you want to monitor, for example, `azure-dev-policy`.

<div id="cspm-set-up-cloud-access-section-azure"></div>

### Set up cloud account access
To setup CSPM for an Azure subscription, you will need admin privileges for that subscription.
...
1. Under **Where to add this integration**:
    ...
    1. (Optional) Change the `Resource Group Name` parameter. Otherwise, the name of the resource group defaults to `cloudbeat-` and a timestamp.
...
1. On the application page, go to **Certificates & secrets** > **New client secret**. Note down the **Value**; this is the only time it will be displayed.
1. Go to your Azure subscription list and select the subscription you want to monitor with CSPM.
1. Go to **Access control (IAM)**, and select **Add Role Assignment**.
...
1. Install ((agent)) on your chosen host, then set the following environment variables with the application (client) ID, directory (tenant) ID, and client secret that you got from Azure AD:
    - `AZURE_CLIENT_ID`
    - `AZURE_TENANT_ID`
    - `AZURE_CLIENT_SECRET`
1. Return to ((kib)) and wait for the confirmation of data received from your new integration.