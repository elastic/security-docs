...
* <DocLink id="serverlessSecurityAlertsUiManage">Alerts</DocLink>, stored in the `.alerts-security.alerts-<space-id>` index alias
* <DocLink id="serverlessSecurityAssetCriticality">Asset criticality level</DocLink>, stored in the `.asset-criticality.asset-criticality-<space-id>` index alias
...
The engine then verifies the entity's <DocLink id="serverlessSecurityAssetCriticality">asset criticality level</DocLink>. If there is no asset criticality assigned, the entity risk score remains equal to the aggregated score from the **Alerts** category. If a criticality level is assigned, the engine updates the risk score based on the default risk weight for each criticality level:
