...
This feature uses a {ref}/transforms.html[transform] to organize network data by host and process name, then runs scripted metric aggregations on the host-process name pairs. For a given time window, the scripted metric aggregation checks each pair for the following:
...
The values highlighted above are typical of beaconing behavior, and can help in your investigation.
...
As mentioned previously, beaconing is not used exclusively by malware. Many legitimate, benign processes also exhibit beacon-like activity. In order to reduce false positives, default filters in the transform query exclude known beaconing processes and IPs that fall into two groups:
