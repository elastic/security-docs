...
If so, please allow an hour for the data to be generated. It's also recommended you verify that user risk score data has been generated by doing the following:
...
GET ml_user_risk_score_latest_<space-id>/_search
----------------------------------

If no data returns, you'll need to check if the source index (`.alerts-security.alerts-<space-id>`) has alert data while `ml_userriskscore_pivot_transform<space-id>` is running.

[source,console]
----------------------------------
GET transform/ml_userriskscore_pivot_transform<space-id>/_stats?human=true
----------------------------------
