...
The data tables beneath the histogram display alert counts by users, alert counts by host, and associated MITRE ATT&CK tactics seen for risky users. The table data is sorted in reverse chronological order by default, with the highest total risk score at the top. Use this information to triage users with the highest risk in your network.
...
After enabling or upgrading user risk score, you may get the following message: 

[role="screenshot"]
image::images/restart-urs.png[Restart user risk score]

If so, click *Restart* and allow at least an hour for the data to be generated. If you still don't see any data, verify that user risk score data has been generated:
...
[source,console]
----------------------------------
GET transform/ml_userriskscore_pivot_transform_default/_stats?human=true
----------------------------------
...
If there's no response, verify that relevant <<rules-ui-management, rules>> are running and that alert data is being generated. If so, allow an hour for the user risk data to appear.