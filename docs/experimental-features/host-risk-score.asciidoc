...
If so, please allow an hour for the data to be generated. It's also recommended you verify that host risk score data has been generated by doing the following:
...
GET ml_host_risk_score_latest_<space-id>/_search
----------------------------------

If no data returns, you'll need to check if the source index (.`alerts-security.alerts-<space-id>`) has alert data while `ml_hostriskscore_pivot_transform<space-id>` is running.

[source,console]
----------------------------------
GET transform/ml_hostriskscore_pivot_transform<space-id>/_stats?human=true
----------------------------------
...
If there's no response, verify that relevant <<rules-ui-management, rules>> are running and alert data is generated. If so, restart the module and allow an hour for the data to generate.