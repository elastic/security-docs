...
The data tables beneath the histogram display associated rules, users, and MITRE ATT&CK tactics seen on risky hosts. The table data is sorted in reverse chronological order by default, with the highest total risk score at the top. Use this information to triage hosts with the highest risk in your network.
...
If so, click *Restart* and allow at least an hour for the data to be generated. If you still don't see any data, verify that host risk score data has been generated:
...
[source,console]
----------------------------------
GET transform/ml_hostriskscore_pivot_transform_default/_stats?human=true
----------------------------------
...
If there's no response, verify that relevant <<rules-ui-management, rules>> are running and that alert data is being generated. If so, allow an hour for the host risk data to appear.