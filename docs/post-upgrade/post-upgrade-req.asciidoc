...
NOTE: If you are upgrading from a minor release to {stack} version >= 7.11.0, there is now a {api-kibana}/operation/operation-createalertsmigration[detection alert migration API] that you can use instead of the manual process described below.
...
To display reindexed alerts in the {security-app}, create an index alias:
