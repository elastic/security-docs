...
[discrete]
[[self-protection]]
=== Elastic Endpoint self protection and permissions

Self protection means that the {elastic-endpoint} has guards against users and attackers that may try to interfere with its functionality. This protection feature consistently is enhanced to further prevent attackers who may attempt to use newer, more sophisticated tactics to interfere with the {elastic-endpoint}.

In {stack} version 7.11.0+, self protection defines the following permissions:
...
* Administrator/root users *can* stop the {elastic-agent}'s service. On Windows, run the `sc stop "Elastic Endpoint"` command. On macOS, run the `sudo launchctl stop elastic-agent` command.

NOTE: A user cannot directly kill the {elastic-agent}; however, a user _can_ terminate its running process because the {elastic-endpoint} is trying to protect the Agent from being modified while it runs (e.g., code injected into it). The user's inability to terminate the Agent is a purposeful feature of {elastic-endpoint}'s protection of the Agent.
