...
Elastic Security combines the threat detection of SIEM and the prevention,
detection, and response of Endpoint Security via an autonomous agent into a
unified software solution. These analytical and protection capabilities,
leveraged by the speed and extensibility of Elasticsearch, enable analysts to
defend their organization from threats before damage and loss occur.
...
Here's a short overview of the components:

* Data is shipped from your hosts to {es} via {es-sec-endpoint} and beat modules:
** <<install-endpoint, {es-sec-endpoint}>>: Protects your hosts against malware
and ships these data sets:
...
* The Security app in {kib} is used to manage the Detection engine, Cases, and
Timelines, as well as administer hosts running Endpoint Security:
** Detection engine: Automatically searches for suspicious host and network activity:
*** <<detection-engine-overview, Detection rules>>: Periodically search the data ({es} indices) send from your hosts for
suspicious events. Rules search for the source event attributes defined in
their queries. You can create your own rules and make use of our
<<prebuilt-rules, prebuilt ones>>.
*** <<included-jobs, {ml-cap} jobs>>: 
...
Endpoint Security, Beats and Beat modules,
