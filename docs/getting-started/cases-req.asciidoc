...
NOTE: Privileges set at the sub-feature level will override base privileges for the {es-sec} app. This means that, unless configured, the Cases feature takes on the base privileges set for the {es-sec} solution. For example, a role with `Read` base privileges only can view {es-sec} features and artifacts (including Cases) but can't modify them. However, setting the role's sub-feature privileges for Cases to `All` will allow it to view and manage Cases only.
