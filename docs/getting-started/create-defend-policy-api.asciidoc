...
Make a call to create an agent policy that you will add your {elastic-defend} integration to. Replace `<KIBANA-VERSION>` with your version of {kib}.
...
Next, add the {elastic-defend} integration to the policy that you created in step 1.

In the following call:

. Replace `<KIBANA-VERSION>` with your version of {kib}.
. Replace `<POLICY-ID>` with the agent policy ID you received in step 1.
. Replace `<LATEST-ELASTIC-DEFEND-PACKAGE-VERSION>` with the latest {elastic-defend} package version. To find this, go to **Management** -> **Integrations** and select *{elastic-defend}*. This will be a version such as 8.9.1.
...
Make a note of the `<PACKAGE-POLICY-ID>` you receive in the response. This refers to the {elastic-defend} policy and you will use this when making a call in step 3.
...
. Replace `<PACKAGE-POLICY-ID>` with the {elastic-defend} policy ID you received in step 2.
. Replace `<KIBANA-VERSION>` with your version of {kib}.
. Replace `<LATEST-ELASTIC-DEFEND-PACKAGE-VERSION>` with the latest {elastic-defend} package version. To find this, go to **Management** -> **Integrations** and select *{elastic-defend}*. This will be a version such as 8.9.1.

This saves your customized {elastic-defend} policy.
