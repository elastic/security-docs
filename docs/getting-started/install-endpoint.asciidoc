...
Platinum and enterprise users can customize the notifications on their host devices. These notifications appear whenever malware is detected or prevented with the syntax `Elastic Security {action} {filename}`.

Policy configuration determines default notification behavior: 

- **Prevent**: Notifications appear by default. Deselect the **Notify User** option to hide them.
- **Detect**: Notifications do not appear. Select the **Notify User** option to begin receiving them.

If the malware policy is not selected, notifications will not appear on the host device.
