...
| All Traditional Endpoint presets have the following preventions enabled by default: machine learning malware, ransomware, memory threat, malicious behavior, and credential theft.
...
* Events collected: All

|*Cloud Workloads presets*
|Both cloud Workload presets are intended for monitoring cloud-based Linux hosts, and therefore have <<session-view,Session data>> collection enabled by default, which enriches process events. All preventions are disabled by default.
