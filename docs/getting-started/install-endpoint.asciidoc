...
| All Traditional Endpoint presets have the following preventions enabled by default: malware, ransomware, memory threat, malicious behavior, and credential theft.
...
|Both cloud Workload presets have <<session-view,Session data>> collection enabled by default, which enriches process events, thereby increasing data throughput. All preventions are disabled by default.
