...
3. Select an Agent Policy for the Elastic Agent. You can either leave the default Policy selected, select an existing Policy from the drop-down, or select *Create agent policy** to create a new one. For more details on Elastic Agent configuration settings, see {fleet-guide}/elastic-agent-configuration.html[Configuration settings].
4. Configure the Endpoint Security integration with a name and optional description. When the configuration is complete, click **Save integration** in the lower right corner. Kibana redirects to the Administration page in the {security-app}.
+
[role="screenshot"]
image::images/install-endpoint/add-elastic-endpoint-security.png[]
+
5. In the "Enable Endpoint Security" section on your agent's page, select the name of your new integration. To enroll your agents with Endpoint Security, select **Enroll Agent**.
6. Kibana redirects you to Fleet to add the Elastic Agent to your host.
...
If you have upgraded to a {stack} version that includes the new {fleet} server (>=7.13.0), you will need to redeploy your agents. Review the following scenarios to ensure you take the appropriate steps. If you are running {stack} <=7.13.0, you can skip this section.

* If you redeploy the {agent} to the same machine through the {fleet} application after you upgrade, a new agent will appear.
* If you want to remove the {agent} entirely without transitioning to the {fleet} server, then you will need to manually uninstall the agent on the machine. This will also uninstall the endpoint.
* In the rare event that the {agent} fails to upgrade, you may need to manually uninstall the {endpoint}
