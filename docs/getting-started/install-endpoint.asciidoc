...
Notifications appear on protected hosts whenever malware is detected or prevented. Platinum and enterprise users can customize these notifications using the `Elastic Security {action} {filename}`syntax. 

Policy configuration determines default notification behavior: 

- **Prevent**: Notifications appear by default. Deselect the **Notify User** option to disable them.
