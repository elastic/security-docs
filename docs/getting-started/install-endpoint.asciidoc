...
. Select an agent policy for the {agent}. You can select the default policy, an existing custom policy, or select **Create agent policy** to create a new one. For more details on Elastic Agent configuration settings, see {fleet-guide}/elastic-agent-configuration.html[Configuration settings].
