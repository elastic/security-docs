...
Changes might be required if your nodes have customized roles. When updating node roles, nodes only take on the roles you specify and default roles are removed. If you need to re-assign `transform` role to a node, {ref}/modules-node.html#transform-node[create a dedicated transform node].
