...
//Might be a good idea to remove the bottom two so it's more clear what's required to create the .lists and .items data streams, and to manage lists and list items. Before remove this, check whether lists are considered part of the full detections feature.
* `.lists-<space-id>`
* `.items-<space-id>`
...
|Create the `.lists` and `.items` data streams in your Kibana space
