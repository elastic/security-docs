...
* To provide full access to rule actions and connectors, give your role `All` privileges. With `Read` privileges, you can edit rule actions but will have limited connector management capabilities. For example, `Read` privileges allow you to add or remove an existing connector from a rule, but not to create a new one.
