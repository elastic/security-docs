...
a|Manage lists and items

*NOTE:* To turn on the lists feature, a user with the `manage` cluster privilege must visit the Rules page in your Kibana space. 

| N/A
a| The `read` and `write`,`read`, and `view_index_metadata` index privileges for the following system indices and data streams, where `<space-id>` is the {kib} space name:
