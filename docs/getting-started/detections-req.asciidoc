...
== Enable and Access Detections
Enabling the Detections feature allows you to view and use the Detections page within the Security app. You only need to complete this action once and can choose between enabling it for a single Kibana space or across multiple ones. Once the Detections feature is enabled, additional privileges are needed to use Detection rules and alerts. The table below explains how to enable the Detection feature and describes required privileges for viewing and using rules and alerts. 
