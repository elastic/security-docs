...
//Might be a good idea to remove the bottom two so it's more clear what's required to create the .lists and .items data streams, and to manage lists and list items.
* `.lists-<space-id>`
* `.items-<space-id>`
...
|Create and manage lists and list items
