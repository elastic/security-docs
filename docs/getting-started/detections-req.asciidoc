...
* To provide full access to rule actions and connectors, give your role `All` privileges. With `Read` privileges, your role can edit rule actions but has limited connector management capabilities. For example, `Read` privileges allow you to add or remove an existing connector from the rule, but not to create a new one.
