create[[detections-permissions-section]]
...
== Enable and Access Detections
Enabling the Detections feature allows you to view and use the Detections page within the Security app. You only need to complete this action once and can choose between enabling it for a single Kibana space or across multiple ones.

Once the Detections feature is enabled, additional privileges are needed to use Detection rules and alerts. Similarly, specific requirements must be met to use Machine Learning jobs and rules. Those details are available at <<ml-requirements, Machine learning job and rule requirements>>.
