...
IMPORTANT: In {stack} version 8.0.0, the `.siem-signals-<space-id>` index has been renamed to `.alerts-security.alerts-<space-id>`. If you are upgrading to 8.0.0, it is recommended that users have privileges to both indices. If you are newly installing the {stack}, then users do not need privileges to the `.siem-signals-<space-id>` index.
