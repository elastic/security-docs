...
*Note*: As of {stack} version 8.0.0 and newer, the `.siem-signals-*` index has been renamed to `.alerts-security.alerts-*`. If you are upgrading to 8.0.0, it is recommended that users have privileges to both indices. If you are newly installing the {stack}, then you don't need privileges to the `.siem-signals-*` index.   
