...
* The navigation UI used throughout the {security-app}
...
The `securitySolution:enableGroupedNav` setting enables a new navigation UI for {elastic-sec} that groups related pages and highlights commonly visited areas for a streamlined experience.

By default, the {security-app} uses the legacy navigation UI; turn on `securitySolution:enableGroupedNav` to use the grouped navigation UI.
