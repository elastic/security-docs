...
The `securitySolution:enableExpandableFlyout` setting enables the expandable flyout on the Alerts page. This setting is turned on by default. Turn it off to use the alert details flyout, which is available in {elastic-sec} 8.9 and earlier. 
