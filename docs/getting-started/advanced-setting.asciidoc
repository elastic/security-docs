...
By default, the *Rules* and *Rule Monitoring* tables list the integrations required for each rule, and how many related integrations have been installed and configured. Turn off `securitySolution:showRelatedIntegrations` to hide this information in the tables (related integrations will still appear on rule details pages).
