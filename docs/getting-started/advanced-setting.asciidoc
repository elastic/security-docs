...
The `securitySolution:defaultThreatIndex` advanced setting allows users to specify one or more threat intelligence indices from which {elastic-sec} will collect threat match data. It controls features that query threat indices, such as the Threat Intelligence view on the Overview page, and uses the `filebeat-*`` index by default.

Users can enable cross cluster search by adding threat indices from remote clusters. {filebeat} indices are supported. Non-{filebeat} indices require ECS-compliant threat field mappings to ensure threat matching works.
