...
* IP reputation link on <<network-page-overview, IP details>> pages
...
To change the default interval {es-sec} uses to retrieve data when you open the
UI, update this settings:

`securitySolution:timeDefaults`

To configure how often the displayed {es-sec} data is refreshed by default, modify:
...
The UI {kibana-ref}/set-time-filter.html[time filter] overrides the default values. 
