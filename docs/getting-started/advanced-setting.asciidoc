...
UI, update this setting:

`securitySolution:timeDefaults`

To configure how often the displayed {es-sec} data is refreshed in the UI by
default, modify:

`securitySolution:refreshIntervalDefaults`

NOTE: See {ref}/common-options.html[Date Math] for information about the
required field syntax. The UI {kibana-ref}/set-time-filter.html[time filter]
