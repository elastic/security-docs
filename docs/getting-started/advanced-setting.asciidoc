...
The `excludedDataTiersForRuleExecution` setting does not apply to {esql} or {ml} rules. 

[TIP]
====

The `excludedDataTiersForRuleExecution` setting applies to all rules in a {kib} space. To only exclude cold and frozen data from specific rules, add a {ref}/sql-rest-filtering.html[Query DSL] filter to the rules you want affected. Add the filter when creating a new rule or updating an existing one. 
