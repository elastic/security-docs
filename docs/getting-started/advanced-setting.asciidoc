...
:frontmatter-description: Update advance settings for Security.
...
The `securitySolution:alertTags` field determines which options display in the alert tag menu. The default alert tag options are **Duplicate**, **False positive**, and **Further investigation required**. You can update the alert tag menu by editing these options or adding more.
