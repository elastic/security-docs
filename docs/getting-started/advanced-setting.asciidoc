...
The `securitySolution:enableExpandableFlyout` setting enables the expandable alert details flyout on the Alerts page. This setting is turned on by default. Turn it off to use the previous alert details flyout design that was available in {elastic-sec} 8.9 and earlier. 
