...
The Threat Intelligence view gives you a streamlined way to collect threat intelligence data that you can use for threat detection and matching. Threat intelligence data consists of threat indicators ingested from third-party threat intelligence sources.
...
- If you've chosen a time range that doesn't contain threat indicator event data, you are prompted to choose a different range. Use the the date and time picker in the {security-app} or Kibana to select a new range to analyze.
...
. In the Threat Intelligence view, click *Enable sources*. Scroll down the Integration page and select *Elastic Agent only* so the view only shows {agent} integrations.
+
[TIP]
=========================
...
=========================
. Select an {agent} integration, and complete the installation steps.
...
[float]
[[ti-mod-integration]]
== Enable a {filebeat} Threat Intel module integration

To enable a {filebeat} Threat intel integration:
...
+
NOTE: Threat intelligence indices aren’t required to be ECS compatible. However, we strongly recommend compatibility if you’d like for your alerts to be enriched with relevant threat indicator information. You can find a list of ECS-compliant threat intelligence fields at {ecs-ref}/ecs-threat.html[Threat Fields].
