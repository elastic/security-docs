...
WARNING: This allows a user to run highly privileged commands on a host as a root-level user. Restrict this feature privilege to only {elastic-sec} users who require this level of access.
|==============================================
