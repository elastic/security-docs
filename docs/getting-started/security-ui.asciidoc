...
The Threat Intelligence view leverages the Threat Intel module to ingest, parse, and visualize threat intelligence data from third-party and custom sources. When building an indicator match rule, you can use this data to strengthen and fine-tune detection rule logic. Threat intelligence data can also be valuable when triaging indicator match alerts and analyzing potential threats. If contextual data about the matched indicator is available, that data is added to the alert’s details in the *Overview* and the *Threat Intel* tabs. To learn more about this, visit <<view-alert-details, View alert details>>.
...
The Threat Intelligence view contains the following:

* *The total number of ingested indicators.* The example screenshot shows that 1954 have been ingested.
* *The names of ingested threat intelligence sources.* In the example, the AbuseCH {fleet} integration has been enabled on the Threat Intelligence view. The integration is divided into three individual threat intelligence sources because it collects three sets of data (`malware`, `malwarebazaar`, and `url`).
+
NOTE: The Threat Intelligence view looks for a `threat.feed.name` field value when defining the source’s name. If a source doesn't have the `threat.feed.name` field, or hasn't defined a `threat.feed.name` field value, it will be grouped into the `Other` category and labeled as *Other* in the *Name* column.

* *The number of indicators that have been ingested for each threat intelligence source.* In the example, 872 indicators have been ingested from the AbuseCH Maleware data source, 82 indicators have been ingested from AbuseCH MalwareBazaar, and 1000 indicators have been ingested from AbuseCH URL.
* *Dashboard links that allow you to inspect data from the ingested threat intelligence sources.* In the *Indicator* column, click *Source* for the appropriate source to open the dashboard.
+
NOTE: Generally, dashboards are not created for sources in the Other category. However, if an unnamed source has the threat.feed.dashboard_id field defined, a link to the dashboard becomes available.


//To open a full view of the Threat Intel module, click **View dashboard**. To further inspect data from the ingested threat intelligence sources, click **Source** for the appropriate source.
...
To enable threat intelligence sources:

. Install a {fleet-guide}/install-fleet-managed-elastic-agent.html[{fleet} managed {agent}] or a {filebeat} agent on the servers you want to monitor.
. In the Threat Intelligence view, click *Enable sources*.
+
NOTE: The warning message about disabled threat intelligence sources will display until you've enabled all of the available {elastic-sec} threat intelligence integrations.

. Complete the integration's installation steps.
. If you are enabling a threat intelligence integration that uses {filebeat} or a custom data shipper, update the *Threat indices* <<update-sec-indices, advanced setting>> by adding the appropriate index pattern name after the default {fleet} threat intelligence index pattern (`logs-ti*`)(optional):
.. If the integration uses {filebeat} version 8.0.0, add the appropriate {filebeat} threat intelligence index pattern, for example, `filebeat-8*` or `filebeat*`.
.. If you're enabling multiple versions of the same integration because it uses two or more versions of {filebeat}, differentiate between the {filebeat} threat intelligence indices by using unique index pattern names. For example, if the integration uses {filebeat} version 7.0.0 and 8.0.0, add `filebeat-7*` and `filebeat-8*`.
.. If the integration is using a custom data shipper, add the custom threat intelligence index pattern (for example, `custom-ti-index*`).
+
NOTE: Threat intelligence indices aren’t required to be ECS compatible. However, we recommend compatibility if you’d like for your alerts to be enriched with relevant threat indicator information. By default, indicator match rules are configured to ingest indicators from the Threat Intel {filebeat} module and to check for fields prefixed with `threatintel.indicator`. {filebeat-ref}/filebeat-module-threatintel.html[Threat Intel module] has more information about these fields.
