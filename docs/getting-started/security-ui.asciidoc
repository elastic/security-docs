...
// The cross-reference above and the image reference below both depend on the docs for the Indicators page
// being published first. The link currently points back to the Investigate section landing page just to 
// keep CI from breaking. 
// Replace the link and un-comment the image below once security-docs#2526 is merged & published. 
