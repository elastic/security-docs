...
* *The total number of ingested indicators.* The example screenshot shows that 1993 have been ingested.
...
* *The number of indicators that have been ingested for each threat intelligence source.* In the example, 937 indicators have been ingested from the AbuseCH Maleware data source, 56 indicators have been ingested from AbuseCH MalwareBazaar, and 1000 indicators have been ingested from AbuseCH URL.
...
[role="screenshot"]
image::images/threat-intelligence-module.png[width=90%][height=90%][Shows the Threat Intelligence view on the Overview page]
...
+
NOTE: The warning message about disabled threat intel sources will display until you've enabled all of the available {elastic-sec} threat intelligence integrations.

. Complete the integration's installation steps.
. Update the *Threat indices* (`securitySolution:defaultThreatIndex`) <<update-threat-intel-indice, advanced setting>> by adding the appropriate index pattern name after the default {fleet} threat intelligence index pattern (`logs-ti*`)(optional):
.. If the integration uses {filebeat} version 8.0.0, add the {filebeat} threat intelligence index pattern (for example, `filebeat-8*`).
.. If the integration uses two or more versions of {filebeat}, for example version 7.0.0 and version 8.0.0, differentiate between the {filebeat} threat intelligence index pattern names (for example, add `filebeat-7*` and `filebeat-8*`).
