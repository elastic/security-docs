...
The Host isolation exceptions page allows you to configure specific IP addresses that isolated hosts can still communicate with after they've been blocked from the rest of your network. See <<host-isolation-exceptions, Host isolation exceptions>> for more information.
