...
The Threat Intelligence view gives you a streamlined way to collect threat intelligence data that you can use for threat detection and matching. Threat intelligence data is comprised of threat indicators that are ingested from third-party threat intelligence sources. You can connect to these data sources using {agent} integrations, the {filebeat-ref}/filebeat-module-threatintel.html[Threat Intel module], or custom integrations.

Threat indicators describe potential threats, unusual behavior, or malicious activity on a network or in an environment. They’re commonly used in the rule logic of indicator match rules to detect and match known threats. When an indicator match rule generates an alert, the alert details will display information about the matched threat indicator. To learn more about alerts with threat intelligence, visit <<view-alert-details, View alert details>>.

NOTE: Ingested data is stored in the threat intelligence index, which the Threat Intelligence view queries for indicator data. Threat intelligence indices are specified in the `securitySolution:defaultThreatIndex` advanced setting. Visit <<update-threat-intel-indices, Update default Elastic Security threat intelligence indices>> to learn more about this setting.

The Threat Intelligence view shows you:

* *The total number of ingested threat indicators within the selected time range.* In the example below, a total of 1954 threat indicators have been ingested.
* *Enabled threat intelligence sources.* In the example, the AbuseCH {fleet} integration, which collects three sets of data (`malware`, `malwarebazaar`, and `url`), has been enabled.
+
NOTE: When populating the *Name* column, the Threat Intelligence view looks for a `threat.feed.name` field value to define the source name. If the source doesn't have the `threat.feed.name` field, or hasn't defined a `threat.feed.name` field value, it's named *Other*.

* *The number of threat indicators ingested by a threat intelligence source within the select timeframe.* In the example below, 872 indicators have been ingested from the AbuseCH Maleware data source, 82 indicators have been ingested from AbuseCH MalwareBazaar, and 1000 indicators have been ingested from AbuseCH URL.
* *Links to dashboards links that visualize the ingested indicator data and allow you to inspect it.* In the *Indicator* column, click *Source* for the appropriate source to open the dashboard.
+
NOTE: Dashboards are not created for sources in the *Other* category unless the source has the `threat.feed.dashboard_id` field defined.
...
To set up an {agent} integration in the Threat Intelligence view:
...
To set up a Threat Intel module integration, or a custom integration, in the Threat Intelligence view:

. Set up a way to ingest data into your system.
+
NOTE: If you're using a Threat Intel module integration, refer to the {filebeat-ref}/filebeat-installation-configuration.html[{filebeat} quick start] for instructions about installing a {filebeat} agent.

. Update the Threat indices advanced setting by adding the appropriate index pattern name after the default Fleet threat intelligence index pattern (`logs-ti*`):
...
.. If you're using using an older version of Filebeat _and_ a current one, differentiate between the threat intelligence indices by using unique index pattern names. For example, if you’re using {filebeat} version 7.0.0 and 8.0.0, add `filebeat-7*` and `filebeat-8*`.
.. If you're using a custom integration, add the custom threat intelligence indice name (for example, `custom-ti-index*`).
+
NOTE: Threat intelligence indices aren’t required to be ECS compatible. However, we recommend compatibility if you’d like for your alerts to be enriched with relevant threat indicator information. To learn more about this, read about the default threat indicator path setting (**Indicator prefix override**) in <<rule-ui-advanced-params, Configure advanced rule settings>>. You can find a list of ECS compliant threat intelligence fields at {ecs-ref}/ecs-threat.html[Threat Fields] and {filebeat-ref}/filebeat-module-threatintel.html[Threat Intel module].
