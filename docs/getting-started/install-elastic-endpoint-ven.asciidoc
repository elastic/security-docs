...
--
image::images/allow_network_filter_ven.png[]
--
...
image::images/allow_full_disk_access_notification_ven.png[]
...
+
[role="screenshot"]
image::images/select_fda_ven.png[Select Full Disk Access]
+
. Enable `ElasticEndpoint` AND `co.elastic.systemextension` to properly enable Full Disk Access.
+
[role+"screenshot"]
image::images/allow_fda_ven.png[]

If the endpoint is running {stack} version 7.17.0 or earlier:

. Click the *+* button to view *Finder*.
. The system may prompt you to enter an admin username and password if you haven't previously done so.
+
image::images/enter_login_details_to_confirm_ven.png[]
+
...
+
[role="screenshot"]
image::images/select_fda_setting_ven.png[Select Full Disk Access]
