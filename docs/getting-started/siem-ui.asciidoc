...
TIP: All {elastic-sec} histograms, graphs, and tables contain an **Inspect**
...
NOTE: The default index glob patterns defined for {elastic-sec} events are
...
Select the collapsable *Timeline* button on the right of the {elastic-sec} UI to start an investigation. See the <<timelines-ui,Timelines>> section for more information.
...
The signals histogram displays a count of all the Signals detected by the {security-app} within a set time frame. In the **Stack by** dropdown, select specific parameters for which to visualize the signal count. For example, stack by `user.name` for the graph to visualize the number of signals attributed to a specific user.
...
* External alerts: alerts received from
...
allowing you to search, filter, and aggregate all {elastic-sec} signals.
...
{security-app}. For more information, see <<cases-overview>>.
...
A timeline is responsive and persists as you move through the {security-app}
...
by the KQL bar, you can select whether signals, other raw events, or both are
...
https://github.com/ndjson/ndjson-spec[`ndjson`] file.
