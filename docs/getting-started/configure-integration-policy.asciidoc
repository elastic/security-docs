...
Malware protection also allows you to enable a blocklist of specified applications and files that are prevented from running in host environments. Use the **Blocklist enabled** toggle to enable or disable this feature for all hosts associated with the integration policy. To configure specific applications and files on the blocklist, refer to <<blocklist>>.
