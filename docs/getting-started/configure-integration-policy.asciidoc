...
After the {agent} is installed with the {endpoint-sec} integration, several protections features — including
...
2. From the **Policy** column, select the integration policy you want to configure.
The integration policy configuration page appears.
...
extending the list of processes that {endpoint-sec} considers malicious. Use the **Blocklist enabled** toggle
