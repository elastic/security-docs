...
4. Click the **Trusted applications**, **Event filters**, and **Host isolation exceptions** tabs to review the endpoint policy artifacts assigned to this integration policy (for more information on each artifact type, refer to <<trusted-apps-ov, trusted applications>>, <<event-filters, event filters>>, and <<host-isolation-exceptions, host isolation exceptions>>). On each tab, you can:
* Expand and view an artifact — Click the arrow next to its name.
* View full details for an artifact — Click the actions button (**...**), then select **View full details**.
...
* Assign an existing artifact (Platinum or Enterprise subscription) — Click **Assign _x_ to policy**, then select an application from the flyout. This view lists any existing artifacts that aren't already assigned to the current policy.

NOTE: You can't create new endpoint policy artifact while configuring an integration policy. To create a new artifact, go to its main page in the {security-app} (for example, **Manage** -> **Trusted applications** to add a new trusted application).
