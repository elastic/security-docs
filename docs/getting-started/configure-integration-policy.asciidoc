...
3. Review the following settings on the **Policy settings** tab and configure as appropriate:
* <<malware-protection>>
...
* **Unassign a trusted application** (Platinum+ subscription only): Click the actions button (**...**), then select **Remove from policy**. This does not delete the trusted application; this just unassigns it from the current policy.
* **Assign an existing trusted application** (Platinum+ subscription only): Click **Assign trusted application to policy**, then select an application from the **Assign trusted applications** flyout. This flyout lists any existing trusted applications that aren't already assigned to the current policy.

NOTE: If you want to create a new trusted application, you have to go to **Manage** -> **Trusted applications**. You can't create new trusted applications while configuring an integration policy.
...
[discrete]
[[trusted-apps-policy]]
== Manage a policy's trusted applications

The **Trusted applications** tab lists the trusted applications assigned to the current policy. On this tab, you can do the following:

* To expand and view the configuration settings for a trusted application, click the arrow next to the application's name.
* To view the full details for a trusted application, click the actions button (**...**), then select **View full details**.
* To unassign a trusted application from the policy, click the actions button (**...**), then select **Remove from policy**. This does not delete the trusted application; this just unassigns it from the current policy.
* To assign an existing trusted application to the policy, click **Assign trusted application to policy**. The Assign trusted applications flyout opens, which lists any existing trusted applications that aren't assigned to the current policy.

NOTE: If you want to create a new trusted application, you have to go **Manage** -> **Trusted applications**. You can't create new trusted applications while configuring an integration policy.