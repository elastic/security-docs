...
When *Detect* malware is enabled, {agent} will quarantine any malicious file it finds. Specifically {agent} will remove it from it's current location, obfuscate it (XOR encrypt it with the encryption key `ELASTIC`), move it to a different folder, and rename it as a string like `318e70c2-af9b-4c3a-939d-11410b9a112c`.
...
- On Linux, all {agent} versions move quarantined files to the folder `.equaratine` at the root of the mount point of the file being quarantined.
