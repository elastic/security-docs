...
Select whether you want to use **Reputation service** for additional protection. Elastic's **Reputation service** leverages our extensive threat intelligence knowledge to make high confidence real-time block decisions. For example, reputation service can detect suspicious downloads of binaries with low or malicious reputation.
