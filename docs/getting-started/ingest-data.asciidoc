...
* The {fleet-guide}/elastic-agent.html[{agent}] with the **Elastic Endpoint Integration**, which protects
your hosts and sends logs, metrics, and endpoint security data to {es-sec}. See, <<install-endpoint>>).
* The {agent} with {es-sec} integrations. To access these integrations, open the main menu, go to *Management* -> *Integrations*, then click the Security category filter. See Elastic Integrations for more information.
