...
* The {agent} with integrations, which are made available through the {fleet-guide}/fleet-overview.html#package-registry-intro[Elastic Package Registry (EPR)]. To add an integration, go the Home page and click *Add integrations*. Next, click the *Security* category filter, and then select an integration and follow the guided steps to install it.

+
TIP: For more information on integrations, refer to {integrations-docs}[{integrations}].
+
...
To populate {elastic-sec} with hosts and network security events, you need to install and
...
When you add integrations that use {beats}, you're guided through the {beats} installation process. To begin, click *Home* -> *Add integrations* and then follow the links for the types of data you want to collect.
