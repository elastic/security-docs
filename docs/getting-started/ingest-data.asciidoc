...
* The {agent} with {es-sec} integrations, which are made available through the {fleet-guide}/fleet-overview.html#package-registry-intro[Elastic Package Registry (EPR)]. To access {es-sec} integrations, open the main menu, go to **Management** -> **Integrations**, then click the Security category filter.
