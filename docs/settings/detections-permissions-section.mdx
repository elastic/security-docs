...
<DocLink id="serverlessSecurityDetectionsPermissionsSection" section="enable-and-access-detections">enable detections</DocLink>.
</DocCallOut>
...
configure ((kib)) <DocLink id="serverlessSecurityValueListsExceptions">value list</DocLink> upload limits.
...
{/* The reference to the Detections page might be a bug in classic and Serveless docs. Might need to change it to Alerts and Rules, or something different like "pages that use the Detections feature". If update this para, will need to update the table below as well. */} 
  ...
  <DocRow>
    <DocCell>
      ...
      **NOTE**: To turn on the Detections feature, visit the Detections page for each appropriate Kibana space.
      {/* When we update this table, check if this is still a req */} 
