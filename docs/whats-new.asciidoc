...
[float]
=== Exclude cold and frozen data from rule execution from all rules

Rules that query cold and frozen data tiers might perform more slowly. To ensure rules exclude query results from cold and frozen tiers when executing, configure the `excludedDataTiersForRuleExecution` <<exclude-cold-frozen-data-rule-executions,advanced setting>>. This setting does not apply to {ml} rules. 
