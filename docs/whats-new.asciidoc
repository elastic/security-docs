...
The integration that monitors events and enables threat prevention and detection on your hosts has been renamed from *Endpoint and Cloud Security integration* to *{elastic-defend}*. 
...
* If you have a http://elastic.co/pricing[Platinum subscription] or higher, you can now examine alerts associated with the event by default. 
...
** *Event renderer added to the Overview tab* - The alert rendered view displays event details, such as the alert reason statement, file paths, or process arguments, to provide context for the alert. You can take action (such as Add to Timeline) on individual fields in the statement. 
+
[role="screenshot"]
...
* Osquery results can be added to a case - After users run Osquery from an alert, they can add Osquery results to a new or an existing case. 
* Osquery Response Action - Users can use the Osquery Response Action to immediately query hosts that generate alerts. Please note that Response Actions are currently a technical preview feature. 
* When analyzing an alert, you can now add queries to a rule's investigation guide and run it as part of your investigation. 
