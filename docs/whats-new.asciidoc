...
Elastic prebuilt rules now provide additional information to help you identify, install, and configure a rule's {security-guide}/rules-ui-management.html[prerequisites]. You can confirm these requirements in the *Related integrations* and *Required fields* fields on a rule's details page, and consult the rule's *Setup guide* for additional guidance.
