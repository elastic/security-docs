...
* A newly updated {fleet-server} in {kib} requires you to redeploy your {agent}s. See [insert Fleet link here when merged] to view the appropriate steps to take to ensure your endpoints remain protected.
* New <<prebuilt-rules,prebuilt rules>>.
...
* New updates to the Administration page include updated agent statues and enhancements to <<trusted-apps-ov, Trusted applications>>, including the hash validation, improvederreor messaging, and the ability to run a simple search.
* Alert enhancements:
