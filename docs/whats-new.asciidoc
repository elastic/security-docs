...
Previously, you could only add notification actions to prebuilt rules and couldn't export or import them. Now, you can edit most prebuilt rule settings and customize them to fit your needs. When updating prebuilt rules, your customizations are preserved whenever possible and can be merged with Elastic updates. In addition, you can now export and import prebuilt rules (modified and unmodified) and bulk-update their settings from the **Rules** page.
...
You can now preview logged  {es} requests for new terms, threshold, custom, and machine learning rule types.
...
From the alert details flyout, you can click the history icon (image:detections/images/history-icon.png[History icon,15,15]) to display a list of places that you visited from the alert's details flyout—for example, flyouts for other alerts or users. Click any list entry to quickly access the item's details.
...
A new {kib} feature privilege is now required when configuring third-party response actions. To find and assign the privilege, navigate to **Management** -> **Actions and Connectors** -> **Endpoint Security**.
