...
* You must have _one_ of the following installed on the hosts you want to monitor:
    ...
    * **((filebeat))** - Install [((filebeat))](((filebeat-ref))/filebeat-installation-configuration.html) version 8.x or later. Earlier ((filebeat)) versions are incompatible with ECS and will prevent indicator data from displaying in the Indicators table.
...
* Verify that the index storing indicator documents is included in the <DocLink id="serverlessSecurityAdvancedSettings" section="update-default-elastic-security-indices">default ((elastic-sec)) indices</DocLink> (`securitySolution:defaultIndex`). The index storing indicator documents will differ based on the way you're collecting indicator data:
...
<DocCallOut title="Note">
These troubleshooting steps also apply to the <DocLink id="serverlessSecurityThreatIntelligence">Threat Intelligence view</DocLink>.
</DocCallOut>
...
After you add indicators to the Indicators page, you can <DocLink id="serverlessSecurityIndicatorsOfCompromise" section="examine-indicator-details">examine</DocLink>, search, filter, and take action on indicator data. Indicators also appear in the Trend view, which shows the total values in the legend.
...
Investigate an indicator in <DocLink id="serverlessSecurityTimelinesUi">Timeline</DocLink> to identify and predict related events in your environment. You can add an indicator to Timeline from the Indicators table or the Indicator details flyout.
...
1. Select one of the following:
    ...
    * **Add to new case**: Configure the case details. Refer to <DocLink id="serverlessSecurityCasesOpenManage" section="open-a-new-case">Open a new case</DocLink> to learn more about opening a new case. 
...
Add indicator values to the <DocLink id="serverlessSecurityBlocklist">blocklist</DocLink> to prevent selected applications from running on your hosts. You can use MD5, SHA-1, or SHA-256 hash values from `file` type indicators. 
...
<DocCallOut title="Note">
Refer to <DocLink id="serverlessSecurityBlocklist">Blocklist</DocLink> for more information about blocklist entries. 
</DocCallOut>
