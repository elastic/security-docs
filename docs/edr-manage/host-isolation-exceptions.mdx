...
You can configure host isolation exceptions for specific IP addresses that <DocLink id="serverlessSecurityHostIsolationOv">isolated hosts</DocLink> are still allowed to communicate with, even when blocked from the rest of your network. Isolated hosts can still send data to ((es)) and ((kib)), so you don't need to set up host isolation exceptions for them.
