...
For {sn}, cases are send via {sn}'s
https://developer.servicenow.com/dev.do#!/reference/api/madrid/rest/c_TableAPI[Table API].
For Jira, the https://developer.atlassian.com/cloud/jira/platform/rest/v2/[REST API v2]
is used. The https://developer.ibm.com/security/resilient/rest/[Resilient REST API]
is used to send cases to {ibm-r}.
...
<1> {siem-soln} case `title` fields are mapped to {ibm-r} `name` fields.
