...
You can only create `.lists` and `.items` indices when you have `manage`
privileges for both the
{ref}/security-privileges.html[{es} cluster and the`.lists-<Kibana space>` index].

NOTE: To make sure administrators can always create indices, use a glob 
pattern that matches indices from multiple spaces in the `Indices` field
on the *Create role* page (*Stack Management* -> *Roles* -> *Create role*).
For example, `.lists-*` and `.items-*`.
