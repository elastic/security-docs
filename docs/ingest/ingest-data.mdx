...
* The [((agent))](((fleet-guide))/fleet-overview.html) with the **((elastic-defend))** integration, which protects
    your hosts and sends logs, metrics, and endpoint security data to ((elastic-sec)). See <DocLink id="enSecurityInstallEndpoint">Install and configure the ((elastic-defend)) integration</DocLink>.
