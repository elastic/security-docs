...
* **User risk**: The latest recorded user risk score for each user, and its user risk classification. This feature requires the Security Complete <DocLink id="serverlessGeneralManageProject" text="project feature"/> and must be enabled to display the data. Click **Enable** on the **User risk** tab to get started. To learn more, refer to our <DocLink id="serverlessSecurityUserRiskScore">user risk score documentation</DocLink>.  
