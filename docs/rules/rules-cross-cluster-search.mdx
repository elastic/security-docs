...
1. On the local cluster, [set up a connection](((ref))/remote-clusters.html) to the remote cluster. Note the unique name that you give to the remote cluster, because you'll need to use it throughout this process.

1. On both the local and remote clusters, [create a role for cross-cluster search privileges](((ref))/remote-clusters.html), and make sure the two roles have _identical_ names. Assign each role the following privileges:
