...
To create or edit ((ml)) rules, you must have the [appropriate license](https://www.elastic.co/subscriptions) and user role. Additionally, the selected ((ml)) job must be running for the rule to function correctly.
</DocCallOut>
...
<DocCallOut title="Note">
To use Actions for alert notifications, you need the
[appropriate license](https://www.elastic.co/subscriptions) and user role. For more information, see <DocLink id="serverlessSecurityCasePermissions">Cases requirements</DocLink>.
</DocCallOut>
...
To understand which fields to parse, see the <DocLink id="serverlessSecurityRuleApiOverview">Detections API</DocLink> to view the JSON representation of rules.
