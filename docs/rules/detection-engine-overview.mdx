...
<DocCallOut title="Note">
To use Alerting for detection alert notifications, you need the
...
Cold tier is a [data tier](((ref))/data-tiers.html) that holds time series data that is accessed only occasionally. ((elastic-sec)) supports cold tier data for the following ((es)) indices:
...
Depending on your privileges and whether detection system indices have already been created, you might get one of these error messages when you 
