...
* <DocLink id="serverlessSecurityEndpointSecurity">**Endpoint Security**</DocLink>:
...
* <DocLink id="serverlessSecurityExternalAlerts">**External Alerts**</DocLink>: Automatically creates an alert for
...
alerts (see \<\<alerts-ui-manage>> and <DocLink id="serverlessSecurityAlertsUiMonitor">Monitor and troubleshoot rule executions</DocLink>).

{/* The following sentence is commented out because Security APIs are not yet available for Serverless. 
Once the API is available, we can uncomment it. */}
{/* You can create and manage rules and alerts via the UI or the
<DocLink id="serverlessSecurityRuleApiOverview">Detections API</DocLink>. */}
...
* ((elastic-sec)) does not support the use of frozen tier data with indicator match rules.
...
<DocLink id="serverlessSecurityDetectionsPermissionsSection">Detections prerequisites and requirements</DocLink> provides detailed information on all the
...
when the
<DocLink id="serverlessSecurityDetectionEngineOverview" section="detections-configuration-and-index-privilege-prerequisites">`xpack.encryptedSavedObjects.encryptionKey`</DocLink>
setting has not been added to the `kibana.yml` file. For more information, refer to <DocLink id="serverlessSecurityDetectionsPermissionsSection" section="configure-self-managed-((stack))-deployments">Configure self-managed ((stack)) deployments</DocLink>.
</DocCallOut>

* **`Detection engine permissions required`**

    If you get this message, you do not have the
    <DocLink id="serverlessSecurityDetectionEngineOverview" section="detections-configuration-and-index-privilege-prerequisites">required privileges</DocLink> to view the **Detections** feature,
    and you should contact your ((kib)) administrator.

<DocCallOut title="Note">
For **self-managed** ((stack)) deployments only, this message may be
displayed when the <DocLink id="serverlessSecurityDetectionEngineOverview" section="detections-configuration-and-index-privilege-prerequisites">`xpack.security.enabled`</DocLink>
setting is not enabled in the `elasticsearch.yml` file. For more information, refer to <DocLink id="serverlessSecurityDetectionsPermissionsSection" section="configure-self-managed-((stack))-deployments">Configure self-managed ((stack)) deployments</DocLink>.
</DocCallOut>
