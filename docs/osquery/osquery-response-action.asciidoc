...
When a rule generates an alert, Osquery automatically collects data on the host. Query results are displayed within the *Response* tab in left panel of the alert details flyout. The number next to the *Response Results* tab represents the number of queries attached to the rule, in addition to endpoint response actions run by the rule.
//Need to verify if the last sentence is still correct. 
