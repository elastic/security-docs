[[session-view]]
== Session view
Session View, currently in beta, is an investigation tool that allows you to view and analyze Linux process data organized
...
and investigating session activity on your Linux infrastructure, and understanding user and service behaviour.
...
2. *User information:* the Linux user that executed each session or process, and any exec user changes.
3. *Process and event telemetry:* process information included in the Linux logical event model.
4. *Nested sessions:* sessions started by processes descended from the entry session.
5. *Alerts:* alerts in the context of the processes which caused them.
