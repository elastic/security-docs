...
Terminal output view provide a more complete understanding of commands executed by users or adversaries, and assists with auditing and compliance. It allows you to review a session's terminal output in the context of the original commands. It includes both terminal output and shell commands (if the shell was in echo mode).
