...
3. *User Input:* interactive processes entered by a user into the terminal.
4. *Output:* output generated as a result of process activity.
5. *Process and event telemetry:* process information included in the Linux logical event model.
6. *Nested sessions:* sessions started by processes descended from the entry session.
