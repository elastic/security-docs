...
+
[role="screenshot"]
image::images/session-view-output-badge.png[The Output badge,width=80%,height=50%]

[[session-view-output]]
[discrete]
=== Terminal output view UI
Terminal output view can help provide a more complete understanding of commands executed by users or adversaries, and assist with auditing and compliance objectives. It allows you to review a session's TTY output, in the context of the commands which generated it. It includes both TTY output, and commands entered into a shell as typed — provided the shell was in echo mode (for background information about Linux terminals, refer to https://github.com/elastic/security-team/blob/main/docs/adaptive-workload-protection-team/architecture/linux-terminals.md[Linux terminals]).

[role="screenshot"]
image::images/session-view-output-viewer.png[Terminal output view,width=80%,height=80%]

1. The search bar. Use it to find and highlight search terms within the current session.
The buttons on its right side allow you to jump through search results.
2. The right-side scroll bar. Use it along with the bottom-side scroll bar to navigate output data that doesn't fit on a single screen.
3. The playback controls: *Start*, *Prev*, *Play*, *Next*, and *Last*. Use them to advance or rewind the session's commands and output.
4. The *Fit screen*, *Zoom in*, and *Zoom out* buttons. Use them to adjust the text size.
