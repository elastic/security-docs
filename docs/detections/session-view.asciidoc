...
* The *Output* badge appears next to commands that generated terminal output. Click it to open the terminal output view.
+
[role="screenshot"]
...
* Terminal output that appears in the `process.io.text` field includes https://gist.github.com/fnky/458719343aabd01cfb17a3a4f7296797[ANSI codes] that represent, among other things, text color, text weight, and escape sequences. For maximum reliability, you should post-process this data before using it for alert generation. Additionally, rule queries using this data should include leading and trailing wildcards (for example `process where process.io.text : "*hello world*"`), since output events typically include multiple lines of output.
* Most ANSI codes are stripped out of the data when it appears in terminal output view, but some codes remain, such as `\b`, the escape code for a backspace. For example, if a user accidentally entered `sdo` instead of `sudo`, then pressed backspace twice to fix the typo, the output appearing in terminal output view would be `sdo\b\budo`.
...
* Rules based on output data will identify which `process.io.text` value matched the alert query, without identifying which specific part of that value matched.
