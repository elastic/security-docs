...
[float]
[[data-tier-detections]]
== Compatibility with cold and frozen tier nodes

Cold and frozen tier are {ref}/data-tiers.html[data tier] that holds time series data that are accessed only occasionally and rarely, respectively. In {stack} version >=7.11.0, {elastic-sec} supports cold tier data for the following {es} indices:
