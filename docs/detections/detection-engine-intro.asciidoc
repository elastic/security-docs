...
* Indicator match rules don't support cold or frozen data but will query cold and frozen {ref}/data-tiers.html[data tiers] if they match the rule's index pattern. To exclude query results from cold and frozen tiers, create a <<exclude-cold-frozen-data-individual-rules,Query DSL filter>> and add it to individual rules. 
