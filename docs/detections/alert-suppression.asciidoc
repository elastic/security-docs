...
* New terms rule: <INFORMATION PENDING>
--
+
[NOTE]
======
...
* **Threshold and event correlation (non-sequenced queries only)** - The maximum number of alerts is the value specified for the <<opt-fields-all,`max_signals`>> setting, which is `100` by default.
