...
. Under *If a suppression field is missing*, select how to handle events with missing suppression fields (one or more of the *Suppress alerts by* fields either don't exist on an event or they have a `null` value):
+
--
