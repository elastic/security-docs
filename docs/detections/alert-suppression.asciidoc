...
Except for custom query rules (which don't have an alert suppression limit), each rule type has a maximum number of alerts that can be suppressed:

* **Threshold** - The maximum number of alerts that can be suppressed is the value specified for the <<opt-fields-all,`max_signals`>> setting, which is `100`.
* **Indicator match** - The maximum number is five times the value specified for the <<opt-fields-all,`max_signals`>> setting, which is `500`.