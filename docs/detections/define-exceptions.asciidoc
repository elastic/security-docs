[[define-add-exceptions]]
== Define and add rule exceptions 
...
. In the *Add Rule Exception* flyout, name the exception and add conditions that define when the exception prevents alerts. In the example below, the exception prevents the rule from generating alerts when the `svchost.exe` process runs on hostname `siem-kibana`.
+
[role="screenshot"]
...
+ 
TIP: If a shared exception list doesn't exist, you can <<shared-exception-list,create one>> from the Rule Exceptions page.
