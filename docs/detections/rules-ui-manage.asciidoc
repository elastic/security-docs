...
+
IMPORTANT: After upgrading to 8.8, frequency settings for rule actions created in 8.7 or earlier are converted from the rule level to the action level. Action schedules remain the same and will continue to run on their previously specified frequency (`On each rule execution` `Hourly`, `Daily`, or `Weekly`). Rules without actions will be snoozed indefinitely. 
