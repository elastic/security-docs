...
Instead of turning off rules to stop alert notifications, snooze rule actions for a defined period of time. When you snooze rule actions, the rule continues to run on its defined schedule but alerts won't trigger any actions and notifications aren't sent. 

You can snooze notifications for a specified period of time or indefinitely. You can also schedule and manage recurring downtimes. When rule notifications are snoozed, you can cancel or change the duration of the snoozed state.

You can snooze rule notifications from the Rules table, rule's details page, or *Actions* tab when editing a rule

[role="screenshot"]
image::images/rule-snoozing.png[Rules snooze options,75%]
