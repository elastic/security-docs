...
NOTE: Rules must be enabled for you to manually run them.
...
. Specify a time range for the rule run. By default, the time range is set to the current day, and three hours in the past.
. Click **Run** to manually execute the rule.

The manual run's details are recorded in the <<rule-notifications,Manual runs>> table on the the *Execution results* tab. 
