...
If you try to bulk edit with both prebuilt and custom rules selected, the action is applied only to the custom rules.
====
...
[NOTE]
====
You cannot export Elastic prebuilt rules, but you can duplicate a prebuilt rule and then export the duplicated rule.

If you try to export with both prebuilt and custom rules selected, only to the custom rules are exported.
====
