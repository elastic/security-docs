...
You can also check rules' related integrations in the *Rules* and *Rule Monitoring* tabs. Click the integrations label to display the related integrations in a popup.

// [role="screenshot"]
// image::images/related-integrations-rules-table.png[Rules table with integrations label highlighted]

TIP: You can hide the related integrations label in the rules tables. Go to *{kib}* -> *Stack Management* -> *Advanced Settings*, then turn off `securitySolution:showRelatedIntegrations`.
