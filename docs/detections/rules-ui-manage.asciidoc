...
The rule runs over the time range that you selected. Note that all <<rule-notifications,alert actions>> for the rule are activated, except for **Summary of alerts** actions that run at a custom frequency.
