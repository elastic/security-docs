...
Instead of turning rules off to stop alert notifications, snooze rule actions for a defined time period. When you snooze rule actions, the rule continues to run on its defined schedule, but won't perform any actions and send alert notifications. 
