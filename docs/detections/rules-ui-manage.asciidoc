...
When you snooze rule actions, the rule continues to run on its defined schedule but alerts won't trigger any actions and notifications aren't sent. You can snooze notifications for a specified period of time, indefinitely, or schedule single or recurring downtimes. If you don't specify a time period, the default snooze period is three days. When rule notifications are snoozed, you can cancel or change the duration of the snoozed state.
