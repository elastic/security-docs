...
[float]
[[bulk-fill-gaps-multiple-rules]]
=== Fill gaps for multiple rules 

From the Rules table, fill gaps for multiple rules by using the **Fill gaps** bulk action.

. Find *Detection rules (SIEM)* in the navigation menu or by using the {kibana-ref}/introduction.html#kibana-navigation-search[global search field]. 
. In the Rules table, click the **Rule Monitoring** tab, then click the **Only rules with unfilled gaps** filter in the panel above the table. This filters the Rules table to only show rules without filled gaps.
+
NOTE: The **Only rules with unfilled gaps** filter will exclude rules with partial gaps or gaps that are being filled. 
+
. Select the rules with gaps that you want to fill. 
. Click **Bulk actions -> Fill gaps**.
. Specify when to start and end the manual run that will fill the gaps. 
. Click **Schedule gap fills**. The rule will manually run during the selected time range. 
...
Alternatively, you can check gap fill progress for individual rules by going to their details page, clicking the **Execution results** tab, and going to the <<gaps-table>>.
