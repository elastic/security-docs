...
Manually run enabled rules to execute them instantly. 
...
. Specify a time range for the manual run. By default, the time range is set to the current day and three hours in the past.
