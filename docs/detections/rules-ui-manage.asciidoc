...
Follow these steps to start using the {es-sec-app}'s <<prebuilt-rules, prebuilt rules>>, or reinstall them if you deleted them. 
...
. Go to *Manage* -> *Rules*, then click *Add Elastic rules*. The button's badge shows the number of uninstalled prebuilt rules available. 

. On the *Add Elastic Rules* page, do one of the following:
...
+
TIP: Use the search bar and *Tags* filter to find the rules you want to install. For example, filter by `OS: Windows` if your environment only includes Windows endpoints.

. Go back to the *Rules* page, search or filter for any rules you want to start running, then do the following:
...
+
TIP: Use the search bar and *Tags* filter to find the rules you want to update. For example, filter by `OS: Windows` if your environment only includes Windows endpoints.
