...
. Click **Schedule gap fills**. The rule will manually run over unfilled gaps in the selected time range. 
...
Alternatively, you can check gap fill progress for individual rules by going to their details page, clicking the **Execution results** tab, then going to the <<gaps-table>>.
