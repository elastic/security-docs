...
Similarly, rules will be skipped if they can't be modified by a bulk edit. For example, trying to apply a tag to rules that already have that tag, or trying to apply an index pattern to rules that use data views.
====
