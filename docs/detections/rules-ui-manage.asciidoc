...
* Index pattern — Enter an index pattern (such as `filebeat-*`) to display all rules using that index pattern. 
...
NOTE: Index patterns and MITRE ATT&CK tactics and techniques must be an exact match, are case sensitive, and wildcards are _not_ supported. For example, the index pattern `filebeat-*` is valid, but `filebeat*` is not. Likewise, the MITRE ATT&CK tactic `Defense Evasion` is valid, but `Defense`, `defense evasion`, and `Defense *` are not.
