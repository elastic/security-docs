...
You can sort the rules by clicking on the *Rule*, *Last updated*, or *Enabled* column header. 

TIP: To sort by any other column, enable the *Technical preview* toggle above the table. This experimental table view allows advanced sorting capabilities. If you experience performance issues when working with the table, you can turn this setting off.
...
You can edit the settings of an existing rule. You can also select multiple rules, then edit certain settings in bulk (index patterns and tags).
...
* Edit a single rule: Click the *All actions* icon (*...*) on a rule, then select *Edit rule settings*. The *Edit rule settings* view opens, where you can modify the <<rules-ui-create, rule's settings>>.
...
. On the flyout that opens, enter the settings to change on the rules.
...
. Go to *Detect* -> *Rules*.
. Do one of the following:
* Click the *All actions* icon (*...*) on a rule, then select the required action.
* Select all the rules you want to modify, then select the required action from the *Bulk actions* menu.
* To enable or disable a single rule, click the *Enabled* toggle on the rule.
