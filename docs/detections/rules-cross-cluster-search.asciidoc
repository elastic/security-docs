...
. On the local cluster, {ref}/remote-clusters.html[set up a connection] to the remote cluster. Note the unique name that you give to the remote cluster, because you'll need to use it throughout this process.

. On both the local and remote clusters, {ref}/remote-clusters.html[create a
