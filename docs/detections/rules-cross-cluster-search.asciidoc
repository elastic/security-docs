...
+
IMPORTANT: The rule preview uses the currently logged-in user's cross-cluster search privileges, while the rule itself runs using the privileges snapshot saved in its API key at the moment of key creation. The preview results could be different from the rule's actual behavior if the user performing the preview has different privileges from what's saved in the rule's API key.
