...
Results for single queries are displayed in the *Results* tab. The number of agents that were queried and their responses are shown in the top right corner of the results table. Agent responses can be `Sucessful`, `Not yet responded` (pending), and `Failed`.
