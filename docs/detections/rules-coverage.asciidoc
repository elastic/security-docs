...
To access the **MITRE ATT&CK® coverage** page, find **Detection rules (SIEM)** in the main navigation or look for “Detection rules (SIEM)” using the {kibana-ref}/introduction.html#kibana-navigation-search[global search field], then go to **MITRE ATT&CK® coverage**.
