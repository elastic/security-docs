...
* <<create-esql-rule, *ES|QL*>>: Uses an {ref}/esql.html[Elasticsearch Query Language (ES|QL)] query to search your events and aggregate source event data. Query results are returned in a table, and the results of each row are tracked in an alert.  
+
preview::[]
