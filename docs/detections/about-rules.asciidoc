...
* <<create-esql-rule, *ES|QL*>>: Uses the Elasticsearch Query Language (ES|QL) to find events in the defined data sources and aggregate search results. 
+
preview::[]

creates an alert when one or more documents match the rule's ES|QL query.
