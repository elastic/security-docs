...
+
[NOTE] 
Identical, case-sensitive values are supported for the `is one of` or `is not one of` operators. For example, if you want to match the value `FOO` and `foo` add both values to the **Value** field. 
+
