...
Add conditions that define when the exception prevents alerts:
  ...
  Unmapped fields and fields with conflicts have a warning icon and message. Unmapped fields are fields that are missing from specified indices or don't have a type defined within the indices. Field with conflicts are fields that are defined as different types within the selected indices. If you select an unmapped field or a field with type conflicts, the exception might exhibit unexpected behavior when applied.
  ...
  <type> (number of indices): <index name>

  unmapped (number of indices): <index name>
