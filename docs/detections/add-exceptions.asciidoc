[[add-exceptions]]
== Add and manage exceptions items
You can add multiple exceptions items to a single rule. When you add an exception to a rule, the exception item is assigned to the rule's default list and can only be used by the rule. Exceptions can be created from the rule details page, the Alerts table, or the Rule Exceptions page.

TIP: You can share an exception with multiple rules by using shared exception lists. Refer to <<shared-exception-lists>> to learn more.
...
To view a rule's exceptions, open the rule's details page (*Manage* -> *Rules* -> *<Rule name>*), scroll down the page, and then select the *Rule exceptions* or *Endpoint exceptions* tabs. The tabs display the rule's default list, which is made up of all the exception items assigned to a rule. From the list, you can filter, edit, and delete exceptions.
