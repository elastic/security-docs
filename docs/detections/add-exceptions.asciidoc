...
You can add multiple exceptions to a single rule. An exception can also be reused by multiple rules. When you add an exception to a rule, you can also close all alerts that meet the exception’s criteria.

Exceptions can be created and managed from the rule details page, the Alerts table, or the Rule Exceptions page. To share exceptions with multiple rules, use <<shared-exception-lists,shared exception lists>>. 
