...
Fields with conflicts might have conflicting data types or they might be unmapped within specified indices. Conflicts with data types occur when the same field is mapped to different types across the specified indices. Unmapped fields do not have mapping definitions within the specified indices.   

IMPORTANT: Selecting a field with conflicts can cause unusual exception behavior, such as unexpected alerts from the specified indices. To avoid this, ensure you are defining field values correctly when configuring exception conditions. 

Field with conflicts are marked with a warning icon (image:images/field-warning-icon.png[Field conflict warning icon,20,20]). You can learn more about the conflict by hovering over the field. After you select the field, conflict details moved beneath the **Field** field.
...
In this example, the `_description` field mapping is `text` in the `field_caps_3`, `field_caps_4`, and `field_caps_5` indices, but is unmapped in the `field_caps_1` and `field_caps_2` indices. This could cause unexpected alerts to be generated if the exception needed to apply to the `field_caps_1` and `field_caps_2` indices.

[role="screenshot"]
image::images/warning-unmapped-fields.png.png[Warning for unmapped fields,80%] 
