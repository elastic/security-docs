...
* Be careful when adding exceptions to <<create-indicator-rule,indicator match>> rules. rules. Exceptions are evaluated against source and indicator indices, and when the exception matches events in _either_ indices, alerts are not generated. 
