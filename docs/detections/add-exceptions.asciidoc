...
[NOTE]
=======
Unmapped fields and fields defined as different types in selected indices will have a warning icon and warning message. The message will describe one or more of the following:

* How the field is mapped across the indices that are specified on the exception's rule. 
* Which indices the field is not mapped in. 

If you select an unmapped field or a field with type conflicts, the exception might exhibit unexpected behavior when it's applied.
