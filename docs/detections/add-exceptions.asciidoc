...
You can add multiple exceptions to a single rule. Exceptions can be created and managed from the rule details page, the Alerts table, or the Rule Exceptions page. 

An exception can also be reused by multiple rules. To share exceptions with multiple rules, use <<shared-exception-lists,shared exception lists>>. 
...
.. Go to *Manage* -> *Rule Exceptions*.
.. Expand the Endpoint Security Exception List or click on it to open the shared exception list's details page.
...
. Open the rule's details page (*Manage* -> *Rules* -> *<Rule name>*).
. Scroll down the page, then select the *Rule exceptions* or *Endpoint exceptions* tabs. The default rule list displays. 
+
NOTE: A rule's default rule list contains all the exception items that belong to the rule.
. From the default rule list, filter, edit, and delete exceptions.
