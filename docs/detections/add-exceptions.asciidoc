...
+
TIP: Fields with conflicts are marked with a warning icon (image:images/field-warning-icon.png[Field conflict warning icon,20,20]). Using these fields and might cause unexpected exceptions behavior. For more information, refer to <<rule-exceptions-field-conflicts>>.
