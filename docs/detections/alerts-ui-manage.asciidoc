...
* *Summary*: Shows details about the alert. If the alert has been enriched with indicator match fields, the *threat summary* section also displays the following fields
** matched.field
** matched.type
** source (threat.indicator.provider)
** first seen
** last seen
* *Threat Intel*: Shows the number of source events.
