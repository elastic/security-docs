...
When a conflict does happen, Elastic attempts to resolve it and will suggest a fix for your review. These are called _auto-resolved conflicts_. You can still update rules with auto-resolved conflicts, but we recommend that you carefully review them from the rule update flyout first. 

If Elastic can't resolve the conflict, you must manually fix it before updating the rule. These are called _unresolved conflicts_. To fix unresolved conflicts in a rule:
