...
IMPORTANT: Although you can bulk-update rules with auto-resolved conflicts, we strongly recommend reviewing them from the rule update flyout instead as a best and safer practice. 
...
When a conflict does happen, Elastic attempts to resolve it and will suggest a fix for your review. In these cases, you can still update the rule but we strongly recommend reviewing it from the rule update flyout to safely update it. 

If Elastic can't resolve the conflict, you must manually fix it before updating the rule. To manually fix a conflict:

. From the **Rule update** tab, click on the rule name or click **Review**. This opens the rule update flyout, where you can find rules with unresolved conflicts. 
+
TIP: Rules with unresolved conflicts have the `Action required` badge next to their names.
