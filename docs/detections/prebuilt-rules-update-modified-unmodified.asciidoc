...
IMPORTANT: Although you can bulk-update rules with auto-resolved conflicts, we strongly recommend reviewing them from their rule update flyouts instead as a best and safer practice. 
...
. From the rule update flyout, find rule fields with unresolved conflicts. They will have the `Action required` badge next to their names.
