...
+
NOTE: If you haven't updated the rule in a while, its original version might not be available for comparison. Instead, you will only have access to the rule's current version and the incoming Elastic update. 
...
+
NOTE: You can still update rules with auto-resolved conflicts, but we strongly recommend reviewing them beforehand to avoid overwriting intentional changes or breaking the rule. 
...
TIP: From the **Rule Updates** tab, look for the auto-resolved and unresolved conflict badges to quickly identify rules that you need to review. 

To manually fix a conflict, go to the **Rule update** tab, then:
