...
When creating a value list for a rule exception, be mindful of the list's size and data type. In general, value list exceptions are supported for all rule types, but there are some limitations for extremely large lists or certain data types. The following value list types can _only_ be used with custom query, machine learning, and indicator match rule types:
...
* If a value list can't be used due to <<manage-value-lists,size or data type>>, it will appear grayed out in the *Value* menu.
=======
