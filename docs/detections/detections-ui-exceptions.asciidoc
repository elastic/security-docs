...
[IMPORTANT]: Exceptions require that their field(s) have an appropriate (and consistent) mapping in each index being searched. Exceptions will not be applied to documents whose relevant fields are unmapped or in conflict. See {ecs-ref}[ECS] for more information about supported mappings.
