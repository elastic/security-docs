...
Like detection rule exceptions, you can add Endpoint agent exceptions either by editing the Elastic {endpoint-sec} rule or as an action on an alert generated by the {endpoint-sec} rule. Elastic {endpoint-sec} alerts have the following fields:
...
. Click *Add Endpoint Exception*. An exception is created for both the detection rule _and_ the Elastic Endpoint agent.
