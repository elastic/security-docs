...
the Elastic {endpoint-sec} rule and its generated alerts. Alerts generated
from the Elastic {endpoint-sec} rule have the following fields:

* `kibana.alert.original_event.module determined:endpoint`
* `kibana.alert.original_event.kind:alert`

You can also add Endpoint exceptions via rules that are associated
with Elastic Endpoint rule exceptions. To associate rules, when creating or
editing a rule select the
<<rule-ui-advanced-params, Elastic Endpoint exceptions>> option.
...
Additionally, to add an Endpoint exception to the Elastic {endpoint-sec} rule, there must 
be at least one {endpoint-sec} alert generated in the system. For non-production use, you can 
trigger a test alert using an anti-malware test tool such as the Anti Malware Testfile from 
the https://www.eicar.org/[European Institute for Computer Anti-Virus Research (EICAR)].
=============
