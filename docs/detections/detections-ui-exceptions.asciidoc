...
TIP: You can also use value lists as the indicator match index when creating an indicator match rule. Refer to <<indicator-value-lists>> for more information.
...
* If a list is linked to any rules, you'll get a warning appear asking you to confirm the deletion. If no rules are linked to a list, it is deleted without confirmation.
