...
Like detection rule exceptions, you can add Endpoint agent exceptions either by editing the Elastic {endpoint-sec} rule or as an action on a generated {endpoint-sec} alert. Alerts generated from the Elastic {endpoint-sec} rule have the following fields:
