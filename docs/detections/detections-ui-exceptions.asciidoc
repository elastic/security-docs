...
.. Go to the rule details page (*Detect* -> *Rules*), and then search for and  select the Elastic *{endpoint-sec}* rule.
...
.. Go to *Detect* -> *Alerts*.
.. Scroll down to the Alerts table, and from an Elastic {endpoint-sec}
