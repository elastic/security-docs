...
You can add multiple exceptions to a single rule. An exception can also affect multiple rules.
...
.. Scroll down below the rule details and select the *Rule Exceptions* tab.
+
[role="screenshot"]
image::images/exception-histogram.png[Detail of rule exceptions tab, 75%]
...
.. Scroll down to the *Trend* histogram and select the *Endpoint exceptions* tab.
...
[float]
[[manage-exceptions]]
=== Find other rules using the same exception
From the *Rule exceptions* or *Endpoint exceptions* tab, navigate to an exception list item and click *Affects X rules*. The rule or rules that the exception was added to displays.
