...
Exceptions are automatically grouped into exception lists. The table displays each exception list on an individual row, with the most recently created list at the top. Each row contains information such as the name of rule the exception list is assigned to, the dates the list was created and last edited, and options to export or delete it.
