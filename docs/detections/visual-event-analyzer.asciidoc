...
preview::[]
+
NOTE: Displaying alerts in the process tree requires a https://www.elastic.co/pricing[Platinum license]. In addition, to display it in {elastic-sec}, you must add the following feature flag to the `kibana.yml` file: `xpack.securitySolution.enableExperimental: ['insightsRelatedAlertsByProcessAncestry']` .
