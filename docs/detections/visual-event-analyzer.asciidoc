...
* Any alerts associated with the event
+
preview::[]
+
...
To examine alerts on the process, select the pill that shows the total number of alerts associated with it (*_x_ alert*). You can then click on an alert in the left pane to display its details.

In the example screenshot below, five alerts were generated against the analyzed event (`lsass.exe`). When you click the alert pill, the left pane displays the total number of alerts associated with the event, from oldest to newest. Each alert in the list shows the type of event that produce the alert (`event.category`), the event timestamp (`@timestamp`), and rule that generated the alert (`kibana.alert.rule.name`). You can click on the rule name to open more details about the alert.

preview::[]

NOTE: This feature requires a Platinum or Enterprise license, and you must add the `xpack.securitySolution.enableExperimental: ['insightsRelatedAlertsByProcessAncestry']` feature flag to the `kibana.yml` file.
