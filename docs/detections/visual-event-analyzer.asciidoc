...
To examine alerts on the process, select the pill that shows the total number of alerts associated with it (*_x_ alerts*). You can then click on an alert in the left pane to display its details.

In the example screenshot below, two alerts were generated against the first process in the tree, which is an imitation of the executable file that starts up Microsoft Internet Explorer. When you click the alerts pill, the total number of associated alerts is listed in the left pane and organized from oldest to newest. Each alert shows the event that generated it. In this example, the process that created the alert is shown by its process entity ID `0cki9lis38`. 
