...
Events are categorized based on their `event.category`, which maps to the analyzer as **event.type**. When selecting an `event.type`, a list of all events from that `event.category` appears in the left panel.
