...
Use the date and time filter to define a specific time range for analyzing the event. By default, this filter is set to the time range that was applied to the page that the event was analyzed from. 

[role="screenshot"]
image::images/date-range-selection.png[]

Select a different data view to further filter the events and alerts that are related to the analyzed event.
