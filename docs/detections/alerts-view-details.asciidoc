...
* **Threat match detected**: Shows the number of matched indicators that are present in the alert's document. This information is only available if you're examining an alert was generated from an <<create-indicator-rule,indicator match>> rule. If there are no matched indicators, or you're not examining an alert that wasn't generated from an indicator match rule, the total number of matched indicators is zero.
...
{elastic-sec} searches the alert document for the following fields:
