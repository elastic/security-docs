...
* *Alert-rendered reason statement*: Summarizes alert details in a human-readable format. Shows high-level details, including the alert severity (`kibana.alert.severity`) and the rule that generated the alert (`kibana.alert.rule.name`). Fields are interactive, hover over one to access the available actions.

* *Event renderer*: Displays relevant event details, for example file paths or process arguments, to provide context for the alert. Shows alert details in a human-readable format. Fields are interactive, hover over one to access the available actions.
+
