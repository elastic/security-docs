...
* *Insights*: Offers an additional layer of analysis by showing alerts with the same source event and cases to which the alert was added. Refer to <<alert-details-insights, Insights on alerts>> for more information.
...
The Insights section contains sub-sections for alerts that are related by the same source event and linked cases.

The sub-section for related alerts only displays if one or more alerts was created from the same source event. You can open related alerts in Timeline by clicking the *Investigate in timeline* button.

The sub-section for linked cases shows the total number and names of cases to which the alert has been added. You can click a case's name to open its details.
