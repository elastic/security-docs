...
[role="screenshot"]
image::images/alert-details-flyout-right-panel.png[Right panel of the alert details flyout, 80%]
...
[role="screenshot"]
image::images/alert-details-flyout-preview-panel.gif[Preview panel of the alert details flyout, 80%]
...
* **Field**: Shows <<investigation-section,highlighted fields>> for the alert.
* **Value**: Shows values for highlighted fields. 
...
+
NOTE: Alert documents contain the {ecs-re}ecs-allowed-values-event-kind.html#ecs-event-kind-signal[`event.kind:signal`] field-value pair.  
...
+
NOTE: Event documents contain the {ecs-re}ecs-allowed-values-event-kind.html#ecs-event-kind-event[`event.kind:event`] field-value pair.  

* **Host prevalence**: Shows the percentage of unique hosts that have identical field values. Host prevalence for each field is calculated by taking the number of unique hosts with an identical field value and dividing that number by the total number of unique hosts in your environment. 

* **User prevalence**: Shows the percentage of unique users that have identical field values. User prevalence for each field is calculated by taking the number of unique users with an identical field value and dividing that number by the total number of unique users in your environment. 
