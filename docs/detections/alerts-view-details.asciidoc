...
Within the Insights section, you can click on the title for each insight to expand or collapse it. Some sections are only offered under a certain license and might require you to also enable a feature flag.
...
* *Alerts related by process ancestry* - Shows alerts that are related by process events on the same linear branch. Note that alerts generated from processes on child or related branches are not shown. To further examine alerts, click the *Investigate in timeline* button.
+
beta::[]
+
