...
beta::[]
+
NOTE: This feature requires a Platinum or higher license. In addition, the *Include session data* setting must be enabled on your {endpoint-cloud-sec} integration policy. Refer to <<enable-session-view, Enable Session View data>> for more information.
...
beta::[]
+
NOTE: This feature requires a Platinum r license. To display it within the Insights section, you must also add the `xpack.securitySolution.enableExperimental: ['insightsRelatedAlertsByProcessAncestry']` feature flag to the `kibana.yml` file.
