...
Threat intelligence can also be found on alerts that weren't generated from indicator match rules. To find this information, {elastic-sec} queries the alert document for fields that contain known indicators. If any are found, they're logged in this section. 
