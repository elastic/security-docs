...
Use the alert details flyout to begin an investigation, open a case, or plan a response. At the bottom of the flyout, click **Take action** to find more ways to interact with the alert.
...
The About section in the right panel provides the following information:

* **Rule description**: Describes the rule's purpose or detection goals. Click **Show rule summary** to display more details about the rule within the <<preview-panel,preview panel>>.
...
The Investigation section in the right panel provides the following information:
...
Click **Entities** to expand the **Entities** sub-tab in the left panel. The **Entities** sub-tab provides more information about the user and host. 
...
* **Document count**: Shows the total number of event documents that contain identical field-value pairs. A dash (`——`) displays if there are no event documents that contain the field-value pair.
+
...
The **Response** section in the right panel shows <<rules-ui-create,response actions>> that were added to the rule associated with the alert. Click **Response** to display the response action's results in the left panel.
