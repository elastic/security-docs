...
The Threat intelligence overview shows matched indicators, which provide contextually relevant threat intelligence on the alert. 
...
The Threat intelligence overview shows two types of matched indicators:

* **Threat match detected**: Shows matched indicators that were discovered in the alert document of an alert generated from an indicator match rule. If none exist, the total number of matched indicators is zero.

* **Fields enriched with threat intelligence**: Shows matched indicators that were discovered in the alert document of an alert that _wasn't_ generated from an indicator match rule. If none exist, the total number of matched indicators is zero.
...
The Threat match detected section is only populated with data if you're examining an alert that was generated from an indicator match rule. Indicator matches are logged in this section when certain field values in the alert document match with threat intelligence data you've ingested. 
