...
* *Alerts related by source event* - Shows the ten most recent alerts created by the same source event. This can help you find alerts with a shared origin and provide more context about the source event. Click *Investigate in timeline* to examine related alerts in Timeline.
* *Alerts related by session ID* - Shows the ten most recent alerts generated during the same <<session-view, session>>. These alerts share the same session ID (a unique ID for a given Linux session).
+
beta::[]
+
