...
* *Alert-rendered reason statement*: Summarizes alert details and assembles the information into a human-readable format. High-level details are provided, including the alert severity (`kibana.alert.severity`) and the rule that generated the alert (`kibana.alert.rule.name`). Alert fields are interactive. Hover over alert fields to access multiple actions.

* *Event renderer*: Surfaces important alert details, for example, file paths or process arguments, to provide useful context for understanding the alert. Alert details are assembled into a human-readable format and alert fields are interactive.
+
NOTE: The event renderer only displays beneath the alert-rendered reason statement if an event renderer exists for the alert type.
