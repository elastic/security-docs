...
From the right panel, click **Entities** to open a detailed view of the host an user associated with the alert. The expanded view also includes risk scores and classifications (if you have a Platinum subscription or higher) and activity on related hosts and users. 
...
The Threat intelligence overview shows matched threat indicators, which provide contextually relevant threat intelligence on the alert. Within the Threat intelligence overview, data is organized into two groups.

[role="screenshot"]
image::images/threat-intelligence-overview.png[Overview of threat intelligence on the alert, 70%]

* **Detected threat matches**: Shows the number of matched threat indictors present in the alert document. This information is only available for alerts generated by an <<create-indicator-rule,indicator match rule>>. If there are no threat matches detected, the message "0 threat matches detected" displays.   
* **Fields enriched with threat intelligence**: Also shows the number of matched threat indictors present in the alert document, but this information is only available for alerts that _weren't_ generated by an indicator match rule. If no fields contain matching threat indicators, the number is zero.

[discrete]
[[expanded-threat-intel-view]]
==== Expanded threat intelligence view

From the right panel, click **Threat intelligence** to open the expanded threat intelligence view within the left panel. The expanded view shows threat intelligence that's available for the alert. 
...
To gather threat intelligence on an alert, {elastic-sec} queries alert documents from the past 30 days and searches for fields that contain known threat intelligence. {elastic-sec} looks for the following fields:

NOTE: Additional threat intelligence data is generated for indicator match alerts when indicators are matched with source events, as outlined by the conditions in the relevant indicator match rule. 
...
The Threat match detected section is solely reserved for indicator matches that were identified by an indicator match rule. Indicator matches occur when field values in an alert document match with field values in an indicator document. 

Matched indicators are ordered in reverse chronological order, with the most recent at the top. All mapped fields are displayed for each matched indicator.

**Fields enriched with threat intelligence**

Threat intelligence can also be found on alerts that weren't generated from indicator match rules. To make this data discoverable, {elastic-sec} enriches fields containing threat intelligence. Enriched fields are appended with `threat.indicator`. 

For example, if an alert document contains the `file.hash.md5` field, and the field's value matches a known indicator, {elastic-sec} appends the `file.hash.md5` field with  `threat.indicator`, which changes the field name to `threat.indicator.file.hash.md5`. The field and its value is then displayed in the Fields enriched with threat intelligence section.
...
* **Alerts related by session ID**: Shows the number of alerts that generated during the same session. 
...
The Prevalence overview shows whether data from the alert was frequently observed on other host events from the last 30 days. Prevalence calculations use values from the alert’s highlighted fields and results are displayed as percentages. Highlighted field values are considered prevalent, or _common_, if they're observed on over 10% of hosts in your environment. Highlighted field values are considered not prevalent, or _uncommon_, if they're observed on less than 10% of hosts in your environment. 
...
* **Alert count**: Shows the total number of alert documents that have identical field values, including the alert you're currently examining. For example, if the `host.name` field alert count is 5, there are five total alerts with the same `host.name` field value. The Alert document column only retrieves documents that contain the {ecs-ref}/ecs-allowed-values-event-kind.html#ecs-event-kind-signal[event.kind:signal] field-value pair.
...
* **User prevalence**: Shows the percentage of unique users that have identical field values. User prevalence for highlighted fields is calculated by taking the number of unique users with identical field values and dividing that number by the total number of unique users in your environment. 
