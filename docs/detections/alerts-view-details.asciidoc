...
Threat intelligence can also be found on alerts that weren't generated from indicator match rules. To make this data discoverable, {elastic-sec} enriches fields containing threat intelligence. Enriched fields are appended with `threat.indicator`. For example, if an alert document contains the `file.hash.md5` field, and the field's value matches a known indicator, {elastic-sec} appends the `file.hash.md5` field with `threat.indicator`.
