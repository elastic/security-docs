...
* Find basic details about the alert. This includes the rule that generated the alert, the alert's status, the date and time the alert was created, the alert's severity, and the alert's risk score Severity and risk are inherited from rule that generated the alert.
...
The preview panel shows more information about the rule associated with the alert and the alert reason. This panel overlays the right panel when you expand rule or alert reason details. 
...
The About section is in the right panel of the flyout. It has the following information:

* **Rule description**: Describes the rule's purpose or detection goals. Click *Show rule summary* to display a details about the rule.

* **Alert reason**: Describes the source event that generated the alert. Event details are displayed in plain text and ordered logically to provide context for the alert. Click *Show full reason* to display the alert reason in the event rendered format.  
+
...
The Investigation is in the right panel of the flyout. It has the following information:
...
The Visualizations section is in the right panel of the flyout. It offers a glimpse of the processes that led up to the alert and occurred after it. Click **Visualizations** to display the following following sub-sections:

* **Session view preview**: Shows a preview of <<session-view,session view>> data. [Details that are displayed] Click **Session viewer preview** to open the **Session View** tab in Timeline. 

* **Analyzer preview**: Shows a preview of the <<visual-event-analyzer,visual analyzer graph>>. The preview displays up to three levels of the analyzed event's ancestors and up to three levels of the event's descendants and children. The ellipses symbol (**`...`**) suggests the event has more ancestors and descendants that you can examine. Click **Analyzer preview** to open the Event Analyzer tab in Timeline.

[discrete]
[[insights-section]]
== Insights 

The Insights section is in the right panel of the flyout. It offers different perspectives from which you can assess the alert. Click **Insights** to display the following following sub-sections:
...
The Entities sub-section shows details about the user related to the alert and the host on which the alert was generated against.
...
The **Response** section is in the right panel of the flyout. It shows <<rules-ui-create,response actions>> that were added to the rule associated with the alert. Click **Response** to display the response action's results in the left panel.
