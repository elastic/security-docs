...
* *Alert rendered reason statement*: Summarizes the alert and renders the statement in a human-readable format. High-level alert details are provided, including the alert severity (`kibana.alert.severity`) and the rule that generated the alert (`kibana.alert.rule.name`). Alert details are interactive. Hover over them to access multiple actions.

* *Event rendered reason statement*: Surfaces important alert details to immediately provide useful context for understanding the alert. The statement is also rendered in a human-readable format and alert details are interactive.
+
NOTE: This statement only displays if an event renderer exists for the alert type.
