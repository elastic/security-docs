...
You can click on the section title for each insight to expand or collapse it. Some sections are only offered under a certain license and might require you to also enable a feature flag.

[role="screenshot"]
image::images/insights-section.png[Informational message on enriched data, 600]
...
* *Alerts related by session ID* - Shows the ten most recent alerts that were generated during the same <<session-view, session>>. These alerts share the same Session ID, which is a unique ID for tracking a given Linux session.
+
beta::[]
+
...
* *Alerts related by process ancestry* - Shows alerts that are related by process events on the same linear branch. Note that alerts generated from processes on child or related branches are not shown. To examine the alerts further, click the *Investigate in timeline* button.
+
beta::[]
+
NOTE: This feature requires a Platinum license. To display it within the Insights section, you must also add the `xpack.securitySolution.enableExperimental: ['insightsRelatedAlertsByProcessAncestry']` feature flag to the `kibana.yml` file.
