...
* *MITRE ATT&CK*: Provides relevant https://attack.mitre.org/[MITRE] framework tactics, techniques, and subtechniques.

* *Alert reason*: Provides alert details in a logically-arranged format. Shows high-level details, including the alert severity (`kibana.alert.severity`) and the rule that generated the alert (`kibana.alert.rule.name`). Fields are interactive, hover over one to access the available actions.
