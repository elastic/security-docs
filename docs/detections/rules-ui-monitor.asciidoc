...
To refresh table results, click **Refresh**. To stop a manual run execution, go to the row of an active run, and click **Stop run**.

The Manual run table displays important details such as:

* The status of each manual run:
** **Pending**: The rule is not yet running. It is waiting to be executed. 
...
* When a manual run started and the range of time that it runs.

* The number of rule executions that are failing, pending, running, and completed for a manual run.

* The total number of rule executions that are occurring during a manual run.
