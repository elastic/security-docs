...
Each detection rule keeps a log of every time it ran, its success or failure, and how long it took to do things like searching for data, creating alerts, and overall execution time. This information can help you troubleshoot a particular rule if it isn't performing as expected (for example, if it isn't creating alerts or takes a long time to run).

To access the rule execution log for a detection rule, go to *Detect* -> *Rules*, click the rule's name to open the rule details, then scroll down and click the *Rule execution logs* tab.
