...
** **Scheduled**: Rule executions that were automatically run by the detection engine and according to the rule's schedule.
** **Manual**: Rule executions that you or another user manually triggered.
...
* The **Source event time range** toggle includes more or less data in the table, pertaining to the date and time range that you choose to manually execute a rule.
...
Execution details of manual runs are logged in the Manual runs table. To access the table, go to **Rules** -> **Detection rules (SIEM)**, click the rule's name to open its details, then scroll down and select the **Execution results** tab. Scroll down again to find the Manual runs table.

To update the table results, click **Refresh**. To stop manual runs, click **Stop run**

The table displays important details about active manual runs:

* The status of the rule's manual runs:
** **Pending** The rule is not running yet. It is waiting to be executed. 
** **Running**: The rule is executing during the time range you specified. Some rules, such as indicator match rules, will take longer to complete their run.
** **Error**: The rule has encountered an error during its execution. 

* When the manual run began.

* The time range that was specified for the manual run. 

* The number of rule executions have failed and are pending, running, and completed.

* The total number of rule executions that will occur during the manual run.
