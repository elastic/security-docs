...
* The **Source event time range** toggle includes more or less data in the table, pertaining to the time range of a manual run execution.
...
To refresh table results, click **Refresh**. To stop an active run, go to the appropriate row, and click **Stop run** in the **Actions** column.
