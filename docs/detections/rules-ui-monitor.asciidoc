...
If a rule reaches or exceeds the maximum number of alerts it can generate in a single rule execution, a warning is displayed. The warning appears on the rule's details page and in the rule execution log. When a rule's max alert limit is reached, some alerts might not get created so it's important that you investigate what could have caused the rule hit its limit.   
