...
To view a summary of all rule executions, including most recent failures and execution
...
Each detection rule keeps a log of every time it ran, its success or failure, any warning or error messages, and how long it took to search for data, create alerts, and complete its execution. This can help you troubleshoot a particular rule if it isn't behaving as expected (for example, if it isn't creating alerts or takes a long time to run).

To access a rule's execution log, go to *Detect* -> *Rules*, click the rule's name to open the rule details, then scroll down and select the *Rule execution logs* tab.
...
Use these controls to narrow down what's included in the logs table:
...
* The date and time picker sets the time range of rule executions included in the table. This overrides the time range set at the top of the rule details page.
