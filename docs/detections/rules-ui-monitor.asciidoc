...
* The *Actions* column allows you to show the alerts generated from a given rule execution. Click the filter icon (image:images/filter-icon.png[Filter icon]) to create a global search filter based on the rule execution's ID value. This replaces any previously applied filters, and changes the global date and time picker to 24 hours before and after the rule execution.
