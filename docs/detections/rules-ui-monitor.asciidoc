...
For detailed information on a rule, its generated alerts, and its errors, click on a rule name in the *All rules* table. You can also perform the same actions that are available on the <<rules-ui-management, **Rules** tab>>, such as modifying or deleting rules, activating or deactivating rules, exporting or importing rules, and duplicating prebuilt rules.
