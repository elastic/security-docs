...
For information about rule execution gaps (which are periods of time when a rule didn't run), use the panel above the table. The time filter on the left allows you to select a time range for viewing gap data. The **Total rules with gaps:** field provides two metrics:

* Right metric: Tells you the remaining number of rules with unfilled gaps. 
* Left metric: Tells you the number of rules that are having their gaps resolved. 

The **Only rules with unfilled gaps** filter on the right of the panel lets you only display rules with unfilled gaps. Note that the filter excludes rules with partial gaps or gaps that are being filled. 
...
NOTE: From the Rules table, fill gaps for multiple rules with the <<bulk-fill-gaps-multiple-rules, **Fill gaps** bulk action>>
