...
Use the panel above the table to quickly identify rules with unfilled or partially filled gaps. For an overview of gaps that occurred during a certain period of time, use the drop-down menu at the far left of the panel. To understand how many rules have unfilled or partially filled gaps, check the **Total rules with gaps:** field. Then, select the **Only rules with gaps** filter to show those rules. 

[float]
[[rule-execution-logs]]
=== Execution results tab

Go to the **Execution results** tab (click the rule's name to open its details, then scroll down and select the **Execution results** tab) to access the rule’s execution log, monitor gaps in the rule’s execution schedule, and get details about manual rule runs. These details are stored in tables that you can access by scrolling through the tab. 
...
Gaps in rule executions are periods of time where a rule didn’t run as expected. These gaps can be caused by various disruptions, including system updates, rule failures, or simply turning off a rule. Addressing gaps is essential for maintaining consistent coverage and avoiding missing alerts. 

The Gaps table provides you with a detailed view of gaps in a rule’s execution schedule. Use this information to assess gaps and then fill them. 

TIP: To prevent future gaps, consider <<troubleshoot-gaps,troubleshooting them>> or creating plans for disruptive events.

<image>

In the Gaps table, you can:

* Fill gaps: Go to the gap that you want to fill, then click **Fill gap** to manually run the rule during the gap’s time range.
+
NOTE: If you stop a manual run that was started to fill a gap, the gap’s status is set to `Partially filled`. Click **Fill remaining gap** to start a new manual run that will cover the remaining gap.
+
TIP:  For more control over the range of time that you run the rule, start a <<manually-run-rules,manual run>> instead of using the **Fill gap** option.
+
* Filter the table: Use the **Status** filter to only display gaps that are `Filled`, `Partially filled`, or `Unfilled`. To view gaps that occurred during a specific time frame, use the date and time picker. 
* Learn about the gap: Review each of the columns in the table to understand important details about the gap: 

** **Status**: The current state of the gap. It can be `Filled`, `Partially filled`, or `Unfilled`.
** **Detected at**: The date and time that the gap was first discovered.
** **Manual fill tasks**: The status of the manual run that’s filling the gap.
** **Event time covered**: How much progress has been made on the gap fill task. 
** **Range**: The timestamps for the beginning and end of the gap. 
