...
Note: For a host isolation exception the values for `summary.windows`, `summary.linux`, `summary.macos` and `summary.total` are all the same, as each host isolation exception entry applies to all the OSs.
For example:
