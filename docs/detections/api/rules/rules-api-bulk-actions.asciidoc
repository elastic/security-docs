...
Contrary to the above example, that returns updated rule objects in `attributes.results`, in case of dry run mode, no rules objects will be returned, as rules are not getting saved in Elasticsearch.
