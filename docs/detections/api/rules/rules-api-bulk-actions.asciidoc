...
|`dry_run` |Boolean | If set to `true`, in dry_run mode rules be validated depends on action, but rules won't be saved in ES
|No
|==============================================
...
[discrete]
==== Dry run mode
In dry run mode (when URL query parameter `dry_run=true`) rules will be validated depends on action, but rules won't be saved in ES.
...
The following request adds index pattern `test-*` to the rules with IDs sent in the payload.
Because, it's in dry ryn mode: no changes will be saved for this rule. But in response, we will be able to see how many rules will be updated if the actual bulk edit happens and which errors should we expect in that case.
[source,console]
--------------------------------------------------
...
Response payload returns 2 failed rules and 1 successful. In `attributes.errors` section of response also returned errors for failed rules and ids/name of failed rules.
Contrary to the above example, which returns updated rule objects in `attributes.results`, in case of dry run mode, no rules objects will be returned, as rules are not getting mutated.
