...
**NOTE**: In {elastic-sec} 8.8 and later, the rule-level `throttle` setting is applied to any actions missing action-level frequency settings and is shown in the `frequency` field for individual actions. For more information about action level frequency settings, refer <<optional-actions-fields>>.
