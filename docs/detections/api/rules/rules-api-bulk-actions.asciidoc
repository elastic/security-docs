...
Notice that there are no arrays in `attributes.results`. In dry run mode, rule updates are not applied and saved to {es}, which means the endpoint won't return results for rules that have been `updated`, `created`, or `deleted`.
