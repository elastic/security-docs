...
|`dry_run` |Boolean | Enables dry run mode for the request call
|No
|==============================================
...
[discrete]
==== Dry run mode
In dry run mode (when URL query parameter `dry_run=true`) rules will be validated depends on action, but rules won't be saved in Elasticsearch.
...
Contrary to the above example, which returns updated rule objects in `attributes.results`, in case of dry run mode, no rules objects will be returned, as rules are not getting updates ans saves in Elasticsearch.
