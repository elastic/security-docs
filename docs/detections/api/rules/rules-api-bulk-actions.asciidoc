...
TIP: If `dry_run=true` is present at the end of the URL query parameter, rules specified in the request will be run in dry mode. 

IMPORTANT: Because `export` action doesn't mutate state of rules and returns response in `.ndjson` file, dry run mode is not supported for this action
...
The following request will validate that the `add_index_patterns` bulk action can be successfully applied to three rules. Each rule (which specified by its rule ID) is different: one is a prebuilt Elastic rule, another is a custom machine learning rule, and another is a custom query rule. Because dry run mode is enabled, changes to these rules will not be permanently applied and saved to {es}.
...
The `attributes.errors` section of the response shows that two rules failed to update and one succeeded. The same results would be returned if you ran the same request without dry run mode enabled.
Notice that there are no arrays in `attributes.results`. In the dry run mode, rule updates are not applied and saved to {es}, which means the endpoint won't return results for rules that have been `updated`, `created`, or `deleted`.
