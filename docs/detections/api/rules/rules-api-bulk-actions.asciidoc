...
| Object that describes applying an manual rule run action.
|No.
...
- `attributes.results.updated`: Returns rule objects that were schedule for manual rule runs during the `run` action execution
