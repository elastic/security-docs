...
For `enable`, `disable`, `delete`, `edit`, and `duplicate` actions, a JSON object containing the action's outcome: number of failed and successful outcomes, number of rules, and number of rule objects created, deleted, skipped or updated (depending on the action).

Rules are only `skipped` when performing an  `edit` action, when all the operations to be performed on any specific rule results in a no-op. For example, if the `edit` action is to add a tag to a rule that already has that tag, or to delete an index pattern that doesn't exist. Skipped rules objects will only return their `id`, `name`, and `skip_reason`.
