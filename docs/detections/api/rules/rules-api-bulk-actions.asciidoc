...
|frequency |String a|Object containing action’s frequency:

* `summary` (Boolean, required): Defines whether notification will be sent for summary of alerts or for each generated alert individually.
...
|alerts_filter |Object a|Object containing action’s conditional filters:

* `timeframe` (Object, optional): Object containing the time frame during which this action can be fired.

** `days` (Array of integers, required): List of days of the week on which this action can fire, expressed as numbers between `1-7`, where `1` is Monday and `7` is Sunday. An empty array is equivalent to all days of the week being selected.
** `hours` (Object, required): The hours of the day during which this action can fire. Expressed as two strings in the format `hh:mm` in `24` hour time. A start of `00:00` and an end of `24:00` means the action can fire all day.
...
* `query` (Object, optional): Object containing a query filter to apply to the action to determine whether or not it should fire.
...
**NOTE**: In {elastic-sec} 8.8 and later, we don't recommend using this field as actions are now set on an individual action level using `frequency` field`.  If some of the actions don’t have frequency attribute set then the `throttle` field will be converted into an action level frequency and set to any action without a frequency.
