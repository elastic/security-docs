...
Applies a bulk action to multiple rules. The bulk action is applied to all rules that match the filter or to the list of rule by their ids.
...
| `query` | String |  A string containing a KQL search query to match the rules. | No
| `ids` | String[] |  Array of rule ids, to which bulk action will be applied. Only valid when query is property is undefined | No
...
| Edit object that describes applying update action 
...
Edit object consists of 2 properties:

* `type` - enum - that defines what will be updated in rules
* `value` - any - value which will be applied in edit action.
...
Actions(<<bulk-edit-object-schema, BulkEditAction[]>> ) in `edit` array payload's property are applying in a historical order(from first to last).
...
For `enable`, `disable`, `delete`, `edit`, and `duplicate` actions, a JSON object containing the outcome of the action: number of failed, succeeded, total rules, as well as full rule's objects(either created, deleted or updated depends on action) in array.
...
`500`::
    Partial failure of bulk action

===== Response payload

If processing of any rule fails, partial error will be return with id/name of affected rule and corresponding error together with successfully processed rules(same format as for successful 200 request)
