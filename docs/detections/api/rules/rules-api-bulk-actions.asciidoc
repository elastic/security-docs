...
| `ids` | String[] |  Array of rule ids, to which bulk action will be applied. Only can be defined when query is property is undefined | No
...
| Yes
| `edit` | <<bulk-edit-object-schema, BulkEdit[]>> 
...
If processing of any of rule fail, partial error will be return with id/name of affected rule and corresponding error together with successfully processed rules(same format as for successful 200 request)
