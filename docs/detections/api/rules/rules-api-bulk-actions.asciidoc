...
* `timeframe` (Object, optional): Object containing the time frame for when this action can be activated.

** `days` (Array of integers, required): List of days of the week on which this action can activate. Days of the week are expressed as numbers between `1-7`, where `1` is Monday and `7` is Sunday. To select all days of the week, enter an empty array.
