...
The following request validates `add_index_patterns`` action to the rules, which IDs included in the payload.
Because, it's in dry ryn mode: no changes will be saved for this rule.
