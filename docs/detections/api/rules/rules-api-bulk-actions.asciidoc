...
Rules can only be `skipped` when performing an  `edit` action, when all the operations to be performed on any one rule result in a no-op. For example, if the `edit` action is to add a tag to a rule that already has that tag, and to delete an index pattern that doesn't exist. Skipped rules objects will only return their `id`, `name`, and `skip_reason`.
