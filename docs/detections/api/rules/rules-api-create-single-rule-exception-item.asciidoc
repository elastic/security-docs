...
TIP: For more information about creating exceptions that are used by multiple rules, refer to {api-kibana}/operation/operation-createexceptionlistitem[Create an exception list item]. For more information about creating exception items from a list, such as a list of IP addresses or hosts names, refer to <<lists-api-overview>>.
