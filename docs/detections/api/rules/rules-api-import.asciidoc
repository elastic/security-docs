...
NOTE: Your role needs at least `Read` privileges for the `Action and Connectors` feature to import rules with actions. If you are importing rules without actions, no `Action and Connectors` feature privileges are required. Refer to <<enable-detections-ui>> for more information.
