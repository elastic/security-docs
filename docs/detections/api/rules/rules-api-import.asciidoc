...
[IMPORTANT]
=================
Rule actions and the connectors used by the actions are included in the exported file, but sensitive information about the connector (such as authentication credentials) is _not_ included. During the rule import process, you are prompted to re-add missing connector details.
