...
** days  (`Array<number>, required`): List of days of the week on which this action can fire, expressed as numbers between `1-7`, where `1` is Monday and `7` is Sunday. An empty array is equivalent to all days of the week being selected.
...
** filters  (Array<Filter>, required): A list of `kbn-es-query` filters
