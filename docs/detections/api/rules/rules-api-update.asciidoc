...
===== Optional alert suppression fields for query, indicator match, threshold rule, event correlation (non-sequence queries only), and new terms rules

preview::[]

====== Query rule, indicator match rule, event correlation (non-sequence queries only), and new terms rules
