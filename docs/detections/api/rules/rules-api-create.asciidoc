...
* `threat_match`: rule that matches event values with values the specified {es}
index.
...
[[req-fields-threat-match]]
===== Required fields for threat-match rules
...
Threat-match rule that creates an alert when one of the following is true:
...
POST api/detection_engine/rules
{
    ...
    "description": "Checks the ip-threat-list index for bad IP addresses"
}
--------------------------------------------------
// KIBANA
...
`ip-threat-list`.
<4> Sibling `entries` are evaluated using `or` logic. In this example,
an alert is generated when at least one `entries` object evaluates to
`true`.
