...
* `ecs_mapping` (object, required): Map Osquery results columns or static values to Elastic Common Schema (ECS) fields.
...
For Endpoint Security (`.endpoint`), specify an endpoint response action command and provide an optional a comment:

* `command` (string, optional): To run an endpoint response action, specify a value for the `command` field. The only action that's available is the isolate host response action (`isolate`). Example: `"command": "isolate"` 
* `comment` (string, optional):  Add a note that explains or describes the action. You can see your comment in Response action history log. Example: `"comment": "Check processes"` 
