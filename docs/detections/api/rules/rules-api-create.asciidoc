...
|required_fields |Object[] a| Elasticsearch fields and their types that need to be present for the rule to function. The object has these fields:
...
*NOTE*: The value of `required_fields` does not affect the rule's behavior. It can be thought of as a way to document the fields that the rule expects to be present in the data. Specifying `required_fields` incorrectly won't cause the rule to fail.
