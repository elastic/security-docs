...
* For Osquery (`.osquery`):
** `tbd` (string, required): TBD
* For Endpoint Security (`.endpoint`):
** `command` (string): Isolate
** `comment` (string, optional): Attach a comment to this action's history log. The comment text will appear in associated cases. Defaults to
`TBD`. Maximum length is XXXX characters.
