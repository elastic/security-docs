...
to define values that prevent a rule from generating alerts. Lists are made up
of:
...
`lists` index (the system index used for storing exception lists).
* `all` privileges for the {kibana-ref}/xpack-spaces.html[`Security` feature].
