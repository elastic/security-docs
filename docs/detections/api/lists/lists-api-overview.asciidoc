...
To turn on the lists feature, a user with the `manage` cluster privilege must visit the Rules page in your Kibana space. 

To create list containers and items, a user must have the following privileges: 

* `read` and `write` index privileges for the `.lists` and `.items` data streams (the system data stream used for storing exception lists).
