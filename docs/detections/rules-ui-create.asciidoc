...
Use {ref}/esql.html[ES|QL] to query your source events and aggregate event data. Query results are returned in a table with rows and columns. Each row becomes an alert.

To create an ES|QL rule:

. Go to *Rules* -> *Detection rules (SIEM)* -> *Create new rule*. The *Create new rule* page displays.
. Select **ES|QL**, then write a <<esql-rule-query-types,query>>. 
+
...
ES|QL rule queries are loosely categorized into two types: <<esql-agg-query,aggregating>> and <<esql-non-agg-query,non-aggregating>>. 

[float]
[[esql-agg-query]]
===== Aggregating query 

An aggregating query uses {ref}/esql-functions-operators.html#esql-agg-functions[`STATS...BY`] functions to aggregate source event data. Generated alerts only contain the fields returned by the query.
...
- Then, it filters events where the `host_count` field is encountered more than 20 times during the specified rule interval.

NOTE: Aggregating queries might create duplicate alerts if events that occurred in a rule's additional look-back time are aggregated during the rule's current and previous rule execution cycles.

[float]
[[esql-non-agg-query]]
===== Non-aggregating query 
A non-aggregating query doesn't use `STATS...BY` functions and doesn't aggregate source data. Generated alerts contain source event data and fields returned by the ES|QL query. 
Here is an example of a non-aggregating query:
[source,esql]
-----
...
| LIMIT 10
-----
- This query starts by querying logs from indices that match the pattern `logs-*`. The `[metadata _id, _index, _version]` operator allows alert deduplication.  
...
To deduplicates alerts, the query needs access to the `_id`, `_index`, and `_version` metadata fields within the queried source event documents. You can allow this by adding the `[metadata _id, _index, _version]` operator after the `FROM` source command, for example:
...
Once the metadata fields are provided, a unique alert ID is created for each alert generated by the query.

As you continue developing the query, make sure you don't {ref}/esql-commands.html#esql-drop[`DROP`] or filter out `_id`, `_index`, or `_version` metadata fields. Here is an example of an invalid query that uses the `DROP` command to omit the `_id` property from the results table:
