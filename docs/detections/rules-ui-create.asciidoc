...
the indicator index patterns. The default query (`@timestamp > "now-30d"`) checks the indicator index patterns for matched indicators that have been ingested during the past 30 days.
