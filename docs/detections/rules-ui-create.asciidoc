...
.. **Max alerts per run** (optional): Specify the maximum number of alerts the rule will create each time it runs. Default is 100.
+
NOTE: Do not set this value higher than the {kibana-ref}/alert-action-settings-kb.html#alert-settings[{kib} configuration setting] `xpack.alerting.rules.run.alerts.max`, which determines the maximum alerts generated by _any_ rule in the {kib} alerting framework.
