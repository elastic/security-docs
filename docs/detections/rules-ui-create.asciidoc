...
* <<create-new-terms-rule, *New terms*>>: Generates an alert for each new term it detects in source documents.
...
When you create a rule, you must specify the {es} index pattens for which you'd like the rule to run,  or select a {kibana-ref}/data-views.html[data view] as the source of data. If you select a data view, when you create a rule (with the exception of a {ml} rule), you can select runtime fields associated with that data view.
...
alerts.
+
NOTE: NOTE: You can use {kib} saved queries (image:images/saved-query-menu.png[Saved query menu,17,17]) and queries from saved Timelines (`Import query from saved Timeline`) as rule conditions.
+
.. Use the `Fields` menu to select a field to check for new terms.
.. Use the `History Window Size` to specify the time range to search in minutes, hours, or days to determine if a term is new.
..Continue with <<preview-rules, previewing the rule>> (optional) or click *Continue* to <<rule-ui-basic-params, configure basic rule settings>>.
...
You can preview any rule to learn how noisy it will be before saving it. If necessary, you can then adjust the rule's query.
...
However, if you know your data source has an inaccurate `@timestamp` value, it is recommended you check the *Do not use @timestamp as a fallback timestamp field* option to ignore the `@timestamp` field entirely.
+
