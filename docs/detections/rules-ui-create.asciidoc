...
* To ensure rules don't search cold and frozen data when executing, configure the `excludedDataTiersForRuleExecution` advanced setting, or add a {ref}/sql-rest-filtering.html[Query DSL] filter to individual rules. Refer to <<exclude-cold-frozen-data-rule-executions>> to learn more.
==============
