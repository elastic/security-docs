...
.. *Indicator prefix override* (optional): Specify the document prefix containing your indicator fields. This setting is used to search for threat intelligence that will be used to enrich indicator match alerts. If your indicator data contains indicator fields at a different location, update this setting accordingly to ensure alert enrichment can still be performed.
