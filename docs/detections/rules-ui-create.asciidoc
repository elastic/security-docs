...
. preview:[] (Optional) Use *Suppress Alerts By* to reduce the number of repeated or duplicate alerts created. Enter a field name to group the matching source events by the field's unique values, and only one alert will be created for each group of events. You can also enter multiple fields to group events by unique combinations of values.
+
You'll still be able to view the original source events by investigating the alert in Timeline. Refer to <<suppress-alerts>> for more information.
