...
+
** **Aggregating query**: Uses {ref}/ master/esql-agg-functions.html[`STATS...BY`] functions to aggregate data. Results from each query row are not saved to an alert document within Elasticsearch. 
+
...
-----
+
This query aggregates TBD.

** **Non-aggregating query**: Does not use `STATS...BY` functions and doesn't aggregate data. To allow alert deduplication, and save results from each row of the query, you must add the [metadata _id, _index, _version]` operator after the query's data source. 
+ 
IMPORTANT: To keep alert deduplication turned on, do not drop, or filter out, the `id`, `_index`, `_version` metadata properties in any part the query. 
+
