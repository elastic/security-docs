...
The Osquery Response Action allows you to include live Osquery queries with a custom query rule. Queries are ran every time the rule executes and results are stored in alert details. To learn more, refer to <<osquery-response-action>>.
