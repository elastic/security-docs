...
TIP: When threat matches are found against a list, those matching fields and values are not populated in the alert details. As such, when you <<rule-ui-basic-params, configure basic rule settings>>, it is recommended that you specify those values in the rule `Name`, `Description`, and in the <<timelines-ui, Timeline template>> so you can identify what was matched against the list.
