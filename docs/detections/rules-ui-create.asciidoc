...
A non-aggregating query doesn't use `STATS...BY` functions and doesn't aggregate source event data. Generated alerts contain source event data and fields returned by the ES|QL query. 
...
===== Deduplicating alerts for rules that use non-aggregating queries
