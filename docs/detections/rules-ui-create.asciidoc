...
+
NOTE: If you're suppressing alerts based on a field that has multiple values, an alert grouping is created for each value. For example, if you select to suppress alerts by IP of `[a, b, c]`, alerts will be suppressed for those matching values `a`, `b`, and `c` separately.
