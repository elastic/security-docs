...
** *Quick query preview*: Allows you to select from pre-defined timeframes -- *Last hour*, *Last day*, or *Last month* -- when previewing rule results. Note that threshold and event correlation rules have limited timeframe options.
+
The rule interval and look-back time are also pre-defined for the preview and differ by rule type. These settings cannot be modified.
