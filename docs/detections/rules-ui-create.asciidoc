...
. Check to see if the {ml} jobs required for the rule are running. If a required {ml} job is not running, an alert is displayed.
+
Required {ml} jobs will automatically start when you finish configuring the rule and enable it, but you can also start {ml} jobs while you're still configuring the rule:
