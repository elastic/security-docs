...
** *Summary of alerts*: Select this option if want to reduce the number of notifications you receive without affecting their timeliness. Alert notifications that summarize alerts will be sent at your preferred time intervals. 
+
...
If the rule’s action frequency is a summary of alerts, it passes variables that summarize alerts. Refer to {kibana-ref}/rule-action-variables.html#alert-summary-action-variables[Action frequency: Summary of alerts] for additional alert summary variables. 
...
If the rule’s action frequency is not a summary of alerts (per each alert), it passes the following variables:
