...
the indicator index patterns. The default query (`@timestamp > "now-30d/d"`) checks the indicator index patterns for matched indicators that have occurred in the past 30 days. The beginning of the query period, which is 30 days ago from `now`, is rounded down to the start of the day and resolves as UTC `00:00:00`.
