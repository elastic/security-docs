...
.. *Default risk score*: A numerical value between 0 and 100 that indicates the risk of events detected by the rule. This setting is set to a default value when you change the *Severity* level, but you can also adjust the risk score as needed. General guidelines are:
