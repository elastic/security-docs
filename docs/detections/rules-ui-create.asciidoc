...
* To ensure rules don't search cold and frozen data when executing, either configure the <<exclude-cold-frozen-data-rule-execution, `excludedDataTiersForRuleExecution`>> advanced setting (which applies to all rules in a space), or add a <exclude-cold-frozen-data-individual-rules, Query DSL filter>> to individual rules. 
==============
