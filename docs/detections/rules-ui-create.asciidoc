...
.. *Custom highlighted fields* (optional): Specify one or more highlighted fields for personalized alert investigation flows. You can choose any fields that are available in the <<update-sec-indices,default {elastic-sec} indices>>. There is no limit to the number of fields that you can add. 
+
