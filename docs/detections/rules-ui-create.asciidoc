...
+
NOTE: An array contains each alert generated since the last time the action was executed. For example, ``{{#context.alerts}}Detection alert for user: {{user.name}}{{/context.alerts}}` will create the string `Detection alert for user: <user.name>` for every alert in the array.
+
...
You uploaded a value list of known ransomeware domains, and you want to be notified if any of those domains match in an indicator index pattern.
...
* *Indicator index patterns*: Value lists are stored in a hidden index called `.items-<Kibana space>`. Enter the appropriate {kib} space in this field.
* *Indicator index query*: Enter the field `list_id :`, then select the name of the value list you want to use as your indicator index.
* *Indicator mapping*
** *Field*: Enter the field used for comparing values in the Elastic Security event indices.
** *Indicator index field*: Select the field associated with the type of value list you created.
+
TIP: If you don't remember this information, go to *Detections > Upload value lists*. Locate the appropriate value list and note the field in the corresponding `Type` column.
