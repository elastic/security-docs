...
To understand which fields to parse, the json representation of rules can be viewed with the link:https://www.elastic.co/guide/en/siem/guide/7.10/rule-api-overview.html[Detection rules API], an example of link:http://mustache.github.io/[mustache] syntax to display the list of non disabled filters :
```
