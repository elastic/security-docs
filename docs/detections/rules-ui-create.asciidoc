...
NOTE: New fields are fields that don't exist in the query’s source index. For example...
...
- New fields, which are fields that don't exist in the query’s source index, won't be displayed in the Highlighted fields section of the alert details flyout. To ensure they're visible while you're investigating alerts, add the new fields to the rule's  <<rule-ui-advanced-params,custom highlighted fields>>. 
