...
Use {ref}/esql.html[ES|QL] to query source events and aggregate your event data. Query results are returned in a table that has rows and columns. Results of table rows are tracked in alerts.  

TIP: To learn about the basic structure of an ES|QL query, refer to {ref}/esql-getting-started.html[Getting started with ES|QL].

When creating an ES|QL rule, there are two types of queries that you can write: 

* **Aggregating query**: Uses {ref}/esql-functions-operators.html#esql-agg-functions[`STATS...BY`] functions to aggregate source event data. Generated alerts only contain the fields returned by the query.
+
In the following example of an aggregating query, the an alert is generated whenever `host.name` was encountered more than 20 times during a rule interval.
+
[source,esql]
----
FROM logs*
...
* **Non-aggregating query**: Doesn't use `STATS...BY` functions and doesn't aggregate source data. Generated alerts contain both source event data and fields returned by the ES|QL query. 
+
IMPORTANT: To turn on alert deduplication and ensure source event data is returned in alert documents, you must add the `[metadata _id, _index, _version]` operator after the `FROM` source command. Refer to <<alert-dedupe-non-agg>> for more information. 
+
In the following example of a non-aggregating query, the `logs*` index is searched for events that contain `event.id: 8a4f500d` field-value pair. Only ten results are returned. 
+
[source,esql]
-----
FROM logs* [metadata _id, _index, _version]
| WHERE event.id == "8a4f500d"
| LIMIT 10
-----
...
+
TIP: Click the help icon (image:images/esql-ref-button.png[Click the ES|QL help icon,20,20]) to open the in-product reference documentation for all ES|QL commands and functions.
+
. Click *Continue* to <<rule-ui-basic-params, configure basic rule settings>>.

[float]
[[alert-dedupe-non-agg]]
==== Deduplicate alerts and link source events 

When you write a non-aggregating query, you must configure the query to manually turn on alert deduplication and ensure that source events get linked to alerts. To do this, add `[metadata _id, _index, _version]` after the `FROM` source command, for example:
...
After specifying these metadata properties, make sure you don't {ref}/[`DROP`] or filter them out in any part of the query. Continue reading for examples of invalid non-aggregating queries.

**Example: Dropping the `_id` field**

In the following example, the `DROP` command explicitly omits the `_id` property, so it won't be returned in the results table.
...
**Example: Filterng out the `_id` field**

In the following example, the `KEEP` command omits the `_id` property, so it won't be returned in the results table.
