...
. After you select a connector, set its action frequency:

** *Summary of alerts*: Select this option if want to reduce the number of notifications you receive without affecting their timeliness. Alert notifications that summarize the new, ongoing, and recovered alerts will be sent at your preferred time intervals (per rule run or during a custom frequency).
+
NOTE: Alert summaries are only supported for some rule types.

** *For each alert*: Select this option to ensure notifications are sent every time an alert is generated. 
...
** *if alert matches query*: Enter a KQL query that defines field-value pairs or query conditions that must be met for notifications to send.
** *if alert is generated during timeframe*: Set timeframe conditions for alerts. Alerts must be generated within the timeframe that you define for notifications to send.
...
You can use http://mustache.github.io/[mustache syntax] to add the following placeholders to <<rule-notifications, rule action>> fields. Different variables are provided based on the rule's action frequency. 

If the rule’s action frequency is a summary of alerts, it passes the variables that summarize the new, ongoing, and recovered alerts. Refer to {kibana-ref}/rule-action-variables.html#alert-summary-action-variables[Action frequency: Summary of alerts] more information about summary of alert variables. 
