...
- When creating an aggregating query, avoid dropping fields that you want to search for, or filter on, from the Alerts table. For example, make sure you keep the `host.name`, `user.name`, and `process.name` fields.
...
- If you create a new field, you won't be able to filter or search for that field from the Alerts page (or any page where you can examine alerts) unless you create a runtime field for the new field name. 
