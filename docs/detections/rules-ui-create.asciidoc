...
.. *Indicator prefix override*: Specify the default threat indicator path that the indicator match rule will use when obtaining threat indicator data from the defined indicator indices. This data is used to enrich indicator match alerts with metadata about matched threat indicators. The default value for this field is `threat.indicator`. If your threat indicator data is at a different location, update this setting to ensure the correct field prefix is used so alert enrichment can still be performed.

+
[NOTE]
=========================
Be mindful of the following items when upgrading to {stack} version 8.0.0:

* If the default threat indicator path of an indicator match rule created in {stack} 7.x isn't defined before upgrading, it's automatically set to `threatintel.indicator` during the upgrade process. This allows the rule to continue using indicator data ingested by {filebeat} version 7.x.
* If an indicator match rule created in {stack} 7.x was configured to use threat indicator data ingested by {filebeat} version 7.x, and you've upgraded to {stack} version 8.0.0 and {agent} or {filebeat} version 8.0.0, update the default threat indicator path from `threatintel.indicator` to `threat.indicator`. This allows the rule to use indicator data ingested by {agent} or {filebeat} version 8.0.0.
* Separate rules must be created to query threat intelligence indices created by {filebeat} versions 7.x and 8.0.0 because different default threat indicator path values are required.
=========================
