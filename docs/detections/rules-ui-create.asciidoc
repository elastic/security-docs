...
:frontmatter-description: Create detection rules to monitor your environment for suspcious and malicious behavior.
...
The endpoint response action allows you to automatically run response actions on an endpoint when rule conditions are met. Refer to <<isolate-a-host>> to learn more.

IMPORTANT: Host isolation involves quarantining a host from the network to prevent further spread of threats and limit potential damage. Be aware that automatic host isolation from a detection rule execution could result in unintended consequences, such as disrupting legitimate user activities or blocking critical business processes.
