...
. Go to *Rules* -> *Detection rules (SIEM)* -> *Create new rule*. The *Create new rule* page displays.
. Select **ES|QL**. 
. Write an aggregating or non-aggregating type query:
+
TIP: Click the **ES|QL reference** button (image:images/esql-ref-button.png[Click the ES|QL reference button,20,20]) to the right of the query bar to open an ES|QL syntax reference guide.
+
** **Aggregating query**: Uses {ref}/esql-functions-operators.html#esql-agg-functions[`STATS...BY`] functions to aggregate alert data. An alert is generated for each segment of the query, but is not associated with a specific source event since aggregating queries process data from multiple alert documents. As a result, alerts from aggregating queries do not contain source event data.
+
Example aggregating query: 
+
[source,esql]
----
FROM logs*
...
** **Non-aggregating query**: Doesn't use `STATS...BY` functions and doesn't aggregate alert data. An alert is generated for each segment of the query, and each alert is associated with a source event. As a result, alerts from non-aggregating queries contain source event data.
+ 
IMPORTANT: To turn on alert deduplication and allow query results to be associated with source events, you must add the `[metadata _id, _index, _version]` operator after the query's data source. Do not drop or filter out the `id`, `_index`, `_version` metadata properties in any part the query. 
+
Example non-aggregating query:  
+
[source,esql]
-----
