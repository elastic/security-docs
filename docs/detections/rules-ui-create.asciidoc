...
- When writing an aggregating query, use the {ref}/esql-commands.html#esql-keep[`KEEP`] command to keep fields that you want to examine. For example, if you want to search and filter for the `host.name`, `user.name`, and `process.name` fields, keep them in your query. 
