...
. Select a way to preview the query results:

** *Quick query preview*: Select a timeframe of data to preview query results from the drop-down: *Last hour*, *Last day*, or *Last month*.
** *Advanced query preview*: Set the following fields:
*** *Timeframe*: Select a date time and time for the preview.
***  *Runs ever (Rule interval)*: Specify how often you want the rule to run. 
*** *Additional look-back time*: (Optional) Select a how far back in time the rule should look.
