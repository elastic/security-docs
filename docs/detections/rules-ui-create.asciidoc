...
- If your query contains new fields that aren't {ecs-ref}[ECS-compliant], the detection engine doesn't write them to alert documents and you can't search or filter for them from the Alerts table. As a workaround, create <<runtime-fields,runtime fields>>. 
- Newly-created, non-ECS-compliant fields cannot be added to a rule's <<rule-ui-advanced-params,custom highlighted fields>>. 
