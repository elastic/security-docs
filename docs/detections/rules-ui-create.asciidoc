...
** *Quick query preview*: Allows you to select from pre-defined timeframes -- *Last hour*, *Last day*, or *Last month* -- when previewing rule results. Note that threshold and event correlation rules have limited timeframe options. The rule interval and look-back time are also pre-defined for the preview and differ by rule type. These settings cannot be modified.
** *Advanced query preview*: Allows you to choose a custom timeframe for the rule preview, schedule how often the rule should run, and specify a look-back time.
+
TIP: It is recommended to set the `Additional look-back time` to at least 1 minute. This ensures there are no missing alerts when a rule does not run exactly at its scheduled time. In addition, avoid setting long timeframes with short rule intervals. This might cause the rule preview to timeout.
