...
This query generates an alert whenever the `host.name` field is encountered more than 20 times during the specified rule interval.
...
As you continue to develop the query, make sure you don't {ref}/esql-commands.html#esql-drop[`DROP`] or filter out `_id`, `_index, and `_version` metadata fields. Here is an example of an invalid query that uses the `DROP` command to omit the `_id` property from the results table:
