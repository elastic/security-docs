...
If you create a non-aggregating query, you also have to manually turn on alert deduplication and link source events to alerts. To do this, you must add the `[metadata _id, _index, _version]` operator after the `FROM` source command. For example:  
...
After specifying these metadata properties, make sure you don't {ref}/esql-commands.html#esql-drop[`DROP`] or filter them out of the query. Here is an example of a query that's invalid because it uses the `DROP` command to omit the `_id` property from the results table:
...
Here is another example of an invalid query where the `KEEP` command omits the `_id` property so it's not returned in the results table: 
