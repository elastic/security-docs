...
- The <<opt-fields-all,`max_signals`>> setting defines the maximum number of alerts that a rule can create during a single rule execution cycle. Similarly, the {ref}/esql-commands.html#esql-limit[`LIMIT`] command defines the number of rows an {esql} query returns, which determines the maximum number of alerts the rule generates per rule execution.  
+
In general, the rule respects the `max_signals` value if it's less than the `LIMIT` value. However, if the `max_signals` value is _greater_ than the `LIMIT` value, the rule uses the `LIMIT` value. 

- When writing an aggregating query, make sure to keep fields that you want to search and filter for once alerts are generated. For example, when writing your query, keep the `host.name`, `user.name`, and `process.name` fields to ensure you're able to filter for these fields from the Alerts table.
...
- If your query contains new fields that aren't compliant with ECS, you won't be able to search or filter for them unless you <<runtime-fields,create runtime fields>> for the new fields. 
- Custom highlighted fields are not supported for new fields that are not ECS-compliant. 
