...
.. Define which Elasticsearch indices or data view the rule searches for events.
.. Write an {ref}/eql-syntax.html[EQL query] that searches for matching events or a series of matching events.
+
TIP: When querying for a series of matching events, use the {ref}/eql-syntax.html#eql-missing-events[missing events] syntax to specify events that should _not_ happen.  
+
