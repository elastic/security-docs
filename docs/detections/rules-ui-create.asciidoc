...
+
NOTE: To avoid conflicts between the {ref}/esql-commands.html#esql-limit[`LIMIT` value] (which defines how many rows are returned in the results table) and the <<opt-fields-all,`max_signals`>> value (which determines the max number of alerts that can be generated per rule execution), make sure the `max_signals` value does not exceed the `LIMIT` value. If you don't define a `LIMIT` value, the rule automatically uses the `max_signals` value. 
+
. Click *Continue* to <<rule-ui-basic-params, configure basic rule settings>>.
