...
You can use http://mustache.github.io/[mustache syntax] to add variables to notification messages.  

If the rule’s action frequency is a summary of alerts, it passes {kibana-ref}/rule-action-variables.html#alert-summary-action-variables[alert summary variables], `{{context}}` variables (for example `{{context.alerts}}`) and the `{{state.signals_count}}` variable. If a rule's action frequency is per alert, it passes any of the following variables. 
