...
- When writing an aggregating query, follow the {ref}/esql-commands.html#esql-stats-by[`STATS...BY`] command with fields that you want to search and filter for when alerts are created. For example, placing the `host.name`, `user.name`, `process.name` fields after the `BY` operator of the `STATS...BY` command returns these fields in alert documents, and allows you to search and filter for them from the Alerts table. 
