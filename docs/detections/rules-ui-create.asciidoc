...
Use Response Actions to set up additional functionality that will run whenever a rule executes. You can choo
...
The Endpoint response action allows you to automatically run response actions on an endpoint when the rule's conditions are met. Refer to <<rules-add-endpoint-response-action>> to learn more.
