...
** *Quick query preview*: Allows you to select from three pre-defined timeframes -- *Last hour*, *Last day*, or *Last month* -- when previewing rule results. By default, rules are scheduled to run every 5 minutes with an additional look-back time of 1 minute. These settings cannot be modified.
...
+
NOTE: <Explanation of why the rule preview might not match the defined interval and or timeframe. Also include example of what preview might look like with these settings configured.>
+
TIP: It is recommended to set the `Additional look-back time` to at least 1 minute. This ensures there are no missing alerts when a rule does not run exactly at its scheduled time.
