...
.. *Indicator prefix override*: Specify the default threat indicator path (`threat_indicator_path`) that the indicator match rule will use when obtaining threat indicator data from the default threat intelligence index. This data is used to enrich indicator match alerts with metadata about threat indicator. The default value for this field is `threat.indicator`. If your threat indicator data is at a different location, update this setting to ensure the correct field prefix is used and so alert enrichment can still be performed.

+
NOTE: When upgrading to {stack} version 8.x, the default threat indicator path of an indicator match rule created in {stack} 7.x is automatically updated to `threatintel.indicator`. This allows the rule to continue using indicator data ingested by {filebeat} version 7.x. To make a rule use threat indicator data ingested from {agent} or {filebeat} version 8.x, enter the `threat.indicator` value into the *Indicator prefix override* field. Separate rules must be created to query threat intelligence indices created by {filebeat}  version 7.x and version 8.x because the rules require different values in their default threat indicator paths.
