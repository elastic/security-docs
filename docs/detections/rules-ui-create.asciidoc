...
After specifying these metadata properties, make sure you don't {ref}/esql-commands.html#esql-drop[`DROP`] or filter them out of the query. Here is an example of an invalid query that uses the `DROP` command to omit the `_id` property from the results table:
