...
+
** **Aggregating query**: Uses {ref}/esql-functions-operators.html#esql-agg-functions[`STATS...BY`] functions to aggregate alert data. An alert is generated for each row in the result table, and alert documents only contain fields returned by the query.  
+
