...
.. *Custom highlighted fields* (optional): Specify highlighted fields for personalized alert investigation flows. Fields with data are added to the [investigation-section,Highlighted fields] section within the alert details flyout. You can also find custom highlighted fields in the About section of the rule details page.
+
NOTE: There's no maximum limit to the number of custom highlighted fields you can add.  
//Will change brackets to double angled brackets once merge PR #3816
