...
.. *Indicator prefix override* (optional): Specify the default threat indicator path (`threat_indicator_path`) that the rule will use when searching indicator documents for threat intelligence data. This data will be used to enrich indicator match alerts. The default value for this field is `threat.indicator`. If your indicator data contains indicator fields at a different location, update this setting accordingly to ensure alert enrichment can still be performed.

+
NOTE: The default threat indicator path of indicator match rules created in {stack} 7.x is automatically updated to `threatintel.indicator` when upgrading to {stack} 8.x. This allows your migrated rules to continue using indicator data ingested by {filebeat} 7.x. To make a migrated rule use indicator data ingested from {fleet} or {filebeat} version 8.x, enter the `threat.indicator` value into the *Indicator prefix override* field. 
