...
+
** **Aggregating query**: Uses {ref}/esql-functions-operators.html#esql-agg-functions[`STATS...BY`] functions to aggregate data. Results from each query row are not saved to an alert document within Elasticsearch. 
+
...
** **Non-aggregating query**: Does not use `STATS...BY` functions and doesn't aggregate data. To allow alert deduplication and save query row results, you must add the `[metadata _id, _index, _version]` operator after the query's data source. 
+ 
IMPORTANT: Do not drop or filter out the `id`, `_index`, `_version` metadata properties in any part the query. This turns off alert deduplication.
+
