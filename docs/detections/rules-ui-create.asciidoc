...
- If your query contains new fields that aren't {ecs-ref}[ECS-compliant], the detection engine won't write them to alert documents, and you can't search or filter for them from the Alerts table. As a workaround, create <<runtime-fields,runtime fields>>. 
