...
*Triage and analysis**Investigating Potential Successful SSH Brute Force Attack*The rule identifies consecutive SSH login failures followed by a successful login from the same source IP address to the same target host indicating a successful attempt of brute force password guessing.*Possible investigation steps*- Investigate the login failure user name(s).
...
- Infrastructure or availability issue.*Response and remediation*- Initiate the incident response process based on the outcome of the triage.
