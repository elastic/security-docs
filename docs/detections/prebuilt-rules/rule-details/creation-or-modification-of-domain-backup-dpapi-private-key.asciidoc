...
*Version*: 5 (<<creation-or-modification-of-domain-backup-dpapi-private-key-history, version history>>)
...
[source,markdown]
----------------------------------
## Triage and analysis Domain DPAPI Backup keys are stored on domain
controllers and can be dumped remotely with tools such as Mimikatz.
The resulting .pvk private key can be used to decrypt ANY domain user
masterkeys, which then can be used to decrypt any secrets protected by
those keys.
----------------------------------
