...
*Version*: 4 (<<execution-via-local-sxs-shared-module-history, version history>>)
...
[source,markdown]
----------------------------------
## Triage and analysis The SxS DotLocal folder is a legitimate
feature that can be abused to hijack standard modules loading order by
forcing an executable on the same application.exe.local folder to load
a malicious DLL module from the same directory.
----------------------------------
