...
*Triage and analysis**Investigating Port Forwarding Rule Addition*Network port forwarding is a mechanism to redirect incoming TCP connections (IPv4 or IPv6) from the local TCP port to any other port number, or even to a port on a remote computer.
