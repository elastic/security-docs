...
Adversaries may attempt to access credential material stored in LSASS process memory. After a user logs on, the system generates and stores a variety of credential materials in LSASS process memory. This is meant to facilitate single sign-on (SSO) ensuring a user isn’t prompted each time resource access is requested. These credential materials can be harvested by an adversary using administrative user or SYSTEM privileges to conduct lateral movement using https://attack.mitre.org/techniques/T1550/.
...
Object Access policies https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/event-4656 (Handle to an Object was Requested)
...
Also, this event generates only if the object’s https://docs.microsoft.com/en-us/windows/win32/secauthz/access-control-lists has the required access control entry (ACE) to handle the use of specific access rights.
