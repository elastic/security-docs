...
*Triage and analysis**Investigating Kerberos Traffic from Unusual Process*Kerberos is the default authentication protocol in Active Directory, designed to provide strong authentication for client/server applications by using secret-key cryptography.
...
- Exceptions can be added for noisy/frequent connections.*Response and remediation*- Initiate the incident response process based on the outcome of the triage.
