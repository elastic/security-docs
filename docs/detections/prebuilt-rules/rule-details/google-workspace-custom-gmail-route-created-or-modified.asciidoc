...
*Triage and analysis**Investigating Google Workspace Custom Gmail Route Created or Modified*Gmail is a popular cloud-based email service developed and managed by Google. Gmail is one of many services available for users with Google Workspace accounts.
...
- Identified URLs or attachments can be submitted to VirusTotal for reputational services.*False positive analysis*- This rule searches for domain-wide custom email routes created in the admin console of Google Workspace. Administrators might create custom email routes to fulfill organizational requirements.*Response and remediation*- Initiate the incident response process based on the outcome of the triage.
