...
*Machine learning job*: linux_anomalous_process_all_hosts_ecs, v2_linux_anomalous_process_all_hosts_ecs
...
*Version*: 7 (<<anomalous-process-for-a-linux-population-history, version history>>)
...
process that is rare and unusual for all of the monitored Linux hosts
for which Auditbeat data is available. Here are some possible avenues
of investigation: - Consider the user as identified by the username
