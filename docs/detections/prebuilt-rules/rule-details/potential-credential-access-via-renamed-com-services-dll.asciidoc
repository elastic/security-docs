...
*Triage and analysis**Investigating Potential Credential Access via Renamed COM+ Services DLL*COMSVCS.DLL is a Windows library that exports the MiniDump function, which can be used to dump a process memory. Adversaries may attempt to dump LSASS memory using a renamed COMSVCS.DLL to bypass command-line based detection and gain unauthorized access to credentials.
...
- LSASS Process Access via Windows API - ff4599cb-409f-4910-a239-52e4e6f532ff*Response and Remediation*- Initiate the incident response process based on the outcome of the triage.
