...
* To reduce cluster performance issues, avoid scheduling your rule to run any faster than a one-hour interval. For example, avoid scheduling an indicator match rule to check for indicators every five minutes.
