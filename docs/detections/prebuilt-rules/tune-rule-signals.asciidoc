...
* Add an exception to the rules that <<filter-rule-process, exclude specific processes>>. For example, `process.name is <process-name>`.
...
. In the original prebuilt rule, exclude the relevant user or process
names (`user.name is <user-name>` or `process.name is "process-name"`).
. In the duplicated rule:
* Lower the `Risk score` (*Edit rule settings* -> *About* tab).
* Add an exception so the rule only matches the user or process names excluded
in the first duplication.
