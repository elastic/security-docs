...
* Avoid scheduling indicator match rules to run any faster than a one-hour interval to avoid cluster performance issues. For example, avoid scheduling an indicator match rule to check for indicators every five minutes.
