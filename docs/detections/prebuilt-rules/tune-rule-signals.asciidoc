...
* Avoid cluster performance issues by scheduling your rule to run in one-hour intervals, or higher. For example, avoid scheduling an indicator match rule to check for indicators every five minutes; do schedule it to run every hour.
