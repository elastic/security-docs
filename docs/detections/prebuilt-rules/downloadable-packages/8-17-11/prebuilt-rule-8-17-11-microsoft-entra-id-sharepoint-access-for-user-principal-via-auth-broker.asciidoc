[[prebuilt-rule-8-17-11-microsoft-entra-id-sharepoint-access-for-user-principal-via-auth-broker]]
...
This is a https://www.elastic.co/guide/en/security/current/new-terms-rules.html[New Terms rule] that detects the first occurrence of a user principal name accessing SharePoint Online via the Microsoft Authentication Broker application in the last 14 days.
