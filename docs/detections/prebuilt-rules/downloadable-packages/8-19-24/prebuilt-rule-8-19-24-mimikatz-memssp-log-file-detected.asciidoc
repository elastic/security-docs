[[prebuilt-rule-8-19-24-mimikatz-memssp-log-file-detected]]
...
- Which process activity likely enabled MemSSP before LSASS wrote the log?
  ...
  - Hint: search backward from alert `@timestamp` for process starts containing "memssp", "mimikatz", "mimilib", or script/LOLBIN launchers on the same `host.id`.
...
- Does the host show default in-memory MemSSP only, or a persistent SSP setup?
  ...
  - Focus: companion DLL file events (`file.path`, `file.extension`) and, when registry telemetry is available, LSA Security Packages changes in `registry.path` and `registry.data.strings`.
