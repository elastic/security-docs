[[prebuilt-rule-8-19-24-inbound-connection-to-an-unsecure-elasticsearch-node]]
...
(data_stream.dataset: network_traffic.http OR (event.category: network_traffic AND network.protocol: http)) AND
     http.response.status_code:200 AND destination.port:9200 AND network.direction:inbound AND NOT http.response.headers.content-type:"image/x-icon" AND NOT
