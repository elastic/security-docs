[[prebuilt-rule-8-19-24-file-creation-in-world-writable-directory-by-unusual-process]]
...
not (
  file.path:(
    ...
    /tmp/CVU_19_* or /tmp/ut-backup-files.* or /tmp/tmp.*/terraform/*
  ) or
  file.extension:("json" or "txt") or
  process.executable:(
    /tmp/par-* or /tmp/snap.rootfs_* or /tmp/CVU_*/exectask or /tmp/.mount_*/usr/bin/QGroundControl or
    ...
    "./usr/bin/podman" or "/tmp/newroot/usr/lib/systemd/systemd"
  )
)
