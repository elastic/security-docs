[[prebuilt-rule-8-6-4-first-time-seen-aws-secret-value-accessed-in-secrets-manager]]
...
This rule looks for the retrieval of credentials using `GetSecretValue` action in Secrets Manager programmatically. This is a New Terms rule indicating this is the first time a specific user identity has successfuly retrieved a secret value from Secrets Manager.
