[[prebuilt-rule-8-12-5-network-activity-detected-via-cat]]
...
*Triage and analysis**Investigating Network Activity Detected via cat*Attackers may leverage the `cat` utility in conjunction with a listener to read all bytes of a file, and output the content to a `/dev/tcp` or `/dev/udp` channel to transfer/exfiltrate file contents to a remote system. 
