[[prebuilt-rule-8-19-32-smb-windows-file-sharing-activity-from-the-internet]]
...
- **Fortinet FortiGate** integration (firewall traffic logs)
...
[source, js]
----------------------------------
(data_stream.dataset:(fortinet_fortigate.log or network_traffic.flow or pfsense.log or zeek.smb_cmd or zeek.smb_files or zeek.smb_mapping) or event.category:(network or network_traffic))
