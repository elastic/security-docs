[[prebuilt-rule-8-19-33-file-with-suspicious-double-extension-created-by-web-server]]
...
- A deployment, backup, or testing process run through a web application may generate files such as `page.jsp.zip` or `handler.cgi.sh`; verify the change record, process lineage, file contents, ownership, and whether the destination is expected and non-executable.
