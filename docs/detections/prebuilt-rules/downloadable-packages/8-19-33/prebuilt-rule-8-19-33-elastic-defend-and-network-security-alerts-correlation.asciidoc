[[prebuilt-rule-8-19-33-elastic-defend-and-network-security-alerts-correlation]]
...
// extract source.ip from PANW, Fortigate or Suricata events and host.ip from Elastic Defend alert
|eval fw_alert_source_ip = CASE(data_stream.dataset in ("panw.panos", "fortinet_fortigate.log", "suricata.eve"), source.ip, null),
...
// group by host_source_ip shared between FG/PANW/Suricata and Elastic Defend
