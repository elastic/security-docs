...
+
TIP: Use the **Diff view** drop-down menu to compare changes (field values) in different versions of a rule field. For example, compare the changes that you made to the current version of the field with changes that will be applied from the incoming Elastic update.

** Understand the state of the update: You can view the status of the entire rule update and for each field that's being changed. Refer to <<rule-update-statuses>> to learn more.

** Address update conflicts: Conflicts might occur when updating prebuilt rules and will often require resolution. Refer to <<resolve-reduce-rule-conflicts>> to learn more. 
+
IMPORTANT: Changes that Elastic makes to a rule's type cannot be modified during the update process. 

** Edit final updates to rule fields: You can modify a field's final update by making changes in the *Final update* section.
...
. From the *Rule Updates* tab, do one of the following to update prebuilt rules without conflicts:
...
==== Understand update statuses

This table describes statuses that might appear for rule fields that are being updated.  
...
| *No update*
a| Displays when the current field value differs from the original field value, but the field is not being updated by Elastic. 
...
| *Review required*
a| Displays when Elastic auto-resolved a conflict between the current field value and the value that's provided in the incoming Elastic update. 

To resolve the conflict, review the changes and accept them if they are correct. If they are not, make the appropriate changes in the *Final update* section, then save and accept them.

| *Action required*
a| Displays when Elastic could not auto-resolve the conflict between the current field value and the value that's provided in the incoming Elastic update. 

To resolve the conflict, do one of the following, then save and accept your changes:
...
You may be able to reduce the frequency and complexity of update conflicts by regularly updating prebuilt rules.

Elastic always attempts to resolve conflicts when possible and will suggest solutions for your review. In these cases, you can accept or edit the suggested fixes, then update the rule. If Elastic can't auto-resolve the conflicts, you must manually fix them before updating the rule. 
