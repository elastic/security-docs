...
* Install multiple rules: Select the rules and at the top of the page, either click *Install _x_ selected rule(s)* to install the rules without enabling them, or click image:images/boxesVertical.svg[Vertical boxes button] → **Install and enable** to start running the rules once they're installed.
--
+
