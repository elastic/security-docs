...
NOTE: Rules with conflicts require additional attention before you update them. Refer to <<resolve-reduce-rule-conflicts>> to learn more.
...
+
TIP: Use the **Diff view** drop-down menu to compare different versions of a rule field. For example, compare the changes that you made to the current version of the field with changes that will be applied from the incoming Elastic update.
...
** Address update conflicts: Conflicts might occur when updating customized prebuilt rules and often <<resolve-reduce-rule-conflicts, require additional attention>>. 
+
IMPORTANT: Elastic updates to a rule's type cannot be changed. Before updating the rule, duplicate it if you need to record other changes that you made to the rule. 

** Edit the final update: Change the value that will be applied to the field when you update the rule. To change the value, go to the *Final update* section, then edit and save your changes.
+
[role="screenshot"]
...
| *No update*
a| Displays when the field is not being updated by Elastic, but the current field value differs from the original one. This typically happens when the default field value was changed after the prebuilt rule was installed.

No other actions are required. The field is ready to be updated. 

TIP: You can still change the final field value, if needed. To change the final field value, edit the value in the *Final update* section, then save your changes.
...
You must address this conflict before you can update the rule. To address the conflict, review the changes and accept them if they are correct. If they aren't, make the appropriate changes in the *Final update* section and then save and accept them.
...
* Edit the final field value by combining your changes with the Elastic update or choosing a more appropriate field value.
