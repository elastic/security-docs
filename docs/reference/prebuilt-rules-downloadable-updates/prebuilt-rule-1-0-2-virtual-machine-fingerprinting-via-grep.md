---
mapped_pages:
  - https://www.elastic.co/guide/en/security/current/prebuilt-rule-1-0-2-virtual-machine-fingerprinting-via-grep.html
---

# Virtual Machine Fingerprinting via Grep [prebuilt-rule-1-0-2-virtual-machine-fingerprinting-via-grep]
...
## Investigation guide [_investigation_guide_1381]

## Config

If enabling an EQL rule on a non-elastic-agent index (such as beats) for versions <8.2, events will not define `event.ingested` and default fallback for EQL rules was not added until 8.2, so you will need to add a custom pipeline to populate `event.ingested` to @timestamp for this rule to work.

## Rule query [_rule_query_1614]
