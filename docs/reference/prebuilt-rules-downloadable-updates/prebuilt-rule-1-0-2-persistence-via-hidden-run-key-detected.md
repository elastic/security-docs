...
applies_to:
  ...
  serverless:
    security: all
mapped_pages:
  - https://www.elastic.co/guide/en/security/current/prebuilt-rule-1-0-2-persistence-via-hidden-run-key-detected.html
---

# Persistence via Hidden Run Key Detected [prebuilt-rule-1-0-2-persistence-via-hidden-run-key-detected]
...
## Investigation guide [_investigation_guide_1651]

## Config

If enabling an EQL rule on a non-elastic-agent index (such as beats) for versions <8.2, events will not define `event.ingested` and default fallback for EQL rules was not added until 8.2, so you will need to add a custom pipeline to populate `event.ingested` to @timestamp for this rule to work.

## Rule query [_rule_query_1913]
