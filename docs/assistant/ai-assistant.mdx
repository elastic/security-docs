...
1. Create a [Generative AI connector](((kibana-ref))/gen-ai-action-type.html) using the AI provider's API key and URL to authenticate communication between ((elastic-sec)) and the provider. You can do this in ((kib)) from **Stack Management** -> **Connectors**, or from within AI Assistant.
