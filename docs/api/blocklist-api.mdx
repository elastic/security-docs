---
id: enSecurityBlocklistApi
slug: /en/security/blocklist-api
title: Blocklist
...
Create, retrieve, update, and delete endpoint \<\<blocklist>> entries with the <DocLink id="enSecurityExceptionsApiOverview">Exceptions API</DocLink>. Endpoint {endpoint_artifact_name} are managed using a static container id (`list_id`) of `pass:a[{endpoint_artifact_list_id}]`, which must be created prior to adding the {endpoint_artifact_name}. To use these APIs, you must have privileges to manage endpoints. Refer to <DocLink id="enSecurityAdminPageOv">Endpoints</DocLink> for more information.
