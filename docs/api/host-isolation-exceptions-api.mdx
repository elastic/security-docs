---
id: enSecurityHostIsolationExceptionsApi
slug: /en/security/host-isolation-exceptions-api
...
Create, retrieve, update, and delete endpoint <DocLink id="enSecurityHostIsolationExceptions">host isolation exceptions</DocLink> via API. {endpoint_artifact_name_sentence_start} are managed via the <DocLink id="enSecurityExceptionsApiOverview">Exceptions API</DocLink> using a static container id (`list_id`) of `pass:a[{endpoint_artifact_list_id}]`, which must be created prior to adding the {endpoint_artifact_name}. Access to these APIs requires that a user has authorization to manage endpoints.
