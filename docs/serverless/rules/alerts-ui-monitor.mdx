...
* The **Run type** drop-down filters the table by rule execution type: 
    * **Scheduled**: Rule executions that occured automatically and according to the rule's schedule. 
    * **Manual**: Rule executions that were triggered manually.
...
<div id="manual-rule-run-details"></div>

### Manual rule run details 

As a manual rule run executes, its details are displayed in the Manual runs table. To access the table, go to **Rules** -> **Detection rules (SIEM)**, click the rule's name to open its details, then scroll down and select the **Execution results** tab. The Manual runs table is under the Execution log table.

In the table, you can do the following:

* Find important details about manual run including:
    * The **Status** of the manual rule run.
    * When the manual rule run started and the time range that you selected for the manual rule execution.
    * The number of rule executions are failed, pending, running, and completed, in addition to the total number of rule executions that will occur during the manual rule run.

* Stop the manual rule run from executing. 

* Refresh the table's results.
