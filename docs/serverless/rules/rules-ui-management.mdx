...
Manually run rules to test them instantly or to fill in past gaps in coverage.
...
* Select a rule to open its details page, select the **All actions** menu, then select **Manual run**.
* Select all the rules you want to manually run, select the **Bulk actions** menu, then select **Manual run**.
1. Specify a time range for the rule run. By default, the time range is set to the current day, and three hours in the past.
1. Click **Run** to manually execute the rule.

The manual run's details are recorded in the <DocLink slug="/serverless/security/alerts-ui-monitor" section="execution-results">Manual runs</DocLink> table on the the **Execution results** tab.  
