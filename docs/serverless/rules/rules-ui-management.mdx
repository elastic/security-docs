...
Incorrect scheduling can lead to inconsistent rule results. Before manually running rules, make sure you properly understand and plan for rule dependencies.
</DocCallOut>
...
The manual run's details are recorded in the <DocLink slug="/serverless/security/alerts-ui-monitor" section="manual-runs-table">Manual runs</DocLink> table on the the **Execution results** tab. If you change the settings or manual run schedule of a rule that's being manually run, the changes aren't immediately shown in the Manual runs table. They're only shown after the current manual run job completes. 

<DocCallOut title="Note">
Manual runs are executed with low priority and limited concurrency, which means they might take longer to complete, especially if multiple rules are being manually run.
</DocCallOut>
