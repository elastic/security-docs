...
1. Click **Advanced settings** and fill in the following fields where applicable:
    ...
    1. **Custom highlighted fields** (optional): Specify highlighted fields for personalized alert investigation flows. You can choose any fields that are available in the <DocLink slug="/serverless/security/advanced-settings" section="update-sec-indices">default ((elastic-sec)) indices</DocLink>. There is no limit to the number of fields that you can add. 
