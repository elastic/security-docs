...
* **Asset Criticality**: If the `securitySolution:enableAssetCriticality` <<security-advanced-settings-enable-asset-criticality-workflows,advanced setting>> is on, this section displays the user's current <<security-asset-criticality,asset criticality level>>.
