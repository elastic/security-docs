...
To view the map, you need the appropriate <DocLink slug="/serverless/general/assign-user-roles">prebuilt user role</DocLink>.
</DocIf>
<DocIf condition={"((serverlessCustomRoles))" === "true"}> */}
To view the map, you need the appropriate <DocLink slug="/serverless/general/assign-user-roles">prebuilt user role</DocLink> or a <DocLink slug="/serverless/custom-roles">custom role</DocLink> with at least `Read` privileges for the `Maps` feature.
{/* </DocIf> */}
...
1. In your Beats configuration files, add the pipeline to the `output.elasticsearch` tag:
