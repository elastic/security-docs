...
<div id="exclude-cold-frozen-data-rule-executions"></div>

## Exclude cold and frozen data tiers from rule executions

Set the `excludedDataTiersForRuleExecution` field to ensure that events from cold and frozen [data tiers](((ref))/data-tiers.html) aren't searched during rule executions. Excluding data from cold and frozen tiers might help to improve rule performance or reduce execution time. 

If you specify multiple data tiers, values must be separated by commas, for example: `data_frozen`, `data_cold`. This setting is turned off by default.