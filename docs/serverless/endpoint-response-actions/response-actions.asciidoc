...
<<isolate-host,Isolate the host>>, blocking communication with other hosts on the network.
...
[NOTE]
====
Running this command on third-party-protected hosts might return the process list in a different format. Refer to <<third-party-actions>> for more information.
====
...
[discrete]
[[response-actions-suspend-process]]
=== `suspend-process`
...
[NOTE]
====
Files retrieved from third-party-protected hosts require a different password. Refer to <<third-party-actions>> for your system's password.
====
...
[TIP]
====
You can use the <<query-operating-systems,Osquery manager integration>> to query a host's operating system and gain insight into its files and directories, then use `get-file` to retrieve specific files.
====
...
Click **Response actions history** to display a log of the response actions performed on the endpoint, such as isolating a host or terminating a process. You can filter the information displayed in this view. Refer to <<response-actions-history,Response actions history>> for more details.
