...
[NOTE]
====
* https://www.elastic.co/docs/current/serverless/elasticsearch/manage-project[Project features add-on]: Endpoint Protection Complete
* https://www.elastic.co/docs/current/serverless/general/assign-user-roles[User roles]: **SOC manager** or **Endpoint operations analyst**
...
.. Click **Save**.
. **Create and enable detection rules to generate {elastic-sec} alerts.** (Optional) Create <<rules-create,detection rules>> to generate {elastic-sec} alerts based on SentinelOne events and data.
+
