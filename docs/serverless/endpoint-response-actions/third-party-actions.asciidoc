...
[NOTE]
====
* Third-party response actions require the Endpoint Protection Complete https://www.elastic.co/docs/current/serverless/elasticsearch/manage-project[project feature].
...
[discrete]
[[third-party-actions-supported-systems-and-response-actions]]
== Supported systems and response actions

The following third-party response actions are supported for CrowdStrike and SentinelOne. <<response-actions-config,Prior configuration is required>> to connect each system with {elastic-sec}.
