...
NOTE: While local models work well for <<security-security-assistant, AI Assistant>>, we recommend you use one of <<llm-performance-matrix, these models>> for interacting with <<attack-discovery, Attack discovery>>. As local models become more performant over time, this is likely to change. 
