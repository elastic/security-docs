...
* Click the **Flyout settings** icon (image:images/view-alert-details/-detections-flyout-settings.png[Flyout settings icon,15,15]) to access options for displaying the alert details flyout. The **Overlay** option (which displays the flyout over the Alerts table) is selected by default. Select **Push** to display the flyout to the side of the table instead. In either display view, you can resize the flyout panels to your liking. Click **Reset size** to revert the flyout to its original dimensions. 
