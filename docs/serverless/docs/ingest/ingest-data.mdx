---
id: serverlessSecurityIngestData
...
* The [((agent))](((fleet-guide))/fleet-overview.html) with the **((elastic-defend))** integration, which protects
    your hosts and sends logs, metrics, and endpoint security data to ((elastic-sec)). See <DocLink id="serverlessSecurityInstallDefend" />.
...
* Third-party collectors configured to ship ECS-compliant data. <DocLink id="serverlessSecuritySiemFieldReference" /> provides a list of ECS fields used in ((elastic-sec)).
...
you must add its index to the ((elastic-sec)) indices (update the **`securitySolution:defaultIndex`** <DocLink id="serverlessSecurityAdvancedSettings" section="update-sec-indices">advanced setting</DocLink>).
