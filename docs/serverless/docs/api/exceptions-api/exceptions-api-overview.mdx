---
id: serverlessSecurityExceptionsApiOverview
# slug: /serverless/security/exceptions-api-overview
...
For detection rules, you can also use <DocLink id="serverlessSecurityListsApiOverview">lists</DocLink> to define
...
<DocLink id="serverlessSecurityListsApiCreateContainer">list container</DocLink> with a rule. To use list
exceptions, create an exception item that references the relevant list
container (see this <DocLink id="serverlessSecurityExceptionsApiCreateExceptionItem" section="list-item-example">example request</DocLink>).
</DocCallOut>
...
`.items` indices for the ((kib)) space (see <DocLink id="serverlessSecurityListsIndexApiOverview">Lists index endpoint</DocLink>).
</DocCallOut>
