...
To run and create {ml} jobs and rules, you need the appropriate https://www.elastic.co/docs/current/serverless/general/assign-user-roles[user role].

Additionally, for https://www.elastic.co/docs/current/serverless/custom-roles[custom roles], to configure <<security-alert-suppression,alert suppression>> for {ml} rules, your role needs the following index privilege:
