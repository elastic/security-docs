...
For <DocLink id="serverlessSecurityCspm" text="CSPM"/> billing is per billable, monitored `resource.id`. The following types of assets are considered billable:
...
For <DocLink id="serverlessSecurityKspm" text="KSPM"/>, billing is per monitored Kubernetes nodes (per `agent.id`).
...
For <DocLink id="serverlessSecurityD4cOverview" text="D4C"/>, billing is per agent (per `agent.id`).
