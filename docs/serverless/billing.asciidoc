...
For <<cspm,CSPM>>, billing is based on how many billable resources (`resource.id`s) you monitor. The following types of assets are considered billable:
...
For <<kspm,KSPM>>, billing is based on how many Kubernetes nodes (`agent.id`s) you monitor.

For <<vuln-management-overview,CNVM>>, billing is based on how many cloud assets (`cloud.instance.id`s) you monitor.

For <<d4c-overview,D4C>>, billing is based on how many agents (`agent.id`s) you use.
