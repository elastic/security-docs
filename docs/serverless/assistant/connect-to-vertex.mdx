...
This page provides step-by-step instructions for setting up a Google Vertex AI connector for the first time. This connector type enables you to leverage Vertex AI's large language models (LLMs) within ((kib)). You'll first need to enable Vertex AI, then generate an API key, and finally configure the connector in ((kib)).
...
4. The **URL** field can be left as default.
5. Enter your **GCP Region** and **GCP Project ID**.
6. Under **Default model**, specify which [model](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/models) you want to use.
