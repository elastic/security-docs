...
Replace `<secret token>` with your actual token, and keep it safe since you'll need it to set up the ((elastic-sec)) connector. Replace `<yourdomainname.com>` with your actual domain name. If you decide to change the port number in LM studio to something other than 1234, update the `proxy_pass` value at the bottom of the configuration.
</DocCallOut>
...
1. Filter your search for "Compatibility Guess" to optimize results for your hardware. Results will be color coded:
    1. Green means "Full GPU Possible", which yields the best results.
    1. Blue means "Partial GPU offload possible", which may work.
