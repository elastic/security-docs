...
contextual information, called **Event Renderers**. contextual information, called *Event renderers*. All event renders are turned off by default. To turn them on, use the **Event renderers** toggle at the top of the results pane. To only turn off specific event renders, click the gear (<DocIcon type="gear" title="The customize event renderer button" />) button next to the toggle, and select the ones you want enabled. Close the **Customize event renderers** pane when you're done. Your changes are automatically applied to Timeline.
