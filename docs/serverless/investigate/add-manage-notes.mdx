...
To find Timelines with notes, use the <DocLink slug="/serverless/security/add-manage-notes" section="manage-notes">**Notes** page</DocLink>. Alternatively, go to the **Timelines** page, open the appropriate Timeline, and click the **Notes** tab. The tab display all notes that are attached to the Timeline.
...
* Examine the alert or event that a note is attached to (click the <DocIcon type="expand" title="Preview alert or event details action" /> icon to preview the alert or event)
* Open the Timeline that the note is attached to (click the <DocIcon type="timelineWithArrow" title="Preview alert or event details action" /> icon to open the Timeline)
