...
1. In the **Socket Filter** section, fill in these fields:
    ...
    1. **Socket Filter Designated Requirement**: Enter 
...
1. In the **Network Filter** section, fill in these fields:
    ...
    1. **Network Filter Designated Requirement**: Enter
`identifier "co.elastic.systemextension" and anchor apple generic and certificate 1[field.1.2.840.113635.100.6.2.6] /* exists */ and certificate leaf[field.1.2.840.113635.100.6.1.13] /* exists */ and certificate leaf[subject.OU] = "2BT3HPN62Z"`
1. Save the configuration.
...
1. In the **Settings** section:
    1. Include **Critical Alerts**, and select **Enable**.
    1. Include **Notifications**, and select **Enable**.
    1. Include **Banner alert type**, and set it to **Persistent**.
    1. Include **Notifications on Lock Screen**, and select **Display**.
    1. Include **Notifications in Notification Center**, and select **Display**.
    1. Include **Badge app icon**, and select **Display**.
    1. Include **Play sound for notifications**, and select **Enable**.
1. Save the configuration.
...
1. Add a second entry with the following details:
    1. Under **Identifier**,  enter `/Library/Elastic/Endpoint/elastic-endpoint`.
    1. From the **Identifier Type** dropdown, select **Path**.
    1. Under **Code Requirement**, enter the following:
...
If you need help troubleshooting a deployment failure, refer to <DocLink slug="/serverless/security/troubleshoot-endpoints" section="defend-deployment">((elastic-defend)) deployment issues</DocLink>.
</DocCallOut>