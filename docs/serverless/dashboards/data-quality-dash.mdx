...
After an index is checked, `Pass` or `Fail` appears in its **Result** column. `Fail` indicates mapping problems in an index. To view index details, including which fields weren't successfully mapped, click the **Index details** button under **Actions**.
...
The index details flyout provides more information about the status of fields in that index. Each of its tabs describe fields grouped by mapping status.
...
1. View details for an index that has at least one incompatible field by clicking the **Index details** button under **Actions**.
