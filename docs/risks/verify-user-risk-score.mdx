...
description: Verify that user risk score data installed successfully.
...
After you enable user risk score, the following message may appear: 
...
Run the following commands in Console to query the `ml_user_risk_score_<space-id>` index: 
...
If there's no response, verify that relevant <DocLink id="serverlessSecurityRulesUiManagement">rules</DocLink> are running and that alert data is being generated. If there is a response, click **Restart** and allow an hour for the user risk data to appear.
