---
id: serverlessSecurityVerifyHostRiskScore
slug: /serverless/security/verify-host-risk-score
title: Verify host risk score data (Optional)
description: Verify that host risk score data installed successfully.
tags: [ 'serverless', 'security', 'how-to', 'manage' ]
status: in review
---

<div id="verify-host-risk-score"></div>

{/* To be updated: references to <space-id> throughout the page  */}

After you enable host risk score, the following message may appear:  

![Restart host risk score](../images/verify-host-risk-score/-experimental-features-restart-hrs.png)
...
Run the following commands in Console to query the `ml_host_risk_score_<space-id>` index: 
...
If there's no response, verify that relevant <DocLink id="serverlessSecurityRulesUiManagement">rules</DocLink> are running and that alert data is being generated. If there is a response, click **Restart** and allow an hour for the host risk data to appear.
