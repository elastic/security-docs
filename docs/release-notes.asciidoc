...
Previously when deleting value lists,  references to that value lists would be left behind.
...
* Look-back time logic now displays whatever unit the user selects ({pull}81383[#81383]).
* Fixes a bug where mapping browser fields were automatically reduced ({pull}81675[#81675]).
* Both status data for enabled and disabled rules are now fetchable ({pull}81783[#81783]).
* On the rules page, autorefresh can now be toggled in **Advanced Settings** ({pull}82062[#82062]).
* Severity and risk score overrides are now more flexible ({pull}83723[#83723]).
