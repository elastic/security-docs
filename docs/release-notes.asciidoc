...
[[features-7.14.0]]
==== Features
* Host Isolation is a way for Security users to isolate hosts from their networks while investigating a potential attack.  It stops a possibly infected host from infecting other hosts on the network. Adds advanced policy option for Linux Malware quarantine ({pull}104984[#104984]).
...
* Adds an application cache called `metrics entities` and integrates it within the {es-sec} solution behind a feature flag ({pull}96446[#96446]).
...
* Adds the Swimlane connector for Cases ({pull}100086[#100086]).
...
* Fixes the IP on threshold preview button when selecting an IP data type such as `source.ip` ({pull}105126[#105126]).
...
* Fixes the 404 error on deleted rule alert view ({pull}103491[#103491]).
