...
* Filters alerts table by presence of threat (elastic/security-team ({pull}96096[#96096]).
* Populates threat.indicator.event with _source.event ({pull}95697[#95697]).
* Adds Threat Summary and Threat Details tabs to Alert Side Panel ({pull}95604[#95604]).
* Adds live query history table ({pull}94536[#94536]).
* Update Cloud plugin to handle new config in kibana.yml ({pull}95569[#95569]).

[discrete]
[[bug-fixes-7.13.0]]
==== Bug fixes and enhancements
* Fixes Live query form errors handling ({pull}99015[#99015]).
* Fetch detection adoption metrics ({pull}97789[#97789]).
* Updates Beats fields metadata ({pull}97719[#97719]).
* Fixes UI capabilities validation ({pull}97663[#97663]).
* Osquery agent picker tests/fixes ({pull}97580[#97580]).
* Update detection alert mappings to ECS 1.9 ({pull}97573[#97573]).
* Agent picker follow up ({pull}97357[#97357]).
* ML Rules accept multiple ML Job IDs ({pull}97073[#97073]).
* Adds Security Network ML Module to list of available jobs ({pull}97014[#97014]).
* Updates MITRE Tactics, Techniques, and Subtechniques for 7.13 ({pull}97011[#97011]).
* Improves rule duplication user experience ({pull}96760[#96760]).
* Adds pre-packaged rule updates through the "Prebuilt Security Detection Rules" Fleet integration ({pull}96698[#96698]).
* Adds Nested CTI row renderer ({pull}96275[#96275]).
* Rebuild nested fields structure from fields response ({pull}96187[#96187]).
* Filters alerts table by presence of threat ({pull}96096[#96096]).
* Combine multiple timestamp searches into single request ({pull}96078[#96078]).
* Adds Indicator Match Timeline Template ({pull}95840[#95840]).
* Updated osquery autocomplete to v4.7.0 schema {pull}95741[#95741]).
* Populates threat.indicator.event with _source.event ({pull}95697[#95697]).
* Detection Rule Adoption ({pull}95659[#95659]).
* Adds Threat Summary and Threat Details tabs to Alert Side Panel ({pull}95604[#95604]).
* Timeline http endpoints ({pull}95036[#95036]).
...
* Update agent status labels and colors ({pull}99314[#99314]).
* Fixes an issue where many `OR` clauses take up too much vertical space ({pull}98706[#98706]).
* Text updates ({pull}98192[#98192]).
* Critical bug to add network responses to error toasters ({pull}97945[#97945]).
* Breaks very long hostnames so they are readable ({pull}97253[#97253]).
* Fixes failed to query DNS data - too_many_buckets_exception ({pull}97069[#97069]).
* Enable preview when only filters used ({pull}94018[#94018]).
* Fixes rule details activate switch ({pull}94010[#94010]).
* Sets our default date time to be "today" instead of "Last 24 hours" to enable cachability and fixes one date math bug in the URL ({pull}93548[#93548]).
* Update Cloud plugin to handle new config in kibana.yml ({pull}95569[#95569]).
