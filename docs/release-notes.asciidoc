...
[[features-7.14.0]]
==== Features
* Adds advanced policy option for Linux Malware quarantine ({pull}04984[#104984]).
* Updates text in Endpoint Admin pages ({pull}104649[#104649]).
* Fixes 7.14 UX issues ({pull}104257[#104257]).
* Adds a merge strategy key to kibana.yml and updates docker to have missing keys from security solutions ({pull}103800[#103800]).
* Adds Windows and macOs option to dropdown for creating Exceptions from scratch ({pull}103404[#103404]).
* Investigation time enrichment UI ({pull}103383[#103383]).
* Adds a version and OS check for Host Isolation ({pull}103026[#103026]).
* Shows case names in isolation success message ({pull}102664[#102664]).
* Implements best effort merging of constant_keyword, runtime fields, aliases, and copy_to fields ({pull}102280[#102280]).
* Paginates actions log with infinite scroll ({pull}102261[#102261]).
* Event enrichment search strategy ({pull}101553[#101553]).
* Cases in the observability app {pull}101487[#101487]).
* Adds Saved queries {pull}100965[#100965]).
* Validate path values for trusted apps ({pull}99035[#99035]).
* Cases UI Plugin for RAC {pull}97646[#97646]).
* Allows wildcard in trusted app paths ({pull}97623[#97623]).
* (Phase 1) Adds an application cache called metrics entities and integrates it within Security Solutions behind a feature flag {pull}96446[#96446]).
* Makes Endpoint Exception field options aware of OS, introduce OS selection to Endpoint Exceptions flow ({pull}95014[#95014]).
* Removes external "Launch App Search" button ({pull}100815[#100815]).
* Adds support for fingerprint processor ({pull}100541[#100541]).
* Adds support for registered_domain processor ({pull}99643[#99643]).

[discrete]
[[bug-fixes-7.14.0]]
==== Bug fixes and enhancements
* Adds overflow container for exceptions list table ({pull}103377[#103377]).
* Side Navigation phase 2 ({pull}103275[#103275])
Osquery telemetry updates ({pull}100754[#100754]).
* Implements "select all" rules ({pull}100554[#100554]).
* Adds Threat Intel dashboard links to Overview page ({pull}100423[#100423]).
* Swimlane Connector for Cases ({pull}100086[#100086)]).
* Improves missing privileges callout UX ({pull}98125[#98125]).
* Adds advanced policy option for Linux Malware quarantine ({pull}104984[#104984]).
* Fixes ip on threshold preview button when selecting an ip data type such as source.ip ({pull}105126[#105126]).
* Gets rid of rule exception comma delimiter for "is one of" operator ({pull}104960[#104960]).
* Fixes stale validation messages on the action edit form ({pull}104868[#104868]).
* Fixes rule table sort not working for certain fields ({pull}103960[#103960]).
* Allow activity log scrolling on small screens ({pull}103852[#103852]).
* Updates memoization logic of AlertsUtilityBar component ({pull}103746[#103746]).
* Disables loadPrebuiltRulesAndTemplatesButton ({pull}103568[#103568]).
* Fixes 404 error on deleted rule alert view ({pull}103491[#103491]).
* Includes actions and responses for endpoints only ({pull}103159[#103159]).
* Osquery telemetry usage bug ({pull}102762[#102762]).
* Fixes empty exceptions filter bug ({pull}102583[#102583]).
* Removes search bar on activity log ({pull}102550[#102550]).
* Does not show activity log error popups ({pull}102450[#102450]).
* Osquery app bug squashing ({pull}102406[#102406]).
* Actions log entries display relative time only up to 1 hr from now ({pull}102162[#102162]).
* Updates detection alert mappings to ECS v1.10.0 ({pull}101680[#101680]).
* Fixes timestamp bugs within source indexes when the formats are not ISO8601 format ({pull}101349[#101349]).
* Exposes the EQL query in the kibana logs for detections ({pull}100565[#100565]).
* Invalid KQL Query Bug ({pull}99442[#99442]).
* Fixes deleted rule view page ({pull}99406[#99406]).
* Updates APM index pattern ({pull}104344[#104344]).
* Adds media_type to set processor ({pull}101035[#101035]).
* Adds extract_device_type to user agent processor ({pull}100986[#100986]).
* Adds support for IP type in convert processor ({pull}100531[#100531]).
* Heatmap / Swim lane integration ({pull}97978[#97978]).
* Updates help text for common repository settings ({pull}97652[#97652]).
* Collects all documents that fail to transform before stopping the migration ({pull}96986[#96986]).
* Provides guidance of "Custom" allocation behavior in ILM ({pull}96111[#96111]).
* Fixes remote clusters details flyout for long strings ({pull}105592[#105592]).
* Fixes some hardcoded translations on Dashboard & Discover ({pull}105555[#105555]).
* Fixes of the `ja-JP.json not found` error ({pull}105023[#105023]).
* Remove default value for error_distance param ({pull}102222[#102222]).
* Fixes default paramenter for ignore_missing in csv processor ({pull}100316[#100316]).
* Fixes assignment v comparison and null type check in metrics detail page ({pull}95102[#95102]).
* Histogram IP legends error fixed ({pull}99468[#99468]).
