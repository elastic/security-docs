...
==== Breaking changes
* With the new ECS updates, threat intelligence data will not display on detection alerts with threat matches. To ensure threat intelligence data properly displays, users will need to migrate their alerts.

[discrete]
[[features-7.15.0]]
==== Features
* Adds new functionality and usability improvements to the Alerts page:
** Event rendered view (#108644)
** Adds reason field (#108449)
** Adds row renderer popover to alert table "reason" field (#108054)
** Adds reason field (#107532) → duplicate can only note one or include both prs?
** Changes in-progress status to acknowledged (#107972)
** Status filter refactor (#107249)
** Remove drag drop security solutions (#107162)
** Remove drag drop security solutions (#106721)
** Changes the JSON View tab name to JSON (#106524)
** Flyout overview hover actions (#106362)
** Adds alerts count table to Security Solutions alerts page (#106358)
** Update flyout table design (#105996)
* Users can add a Lens visualization to their case {pull}96703[#96703] {pull}109178[#109178].
* Adds indicator match rule {pull}107977[#107977].
...
* Improves the threat match query performance {pull}106150[#106150].
* Allows users to select a custom date range when exploring their endpoint’s activity log {pull}104085[#104085].
* User can make exceptions for memory protection alerts {pull}102196[#102196].
...
* Allows bulk close on exception to close acknowledged alerts {pull}110147[#110147].
* Removes missing fields the from the Trend histogram and count table on the Alerts page {pull}108843[#108843].
* Updates ECS 1.11 signal mappings {pull}108764[#108764].
* Adds the `operatorsList` property in exceptions builder that allows users to define options in the operators list. {pull}108015[#108015].
* Updates alerts enriched with threat intelligence to use the latest ECS threat fields {pull}107988[#107988].
...
* Shows the statuses of scheduled query {pull}106600[#106600].
...
* Fixes the bug that cause fields to reset on the Timeline page when users viewed the alert in Timeline {pull}109086[#109086].
