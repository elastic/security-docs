...
[[features-7.14.1]]
==== Features
The `securitySolution:defaultThreatIndex` advanced setting allows you to specify one or more threat intelligence index from which {es-sec} will collect threat match data. Features that query threat intelligence indices, such as the Threat Intelligence view on the Overview page, use the `securitySolution:defaultThreatIndex` setting and the `filebeat-*`` index is the default selection.

Cross cluster search can be enabled on this setting by adding threat indices from remote clusters. While the mappings for the Filebeat indices work out of the box, any other indices added should have ECS-compliant mappings for threat fields to ensure that threat-matching works properly ({pull}108389[#108389]).

[discrete]
[[bug-fixes-7.14.1]]
==== Bug fixes and enhancements
* Fixes the AlienVault OTX event count on the Threat Intelligence view ({pull}108448[#108448]).
* Fixes a configuration bug ({pull}107637[#107637]).
* Adds a migration for the `author` field ({pull}107230[#107230]).
