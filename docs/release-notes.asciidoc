...
[[features-7.14.0]]
==== Features
* Host Isolation is a way for {es-sec} users to isolate hosts from their networks, while investigating a potential attack. It stops a possibly infected host from infecting other hosts on the network.
* Adds the option to choose Windows and macOS when creating a new endpoint exception ({pull}103404[#103404]).
...
* Gets rid of rule exception comma delimiter for the "is one of" operator ({pull}104960[#104960]).
...
* Disables the Load Elastic prebuilt rules and timeline templates button if pre-built rules are loading ({pull}103568[#103568]).
...
* Fixes timestamp bugs within source indexes when the formats are not using ISO8601 format ({pull}101349[#101349]).
...
* Updates the APM index pattern ({pull}104344[#104344]).
...
* Fixes long strings in the remote clusters details flyout ({pull}105592[#105592]).
...
* Removes default value for the `error_distance` parameter ({pull}102222[#102222]).
* Fixes the default parameter for `ignore_missing` in the CSV processor ({pull}100316[#100316]).
* Fixes assignment v comparison and null type check in metrics detail page ({pull}95102[#95102]).
* Fixes the histogram IP legends error ({pull}99468[#99468]).
