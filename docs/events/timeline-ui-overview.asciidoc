...
- Explore your events by writing an {esql} query.
+
NOTE: An empty query bar will return an error message. 
+
For example, start the following query and iterate on it to fine tune your results: 
+
[source,esql]
----
...
----
+
** This query starts by querying documents within the Security alert index (`.alerts-security.alerts-default`) and indices specified in the <<update-sec-indices,Security data view>>.
** Then, the query limits the output to the top 10 results.
** Next, it keeps the `@timestamp`, `message`, `event.category`, `event.action`, `host.name`, `source.ip`, `destination.ip`, and `user.name` fields in the output.
