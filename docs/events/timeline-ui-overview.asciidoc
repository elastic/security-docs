...
From the **ES|QL** tab, write {ref}/esql.html[Elasticsearch Query Language (ES|QL)] queries to interact with your alert data, then visualize query results using {kibana-ref}/discover.html[Discover] functionality available in the tab.
...
NOTE: The default query searches the Security alert index (`.alerts-security.alerts-default`) and  indices specified in the Security data view. If you update the <<update-sec-indices,Security data view>> setting, the changes are reflected in the default query.