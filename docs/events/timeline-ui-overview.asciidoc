...
[discrete]
[[filter-with-esql]]
== Filter Timeline results with ES|QL

preview::[]

From the *ES|QL* tab, you can write {ref}/esql.html[ES|QL] queries to filter Timeline results.

The pre-configured ES|QL query returns the first ten results from the default Security alert index (`.alerts-security.alerts-default`) and indices defined in the <<update-sec-indices,default Security Data View>>. Expand the query, or remove it and create your own. 

[role="screenshot"]
image::images/esql-tab.png[a Timeline's ES|QL tab]

The *ES|QL* tab uses the {kibana-ref}/discover.html[Discover] UI, and allows you to:
