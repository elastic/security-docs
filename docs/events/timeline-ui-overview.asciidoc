...
contextual information, called *Event renderers*. All event renders are turned on by default. To turn them off, use the **Event renderers** toggle at the top of the results pane. To only turn off specific event renders, click the gear (image:images/customize-event-renderers.png[The customize event renderer button,20,20]) button next to the toggle, and deselect the ones you want disabled. Close the **Customize event renderers** pane when you're done. Your changes are automatically applied to Timeline.
...
To add a field, hover over the field and click the **Add field as a column** button (image:images/add-field-button.png[The button that lets you to add a field as a column,20,20]) or drag and drop the field into the table. To remove a field, hover over the field, and click the **Remove field as a column** button (image:images/remove-field-button.png[The button that lets you to remove a field as a column,20,20]). 
