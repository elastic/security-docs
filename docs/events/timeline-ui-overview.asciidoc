...
The *ES|QL* tab allows you to interact with your alerts using {ref}/esql.html[Elasticsearch Query Language (ES|QL)] queries and {kib-ref}/discover.html[Discover] functionality.  

The tab's default query specifies Security alert indices as the data source and searches for alerts that were generated during the last 15 minutes. Up to 10 results can be returned in the table beneath the query bar. You can modify the query's data source, change its time range, and increase the `limit` to display different results.

NOTE: The default query searches the Security alert index (`.alerts-security.alerts-default`) and indices specified in the <<update-sec-indices,Security data view>>. Any changes you make to the Security data view setting are reflected in the query.

[role="screenshot"]
image::images/esql-tab.png[a Timeline's ES|QL tab]

As a security analyst, you can use the *ES|QL* tab to interact with your alert data as you investigate potential threats. Refer to the Introduction to ES|QL blog article for examples.

TIP: To learn more about ES|QL syntax, click the **ES|QL reference** button [book icon] to the right of the query bar.
