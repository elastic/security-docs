...
You can use ES|QL in Timeline by opening the **ES|QL** tab. From the tab, you can: 
...
- Visualize query results using {kibana-ref}/discover.html[Discover] functionality that's available in the tab.
...
To get started using ES|QL, read the tutorial on {ref}/esql.html[using ES|QL in Kibana]. Much of the functionality that's offered in Kibana is also offered in Timeline.  

To find examples of ES|QL queries used during threat hunting activities, check out https://www.elastic.co/blog/introduction-to-esql-new-query-language-flexible-iterative-analytics[our blog].
