...
Use the *ES|QL* tab to write {ref}/esql.html[ES|QL] queries that allow you to iteratively explore and transform your alert data.

The tab's default query searches the Security alert index (`.alerts-security.alerts-default`) and <<update-sec-indices,Security data view>> indices for alerts that were recently generated. By default, the query looks for alerts that were generated during the last 15 minutes and returns up to 10 results. Modify the query time range and increase the `limit` to find more alerts. 

TIP: To learn more about ES|QL syntax, click the **ES|QL reference** button [book icon] to the right of the query bar.

The *ES|QL* tab uses the Discover UI to present query results. Refer to {kibana-ref}/discover.html[Discover] to learn more about Discover features that are available in the tab. 

[role="screenshot"]
image::images/esql-tab.png[a Timeline's ES|QL tab]

As a security analyst, you can use the *ES|QL* tab to: 
