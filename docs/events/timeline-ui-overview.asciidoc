...
The following image shows what matched ordered events look like in the Timeline table. Events the belong to the same sequence are grouped together, shaded red or blue, and ordered by the time that they occurred. By default, matched events are ordered from oldest to newest. 
...
* Choose whether you want to see all data sources, only events, only detection alerts, or a custom data source (custom data sources might include Kibana index patterns). By default, all data sources are displayed.
