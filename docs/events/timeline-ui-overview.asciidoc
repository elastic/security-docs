...
Go to the right of the date and time picker, then click *Data view*. To hide raw events and display <<det-engine-terminology, detection alerts>> only, select *Show only detection alerts*. This setting is not selected by default. Keep it de-selected to display both raw events and detection alerts in Timeline.
