...
TIP: Click the star icon (image:images/favorite-icon.png[Click the favorite icon,20,20]) to favorite your Timeline so you can quickly find it later.
...
TIP: Collapse the query builder and provide more space for Timeline results by clicking the query builder button (image:images/query-builder-button.png[Click the query builder button,20,20]).
...
== Create new or open existing Timelines

To make a new Timeline from the Timelines page, click **Create new Timeline**. To make one from an open Timeline, click **New** → **New Timeline**. 

To open an existing Timeline from the Timelines page, click a Timeline's title. To do so from a Timeline, click **Open**, then click the a Timeline's title.
