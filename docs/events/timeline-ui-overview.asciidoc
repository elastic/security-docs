...
right corner, then select one of these:
...
. Click the *All actions* icon in the relevant row, then select the appropriate action:
...
. To import Timelines, click *Import*, then select or drag and drop the Timeline `ndjson` file.
+
...
* Choose which data view to show, and whether to show only detection alerts.
