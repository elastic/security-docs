...
Use the *Correlations* tab to investigate Timeline results with EQL queries. You can enter EQL queries into the free-form text box under this tab and a link to EQL documentation is available in the bottom right corner of the text box.
...
The following image shows what matched ordered events look like in the Timeline table. Events are paired into sets of two and shaded red or blue to indicate that they are matched:

From the *Correlations* tab you can also perform the following:

* Specify the span of time that you want to investigate.
* Re-order the columns and choose which fields display.
* Choose whether you want to see events, detection alerts, all data sources, or a custom data source. All data sources are displayed by default.
