...
NOTE: The {esql} tab is turned on and displays by default. To turn it off hide it, you must edit your {cloud}/ec-manage-kibana-settings.html#ec-manage-kibana-settings[{kib} user settings] and set the `timelineEsqlTabDisabled` feature flag to `true`. 
