...
From the **ES|QL** tab, write {ref}/esql.html[Elasticsearch Query Language (ES|QL)] queries that interact with your alert data, then visualize query results using {kibana-ref}/discover.html[Discover] functionality in the tab.

TIP: Click the **ES|QL reference** button (image:images/esql-ref-button.png[Click the ES|QL reference button,20,20]) to the right of the query bar to open the ES|QL syntax reference guide.

[role="screenshot"]
image::images/esql-tab.png[a Timeline's ES|QL tab]

ES|QL provides a powerful way to filter, transform, and analyze alert data stored in Elasticsearch. ES|QL queries use "pipes" to manipulate and transform data in a step-by-step fashion. This approach allows you to compose a series of operations, where the output of one operation becomes the input for the next, enabling complex data transformations and analysis. To learn more about leveraging ES|QL for threat hunting, check out https://www.elastic.co/blog/introduction-to-esql-new-query-language-flexible-iterative-analytics[our blog].

The **ES|QL** tab's default query searches for alerts that were generated during the last 15 minutes, and returns up to 10 results. Modify any part of the query and selected time frame to display different results.

NOTE: In addition to searching the Security alert index (`.alerts-security.alerts-default`), the default query checks indices specified in the <<update-sec-indices,Security data view>>. Any changes you make to the Security data view setting are reflected in the default query.