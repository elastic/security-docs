...
The {ref}/esql.html[Elasticsearch Query Language (ES|QL)] provides a powerful way to filter, transform, and analyze alert data stored in Elasticsearch. ES|QL queries use "pipes" to manipulate and transform data in a step-by-step fashion. This approach allows users to compose a series of operations, where the output of one operation becomes the input for the next, enabling complex data transformations and analysis.

TIP: Check out https://www.elastic.co/blog/introduction-to-esql-new-query-language-flexible-iterative-analytics[our blog] to learn how you can leverage ES|QL for threat hunting.

Use the *ES|QL* tab to write ES|QL queries that interact with your alert data, then visualize query results using {kibana-ref}/discover.html[Discover] functionality in the tab. 

[role="screenshot"]
image::images/esql-tab.png[a Timeline's ES|QL tab]

The tab's default query searches for alerts that were generated during the last 15 minutes, and returns up to 10 results. Modify any part of the query and selected time frame to display different results. You can also click the **ES|QL reference** button [book icon] to the right of the query bar to open a reference guide for ES|QL syntax.
