...
The Intelligence page makes it easy to access and analyze your threat intelligence data by centralizing Indicators of Compromise (IoCs) from enabled threat intelligence feeds and providers. This guide defines threat intelligence and IoCs, helps you get started using the Intelligence page, and explains how to work with indicators.
...
* *{filebeat}* - Install {filebeat-ref}/filebeat-installation-configuration.html[{filebeat}] version 8.x. Earlier versions are incompatible with ECS and will prevent indicator data from displaying in the Indicators table.
--
...
** *{agent} threat intelligence integrations* - Indicator documents are stored in the `logs_ti*` index.
** *{filebeat} threat intelligence integrations* -
Indicator documents are stored in the `filebeat-*` index.
