...
After deploying {elastic-defend}, you might encounter warnings or errors in the endpoint **Policy status** in {fleet} if your mobile device management (MDM) is misconfigured or certain permissions for {elastic-endpoint} aren't granted. Read the following sections to learn more about issues that can cause warnings or failures in the endpoint policy status.
