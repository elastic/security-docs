...
* `Operator`: Select an operator to define the condition:
   ...
   ** `matches`: Can include wildcards in `Value`, such as `C:\path\*\app.exe`. This option is only available for the `Path` field type. Available wildcards are `?` (match any one character) and `*` (match zero to many characters).
