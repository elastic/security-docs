...
* `Field`: Select the field you want to use to identify the trusted application:
** `Hash`: The hash value of the application's executable. The hash can be MD5, SHA-1, or SHA-256.
** `Path`: The full file path of the application's executable.
** `Signature`: (Windows only) The name of the application's digital signer.
+
TIP: To find the signer's name for an application, go to *Kibana* -> *Discover* and query the process name of the application's executable (for example, `process.name : "app.exe"`). Then search the results for the `process.code_signature.subject_name` field, which contains the signer's name.
