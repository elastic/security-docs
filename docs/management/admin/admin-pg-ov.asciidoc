...
** *Reassign agent policy:* Change the agent policy assigned to the host in {fleet}. See {fleet-guide}/agent-policy.html#apply-a-policy[Apply a policy].
...
Click any link in the *Endpoint* column to display host details in a flyout. You can also use the *Take Action* menu button to perform the same actions as those listed in the Actions context menu (such as isolating the host, viewing host details, viewing and reassigning the agent policy, and so on).
...
NOTE: The time picker on the right side of the page allows you to set a time interval to automatically refresh the Endpoints list — for example, if new endpoints were added or deleted.
