...
Click any *Hostname* link to display those host details in a flyout. You can also execute the same actions as those listed in the Actions context menu from the *Take Action* menu (view the associated policy, view the response details, and reassign the policy).
...
To view policy status details, click the link and review the data in the flyout that displays. In the following image, you can see that the malware configuration and logging failed, generating a "Failed" policy status.
