...
** `Unhealthy`: The agent is online but requires attention from an administrator because it's reporting a process being unhealthy. An unhealthy status could also mean an upgrade failed and was rolled back to its previous version. Refer to <<ts-unhealthy-agent,Troubleshooting>> for more on resolving an unhealthy agent status.
...
The status of the integration policy appears in the *Status* column and displays one of the following possibilities:
...
To view policy status details, click the link and review the data in the details flyout.
