...
Isolated hosts, however, can still send data to {es} and {kib}. You can also create <<host-isolation-exceptions, host isolation exceptions>> to allow isolated hosts to communicate with specific IP addresses even after they've been blocked from the rest of your network
