...
After the host is successfully isolated, an *Isolated* status is added to the endpoint. Any active end user receives a notification that the computer has been isolated from the network:
...
After the host is successfully released, the *Isolated* status is removed from the endpoint. Any active end user receives a notification that the computer has been reconnected to the network:
