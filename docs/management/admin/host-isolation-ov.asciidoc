...
After the host is successfully isolated, an *Isolated* status is added next to *Agent Status*. Any user currently on the host receives a notification that the computer has been isolated from the network:
...
After the host is successfully released, the *Isolated* status is removed from the *Agent Status*. Any user currently on the host receives a notification that the computer has been reconnected to the network:
