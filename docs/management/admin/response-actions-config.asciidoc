...
. **Generate API access tokens in SentinelOne.** You'll need these tokens in later steps, and they allow {elastic-sec} to collect data and perform actions in SentinelOne. Create two API tokens, and give each token the least privilege required for its usage:
+
--
   - SentinelOne integration token: Permission to read SentinelOne data.
   - SentinelOne connector token: Permission to perform actions on SentinelOne-protected hosts (for example, isolating and releasing an endpoint).
--
+
