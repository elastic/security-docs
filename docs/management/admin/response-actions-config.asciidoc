...
. **Install the SentinelOne integration.** The integration uses the SentinelOne API to collect and ingest logs into {elastic-sec}.
+
.Expand for details
[%collapsible]
====
...
. **Create a SentinelOne connector.** This connector enables {elastic-sec} to perform actions on SentinelOne-protected hosts.
+
.Expand for details
[%collapsible]
====
