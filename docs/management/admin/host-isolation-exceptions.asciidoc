...
* `Per Policy` (Platinum or Enterprise subscription only): Assign the host isolation exception to one or more specific {endpoint-sec} integration policies. Select each policy where you want the host isolation exception to apply.
+
...
You can individually configure each host isolation exception. With a Platinum or Enterprise subscription, you can also change the policies applied to each host isolation exception.
