...
Host isolation is a https://www.elastic.co/pricing[Platinum or Enterprise subscription] feature. By default, a host isolation exception is recognized globally across all hosts running {endpoint-cloud-sec}. You can also assign a host isolation exception to a specific {endpoint-cloud-sec} integration policy, affecting only the hosts assigned to that policy.
...
+
* `Global`: Assign the host isolation exception to all integration policies for {endpoint-cloud-sec}.
* `Per Policy`: Assign the host isolation exception to one or more specific {endpoint-cloud-sec} integration policies. Select each policy where you want the host isolation exception to apply.
+
...
You can delete a host isolation exception, which removes it entirely from all {endpoint-cloud-sec} policies.
