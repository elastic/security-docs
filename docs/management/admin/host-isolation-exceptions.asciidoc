...
By default, a host isolation exception is recognized globally across all hosts running {endpoint-sec}. If you have a https://www.elastic.co/pricing[Platinum or Enterprise subscription], you can also assign a host isolation exception to a specific {endpoint-sec} integration policy, affecting only the hosts assigned to that policy.
...
* `Per Policy` (Platinum or Enterprise subscription only): Assign the host isolation exception to one or more specific {endpoint-sec} integration policies. Select each policy in which you want the host isolation exception to be applied.
+
NOTE: You can also select the `Per Policy` option without assigning a policy to the host isolation exception at this time. For example, you could do this if you want to create and review your host isolation exception configurations before putting them into action with a policy.
...
You can change the configuration of each host isolation exception. If you have a Platinum or Enterprise subscription, you can also change the policy assigned to a host isolation exception.
