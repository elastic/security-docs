...
If a host is unavailable, pending actions will wait to execute once the host comes online. Pending actions will wait for two weeks before they expire, and can be tracked in the actions log.
...
IMPORTANT: Once you submit a response action, it cannot be cancelled, even if it's pending for an offline host.
