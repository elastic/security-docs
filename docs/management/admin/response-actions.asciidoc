...
NOTE: When {elastic-defend} prevents file activity due to <<malware-protection,malware prevention>>, the file is quarantined on the host and a Malware Prevention Alert is created. To retrieve this file, copy the path from the alert's *Quarantined file path* field (`file.Ext.quarantine_path`) and paste it into the `--path` parameter.
