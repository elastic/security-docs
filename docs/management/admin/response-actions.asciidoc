...
Click *Response actions history* to display a log of the response actions performed the host, such as isolating a host or terminating a process. You can use the tools at the top to filter the information displayed in this view. Refer to <<response-actions-history>> for more details.
