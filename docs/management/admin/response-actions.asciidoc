...
Retrieve a file from a host. You must include the following parameter to specify the file's location on the host:
...
When you use `get-file` to retrieve a quarantined file, it's saved in a password-protected .zip archive to prevent it from running. Use the passcode `elastic` to open the .zip in a safe environment.
====
