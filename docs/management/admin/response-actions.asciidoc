...
Upload a file to the host. The file is saved to the location on the host where {elastic-endpoint} is installed; the full path is returned in the console for reference after you run the command.
...
TIP: You can combine this with the `execute` response action to upload and run scripts for mitigation or other purposes.
