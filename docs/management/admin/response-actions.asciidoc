...
Upload a file to the host. The file is saved to the location on the host where {elastic-endpoint} is installed; the full path is returned in the console for reference after you run the command. Use these parameters:
...
TIP: You can follow this command with the `execute` response action to upload and run scripts for mitigation or other purposes.

NOTE: By default, the file size limit is 25 MB, configurable up to 100 MB by modifying the `maxUploadResponseActionFileBytes` setting in `kibana.yml`. The value must be entered in bytes (the maximum is `104857600` bytes).
