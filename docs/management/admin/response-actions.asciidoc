...
Scan a specific file or directory on the host for malware. The scan uses the <<malware-protection,malware protection settings>> (such as Detect or Prevent options, or enabling the blocklist) in the host's associated {elastic-defend} integration policy. Use these parameters:
