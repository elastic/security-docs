...
WARNING: This response action runs commands as a highly privileged, root-level user on the host. Be careful with any commands that could cause irrevocable changes to a host.
