...
Only one entry item is allowed for each blocklist item (`file.path`, `file.hash.*` or `file.Ext.code_signature`) except for `file.hash.*` where we allow an entry for each allowed hash types.
