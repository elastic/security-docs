...
Only one entry of `file.path`, `file.hash.*` or `file.Ext.code_signature` is allowed per blocklist item. Each entry can have one or more values. You can also add a list of hashes for each allowed hash type (`md5`, `sha1`, or `sha256`) to the `file.hash.*` entry.
