...
* `field` : The `process.hash.md5`, `process.hash.sha1` or `process.hash.sha256` values are supported.
...
Only one entry item is allowed per blocklist item (`file.path`, `file.hash.*` or `file.Ext.code_signature`). The exception is `file.hash.*`, which allows an entry for each of the allowed hash types.
