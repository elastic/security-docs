...
Create, retrieve, update and delete endpoint <<event-filters>> via API. {endpoint-artifact-name} are managed via the <<exceptions-api-overview>> using a static container id (`list_id`) of `pass:a[{endpoint-artifact-list-id}]` , which must be created prior to adding a event filter. Access to these APIs require that a user have authorization to manage endpoints.
...
[#exception-event-filter-item-entry-schema]
===== `entry` object schema

{endpoint-artifact-name} allow for an unlimited number of condition to be defined. The supported type operators are: 

====== Operator `is`

Whatever matches with a single value
...
Whatever doesn not matches with a single value
...
Whatever matches with an array of values
...
Whatever does not matches with an array of values
...
You can add nested conditions to your entries array, for example:
