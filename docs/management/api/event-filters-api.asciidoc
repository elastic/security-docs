...
{endpoint-artifact-name} allow for an unlimited number of conditions to be defined. Each {endpoint-artifact-name-single} entry contains an `entry` object that has`type`, `value`, `field` and `operator` keys with values. The following operators are supported in an entry object: 
