...
Create, retrieve, update and delete endpoint <<event-filters-ov>> via API. {endpoint-artifact-name} are managed via the <<exceptions-api-overview>> using a static container id (`list_id`) of `pass:a[{endpoint-artifact-list-id}]` , which must be created prior to adding a event filter. Access to these APIs require that a user have authorization to manage endpoints.
