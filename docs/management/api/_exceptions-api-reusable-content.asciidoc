...
// tag::endpoint-exceptions-item-create-update-payload-requirements[]
- The `list_id` value must be `pass:a[{endpoint-artifact-list-id}]`.
- The `namespace_type` value must be `agnostic`.
- The `type` value must be `simple`.
// end::endpoint-exceptions-item-create-update-payload-requirements[]
...
ifeval::["{endpoint-artifact-list-id}" == "endpoint_host_isolation_exceptions"]
All of the following valid OS types should be used when creating {endpoint-artifact-name}:
endif::[]
ifeval::["{endpoint-artifact-list-id}" == "endpoint_host_isolation_exceptions"]
