...
Create, retrieve, update, and delete endpoint <<host-isolation-exceptions>> via API. {endpoint-artifact-name} are managed via the <<exceptions-api-overview>> using a static container id (`list_id`) of `pass:a[{endpoint-artifact-list-id}]`, which must be created prior to adding a trusted application. Access to these APIs requires that a user has authorization to manage endpoints.
...
include::_exceptions-api-reusable-content.asciidoc[tags=endpoint-exceptions-item-create-update-payload-requirements]
...
Deletes a {endpoint-artifact-name-single} with `item_id` of `1342081d-66ad-4b03-b554-131195ce404b`:
...
{endpoint-artifact-name} allow for at most 1 condition to be defined. The following entry is supported by {endpoint-artifact-name}:

====== Destination IPs

Destination IPs are supported by all xref:exception-item-os-types[OS types]. A destination IP entry has the following structure:
