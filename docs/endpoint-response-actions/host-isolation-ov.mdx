...
Isolated hosts, however, can still send data to ((es)) and ((kib)). You can also create <DocLink id="enSecurityHostIsolationExceptions">host isolation exceptions</DocLink> for specific IP addresses that isolated hosts are still allowed to communicate with, even when blocked from the rest of your network.
...
* To isolate and release hosts running any operating system, you must have the **Host Isolation** <DocLink id="enSecurityEndpointManagementReq">privilege</DocLink>.
...
All actions executed on a host are tracked in the host’s response actions history, which you can access from the Endpoints page. Refer to <DocLink id="enSecurityHostIsolationOv" section="view-host-isolation-history">View host isolation history</DocLink> for more information.
...
1. Open the response console for the endpoint (*Manage* -> **Endpoints** -> **Actions** menu (*...*) -> **Respond**).
...
1. Add an endpoint response action to a new or existing custom query rule. The endpoint response action will run whenever rule conditions are met: 
    * **New rule**: On the last step of <DocLink id="enSecurityRulesUiCreate" section="create-a-custom-query-rule">custom query rule</DocLink> creation, go to the Response Actions section and click the **Endpoint Security** icon.
...
<DocImage size="m" url="../images/host-isolation-ov/-management-admin-host-isolated-notif.png" alt="Host isolated notification message" />
...
<DocImage size="m" url="../images/host-isolation-ov/-management-admin-host-released-notif.png" alt="Host released notification message" />
...
Go to **Manage** -> **Endpoints**, click an endpoint's name, then click the **Response action history** tab. You can filter the information displayed in this view. Refer to <DocLink id="enSecurityResponseActionsHistory">Response actions history</DocLink> for more details.
