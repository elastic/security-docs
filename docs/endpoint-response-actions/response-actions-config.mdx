...
title: Response actions configuration
description: Configure third-party systems to perform response actions on protected hosts.
...
1. **Create a SentinelOne connector.** Elastic's [SentinelOne connector](((kibana-ref))/action-types.html) enables ((elastic-sec)) to perform actions on SentinelOne-protected hosts.
