...
* Data is shipped from your hosts to ((es)) in the following ways:
    ...
    * [Beat modules](https://www.elastic.co/integrations?solution=security): ((beats))
        ...
        <DocLink id="enSecurityIngestData" section="enable-modules-and-configuration-options">here</DocLink>.
...
<DocLink id="enSecurityIngestData">Ingest data to Elastic Security</DocLink> and <DocLink id="enSecurityInstallEndpoint">Install and configure the ((elastic-defend)) integration</DocLink> describe how to ship security-related
data to ((es)).
...
* Index patterns controlled by {book['es-sec']}, including signals and list indices
...
You can use {book['es-sec']} with other Elastic products and features to help you
...
By default, {book['es-sec']} monitors [APM](((apm-app-ref))/apm-getting-started.html)
...
events. {book['es-sec']} supports events and indicator index data from any ECS-compliant data source.

<DocCallOut title="Important" color="warning">
{book['es-sec']} requires [ECS-compliant data](((ecs-ref))). If you use third-party data collectors to ship data to ((es)), the data must be mapped to ECS.
\<\<siem-field-reference>> lists ECS fields used in {book['es-sec']}.
</DocCallOut>
