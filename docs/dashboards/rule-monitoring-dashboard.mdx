---
id: serverlessSecurityRuleMonitoringDashboard
...
* At least `Read` [((kib)) privileges](((kibana-ref))/kibana-role-management.html#adding_kibana_privileges) for both the **Analytics > Dashboard** and **Security > Security** ((kib)) features.

* At least `read` [index privileges](((kibana-ref))/kibana-role-management.html#adding_index_privileges) for the `.kibana-event-log-*` index.
...
Open a panel's options menu (<DocImage flatImage alt="Options menu,18,18" url="../images/rule-monitoring-dashboard/-detections-three-dot-icon.png" />) customize the panel or use its data for further analysis and investigation:
