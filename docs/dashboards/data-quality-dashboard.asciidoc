...
== Check indices
When the dashboard gets opened for the first time in a project, data does not appear until you (or a teammate) select indices to check. 
...
Once checked, an index's data quality information persists indefinitely. You can see when it was last checked, and recheck it at any time.

IMPORTANT: Data quality results are stored in a data stream using the following index pattern: `kibana-data-quality-dashboard-results-<spaceId>`. For example, results from the default space are stored in: `kibana-data-quality-dashboard-results-default`. 
