...
IMPORTANT: The dashboard uses data from the `logs-*` index pattern, which must be present in the {security-guide}current/advanced-settings.html#update-sec-indices[`securitySolution:defaultIndex` advanced setting] for the dashboard to function (it is present by default). To collect data from multiple {es} clusters (a cross-cluster deployment), update `logs-*` to `*-logs-*`.
