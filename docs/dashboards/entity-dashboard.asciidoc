...
Anomaly detection jobs identify suspicious or irregular behavior patterns. The Anomalies table displays the total number of host and user anomalies identified by these prebuilt {ml} jobs (named in the **Anomaly name** column).

.Requirements
[sidebar]
-- 

To display anomaly data, you must {ml-docs}/ml-ad-run-jobs.html[install and run] one or more <<prebuilt-ml-jobs, prebuilt anomaly detection jobs>>. Custom anomaly detection jobs cannot be added to the Entity Analytics dashboard.
...
If anomaly data is missing, click the *Run job* link next to a {ml} job to install and start the job. 
