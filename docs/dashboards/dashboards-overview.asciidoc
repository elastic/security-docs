...
To create a new custom dashboard, click **Create Dashboard**. You can control which custom dashboards appear in the table by using the text search field to filter by name and description, or by using the *Tags* menu to filter by tag. You can also click one of a custom dashboard's tags to toggle whether to filter for that tag. To create a new tag, open the *Tags* menu and click *Manage tags*.
