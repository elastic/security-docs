...
+
IMPORTANT: If you've upgraded from {stack} version 7.15.0 or earlier to 7.16.0 or later, you must complete several prerequisites before creating a new {sn-itsm} or {sn-sir} connector. For more information, refer to {kibana-ref}/servicenow-itsm-connector-prerequisites.html[{sn-itsm} connector and action] and {kibana-ref}/servicenow-sir-connector-prerequisites.html[{sn-sir} connector and action].
...
| *Use OAuth authentication* | ({sn} only) Enable the toggle to use open authorization (OAuth) to authenticate a connection between Elastic and {sn}. Refer to the {sn} {kibana-ref}/action-types.html[Connector] topics for more information.

To use open authorization (OAuth), you must {kibana-ref}/servicenow-itsm-connector-prerequisites-rsa-key.html[create an RSA keypair and add an X.509 Certificate] and also {kibana-ref}/servicenow-itsm-connector-prerequisites-endpoint.html[create an OAuth JWT API endpoint for external clients with a JWT Verifiers Map.]
...
| *Client ID* | ({sn} only and displays if *Use OAuth authentication* is toggled on) The client ID assigned to your OAuth application.
...
| *JWT Verifier Key ID* | ({sn} only and displays if *Use OAuth authentication* is toggled on) The key ID assigned to the JWT Verifier Map of your OAuth application.
