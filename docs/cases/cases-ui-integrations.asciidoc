...
** *All*: If you select this connector type, you can choose to set all or no field mappings before creating your new {swimlane} connector. However, note that if you don't set field mappings now, you'll be prompted to do so if you want to use the connector for a case or a rule.
** *Alerts*: If you select this connector type, you must provide an alert ID and rule name.
** *Cases*: If you select this connector type, you must provide a case ID, a case name, comments, and a description.
...
NOTE: Data from mapped case fields can be pushed to external systems, but not pulled in.
...
Security case title is sent to {swimlane}, the existing {swimlane} `caseName` field is
overwritten.
** *Description*: Mapped to the {swimlane} `Description` field. When an update to a
Security case description is sent to {swimlane}, the existing {swimlane} `Description`
field is overwritten.
** *Comments*: Mapped to the {swimlane} `Comments` field. When a comment is updated
in a Security case, a new comment is added to the {swimlane} incident.
