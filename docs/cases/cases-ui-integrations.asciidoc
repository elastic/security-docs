...
number.
* *Application ID* (Swimlane connectors only): The application ID of your Swimlane application.
...
* *API token* ({swimlane} connectors only): The {swimlane} API authentication token used for HTTP Basic authentication.

. Choose the connector type (for {swimlane} connectors only):
** *All*: If you select this connector type, you can set all or no fields.
** *Alerts*: If you select this connector type, you must provide an alert ID, rule name, and alert source.
** *Cases*: If you select this connector type, you must provide a case name, case ID, description, and comments.
