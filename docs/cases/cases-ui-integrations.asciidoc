...
field is overwritten.
** *Comments*: Mapped to the {swimlane} `Comments` field. When a new comment is added to a Security case, or an existing one is updated, the field that is mapped to the {swimlane} `Comment` field is ield is appended. Within Swimlane, new and updated comments display individually.
