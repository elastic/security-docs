...
| *Use OAuth authentication* | ({sn} only) Enable the toggle authenticate a connection between Elastic and {sn}. Refer to the {sn} {kibana-ref}/action-types.html[Connector] topics for more information.
...
| *Username* | ({sn} only) The username of the {sn} account used to access the {sn} instance.
