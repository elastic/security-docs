...
* **Open duration**: Time has passed since the case was created.
* **In progress duration**: How long the case has been in the `In progress` state.
* **Duration from creation to close**: Time has passed from when the case was created to when it was closed.
