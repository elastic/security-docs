...
[[cases-export]]
=== Export a case
Exporting cases can help you back up your cases or move them from one space to another. When you export a case, the case details, user actions, and comments are exported to a delimited JSON (ndjson) file. During the export process, text string comments, case alerts, and lens visualizations are exported.

NOTE: Exported alerts will only contain indices' names, the alert document ID, and the ID and name of the rule that triggered the alert. Lens visualizations are exported as a JSON blob.
