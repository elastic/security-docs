...
Cases can be exported and imported from the {kibana-ref}/managing-saved-objects.html[Saved Objects] UI in Kibana.

To export cases:

. Open the main menu, then click *Stack Management > Kibana > Saved Objects*.
. Search for the case by saved object type or by entering the case's name into the search bar.
. Select the case and click the *Export* button.
...
[role="screenshot"]
image::images/cases-export-button.png[]
