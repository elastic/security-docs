...
<<cases-ui-integrations>> describes how to set up external integrations. When configuring case fields, note that data from mapped fields can be pushed to external systems but not pulled in.

You can create and manage cases via the UI or the <<cases-api-overview>>.

NOTE: To send cases to external systems, you need the
https://www.elastic.co/subscriptions[appropriate license]. For information about the role privileges you need to view or update cases, see <<case-permissions>>.
...
NOTE: To create and update cases, your role requires *All* sub-feature privileges for the *Case* feature. To push cases to external systems, your role also needs *All* privileges for the *Action and Connectors* feature. See <<case-permissions>> for more information.
