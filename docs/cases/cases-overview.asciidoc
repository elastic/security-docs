...
IMPORTANT: Before importing a Lens visualization into a space, ensure the visualization's data is present. Without it, the Lens visualization won't work.

[float]
[[cases-export]]
=== Export a case
Use the export option to move cases between different Kibana instances. When you export a case, the case details, user actions, and comments are exported to a newline-delimited JSON (NDJSON) file. During the export process, text string comments, case alerts, and lens visualizations are exported.

NOTE: Exported alerts will only contain indices' names, the alert document ID, and the ID and name of the rule that triggered the alert. Lens visualizations are exported as JSON blobs.
...
+
IMPORTANT: If you've imported a case with a connector, you'll be prompted to re-authenticate the connector. To get to your connector, open the main menu, then click *Stack Management -> Alerts and Insights -> Rules and Connectors -> Connectors*.
