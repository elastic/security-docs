...
Add indicators to <<blocklists, blocklist>> to prevent selected applications from running on your hosts. When creating a blocklist, you can add indicators that have hash values for the MD5, SHA-1, or SHA-256 fields and are of the `file` indicator type.
...
. In the **Conditions** section, verify that the information about the indicator is correct.
