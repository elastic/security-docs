...
You can add indicator values to the blocklist from the Indicators table or the Indicator details flyout. From the Indicators table, select *More actions* (*...​*) -> *Add blocklist entry*.  Alternatively, open an indicator's details, then select *Take action -> Add blocklist entry*. 
