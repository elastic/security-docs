...
When you add an indicator to Timeline, a new Timeline opens with an auto-generated KQL query. The query includes the selected indicator and related fields that have been mapped from relevant source events. The query's time range is automatically set to seven days before and seven after the indicator's `timestamp`. 

You can use the query results to identify and predict related events. They can also help you assess and respond to potential threats.
