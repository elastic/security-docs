...
You can add an indicator to the blocklist from the Indicators table or the Indicator details flyout. From the Indicators table, click the *More actions menu* (*...​*) menu, then select *Add blocklist entry*.  Alternatively, open an indicator's details, then select *Take action -> Add blocklist entry*. 

NOTE: Refer to <<blocklist,blocklist>> for more information about creating and managing blocklist entries. 
