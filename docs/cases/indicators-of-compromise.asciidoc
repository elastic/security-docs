...
Add indicators to blocklists to prevent selected applications from running on your hosts by extending the list of processes the Endpoint considers malicious.

To add an indicator to a blocklist:

. From the Indicators table, click the *More actions menu* (*...​*). Alternatively, open an indicator's details, then select *Take action*.
. Select *Add blocklist entry*. The *Add blocklist* flyout appears. 
...
. In the **Conditions** section, enter the following information about the indicator you want to block:
