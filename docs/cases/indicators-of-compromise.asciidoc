...
Investigate an indicator in <<timelines-ui,Timeline>> to find related security events in your environment. You can add an indicator to Timeline from the Indicators table or the Indicator details flyout.

[role="screenshot"]
image::images/indicator-query-timeline.png[Shows the results of an indicator being investigated in Timeline]

When you add an indicator to Timeline, a new Timeline opens with an auto-generated KQL query. The query includes the selected indicator and related fields mapped from relevant source events. The query's time range is automatically set to seven days before and seven after the indicator's `timestamp`. 

You can use the Timeline query results to identify and predict related events. They can also help you assess and respond to potential threats.

[discrete]
[[example-indicator-timeline]]
=== Example indicator Timeline investigation  

The following image shows a file hash indictor being investigated in Timeline. The indicator field and value pair is:
...
The auto-generated query contains the indicator and the related `file.hash.sha256` field:

`file.hash.sha256 : c207213257a63589b1e1bd2f459b47becd000c1af8ea7983dd9541aff145c3ba`

The query results sh/Users/nastashasolomon/Documents/Elastic/security-docs/docs/cases/images/indicator-in-timeline.pngow a related alert that has a matching `file.hash.sha256` field value. This alert can be further analyzed to determine if it suggests suspicious or malicious activity in the environment.  
