...
Each case has a universally unique identifier (UUID) that you can copy and share. To copy a case's UUID to a clipboard, go to the Cases page and find the case you want to share, click the *Actions* menu, then select *Copy Case ID*. Alternatively, go to a case's details page, click the *More actions* menu (…​), and select *Copy Case ID*.

[role="screenshot"]
image::images/cases-copy-case-id.png[Copy Case ID option in More actions menu 60%,60%]
