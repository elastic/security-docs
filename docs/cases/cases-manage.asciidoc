...
. Give the case a name, add relevant tags, assign a severity, and provide a description.
+
TIP: In the `Description` area, you can use
https://www.markdownguide.org/cheat-sheet[Markdown] syntax.
...
To edit or delete a comment, click the *More actions* menu (*…​*) and select the appropriate option.
...
You can examine individual alerts attached to a case by opening the alert details from the *Activity* tab. To view all alerts attached to the case, click the *Alerts* tab.
