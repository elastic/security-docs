...
. Go to *Investigate* -> *Cases*, then click *Create case*. If no cases exist, the Cases table will be empty and you'll be prompted to create one by clicking *Create case* inside the table.
...
Add a Lens visualization to your case to visualize event and alert data using charts and graphs.
...
IMPORTANT: Set absolute time ranges for your visualizations. This ensures they won't change after being saved to your case, and provides important context for others managing the case.
...
.. Choose whether to keep *Update panel on Security* activated. This option is active by default and automatically adds the visualization to your Visualize Library.
...
. A confirmation message that your file is downloading appears.
