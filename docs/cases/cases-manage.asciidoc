...
Each case has a universally unique identifier (UUID) that you can copy and share. To copy a case's UUID to a clipboard, go to the Cases page, find the case you want to share, click the *Actions* menu, and then select *Copy Case ID*. Alternatively, go to a case's details page, click the *More actions* menu (…​), and select *Copy Case ID*.
