...
+
IMPORTANT: {sn} ITSM and SecOps connectors created in {stack} version 7.15.0 or earlier are marked as deprecated after you upgrade to version 7.16.0 or later. It is strongly recommended to <<post-upgrade-deprecated-sn-connector, update deprecated connectors>> or create new ones to ensure you have access to connector enhancements, such as updating incidents. 
