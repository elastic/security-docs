...
+
IMPORTANT: {sn} ITSM and SecOps connectors created in {stack} version 7.15.0 or earlier use the https://developer.servicenow.com/dev.do#!/reference/api/quebec/rest/c_TableAPI[Table API]. They are marked as deprecated after you upgrade to version 7.16.0 or later. To ensure you have access to connector enhancements, such as updating incidents, we strongly recommended that you <<post-upgrade-deprecated-sn-connector, update deprecated connectors>> or create new ones. 
