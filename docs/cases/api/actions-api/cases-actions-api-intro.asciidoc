...
+
IMPORTANT: {sn} ITSM and SecOps connectors created in {stack} version 7.15.0 or earlier are marked as deprecated after you upgrade to version 7.16.0 or later. For more information about handling deprecated {sn} connectors, and creating new ones, visit <<post-upgrade-deprecated-sn-connector, Update a deprecated ServiceNow connector>>. 
