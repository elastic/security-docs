...
* `source` (string): The name of the {es-sec} case field, which can be 
...
and `comments`.
* `actionType` (string): Determines whether {es-sec} case updates overwrite 
...
<1> {es-sec} case `title` fields are mapped to {sn} `short_description`
fields. When a {es-sec} `title` field is updated and sent to {sn}, the {sn}
`short_description` field is overwritten.

<2> {es-sec} case `description` fields are mapped to {sn} `description`
fields. When a {es-sec} `description` field is updated and sent to {sn},
the {sn} `description` field is overwritten.

<3> {es-sec} case `comments` fields are mapped to {sn} `comments` fields.
When a {es-sec} `comments` field is updated and sent to {sn}, the updated
