...
[discrete]
[[IM-rule-heap-memory]]
.Indicator match rules are failing due to insufficient heap memory
[%collapsible]
====
If you receive the following rule failure: `Bulk Indexing of signals failed: index: ".index-name" reason: "maxClauseCount is set to 1024" type: "too_many_clauses"`, this indicates that the limit is too low for the total number of clauses that a query tree can have. Try increasing your cluster's heap memory.
====
