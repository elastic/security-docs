...
* Open a https://support.elastic.co[support case] with Elastic to alert our Elastic Support team of your system change. If you need additional assistance beyond your support subscription level, you can also purchase a https://www.elastic.co/consulting[consulting engagement]. 
...
. Note that alerts in 8.x are written using the `kibana_system` user instead of the `current user`, so any ingest pipelines configured against the alert index will use the `kibana_system` user privileges, which may break your ingest pipeline.
