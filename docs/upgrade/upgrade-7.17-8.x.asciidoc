...
* If your organization sends alerts (formerly known as signals) to an external SOAR (security orchestration, automation, and response) platform, you may need to change your workflows to accommodate the new {security-guide-all}/8.0/alert-schema.html[alert schema] used in 8.x.
...
** If you're upgrading a self-orchestrated deployment, refer to {stack-ref-80}/upgrading-elasticsearch.html[these instructions] and upgrade the data nodes tier by tier in this order:
... Frozen tier
... Cold tier 
... Warm tier
... Hot tier 
...
. Upgrade {kib}. Refer to {stack-ref-80}/upgrading-kibana.html[these instructions].
+
...
When you manually install {elastic-endpoint}, you must approve a system extension, kernel extension, and enable Full Disk Access (FDA). There is a new FDA requirement in 8.x. Refer to {security-guide-all}/8.7/elastic-endpoint-deploy-reqs.html[Elastic Endpoint requirements] to review the required permissions.
...
The system index for detection alerts has been renamed from `.siem-signals-<space-id>` to `.alerts-security.alerts-<space-id>` and is now a hidden index. Therefore, the schema used for alert documents in {elastic-sec} has changed. Users that access documents in the `.siem-signals` indices using the {elastic-sec} API must modify their API queries and scripts to operate properly on the new 8.x alert documents. Refer to {security-guide-all}/8.0/query-alert-indices.html[how to query alert indices] and review the new {security-guide-all}/8.0/alert-schema.html[alert schema].
...
* You must create separate rules to query threat intelligence indices created by {filebeat} version 7.x and version 8.x because each version requires a different default threat indicator path value. Review the recommendations for {security-guide-all}/8.0/query-alert-indices.html[querying alert indices].