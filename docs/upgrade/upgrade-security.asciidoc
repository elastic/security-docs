...
In 8.8, <<rule-notifications, action frequencies>> are no longer set at the rule level. Now, you can choose frequencies for individual actions and send notifications when an alert is generated or at a specified alert summary interval. 

Be mindful that after you upgrade to 8.8, frequency settings for rule actions created in 8.7 or earlier are converted from the rule level to the action level. Scheduled actions will continue to run on their previously specified frequency (`On each rule execution` `Hourly`, `Daily`, or `Weekly`). Rules without actions will be snoozed indefinitely. 
