...
In 8.8, <<rule-notifications,rule action frequencies>> can be set for individual actions, and you can specify if notifications are sent when an alert is generated or at a custom alert summary interval. When you upgrade to 8.8, action-level frequency settings are applied to rules that have actions and were created in 8.7 or earlier. The frequency setting will default to the action frequency that was selected for the rule before you upgraded. 
