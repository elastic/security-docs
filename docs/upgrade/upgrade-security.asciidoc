...
If you're upgrading to {stack} 8.0 or 8.1, you must update your Elastic prebuilt detection rules while your {stack} is at 7.17, before upgrading to 8.x. To update Elastic prebuilt rules, go to the Rules page and select *Update _x_ Elastic prebuilt rules*. This ensures that you have the latest prebuilt rules before upgrading the {stack} to 8.x.
