...
* <<upgrade-from-7.17-8x, Upgrade from 7.17 to an 8.x version>>
...
+
. If you use any {es} plugins, make sure each plugin has a version compatible with {es} version {version}.
