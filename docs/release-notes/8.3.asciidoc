...
[[known-issue-8.3.3]]
==== Known issues
* An {endpoint-sec} bug on macOS and Linux can cause CPU spikes if Malware prevention is enabled on an {endpoint-cloud-sec} integration integration policy. When this happens, {endpoint-sec} is locked up and stops functioning normally. To avoid this issue, we recommend using {stack} 8.3.2 or earlier. If you are using 8.3.3 and have encountered this issue, you can temporarily resolve it by rebooting your computer and disabling Malware prevention on your {endpoint-cloud-sec} integration integration policy (https://github.com/elastic/endpoint/issues/22[#22]).
