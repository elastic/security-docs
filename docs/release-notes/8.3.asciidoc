...
When adding an exception to a detection rule, the UI offers a matches operator that does not have a corresponding implementation:
...
[[known-issue-8.3.2]]
==== Known issue
* The `matches` operator is not supported for exceptions to detection rules ({issue}136340[#136340]).
