...
* Fixes a bug that exposed the `matches` operator when building exceptions for detection rules ({pull}136340[#136340]).
...
[[known-issue-8.3.2]]
==== Known issue
* The `matches` operator in the *Add Rule Exception* flyout does not work because wildcard matches are not supported for rule exceptions. Using the `matches` operator will cause rules to fail. To restore a failed rule, delete the exception, then refresh the rule ({issue}136340[#136340]).
