...
* Fixes a bug that prevented the All Users and All Hosts pages from reseting after being sorted ({pull}133111[#133111]).
...
* Fixes a bug that caused the *Add Rule Exception* flyout to unexpectedly close when users created the first exception for a rule from an alert ({pull}130187[#130187]).
* Corrects Rule name sorting so detection rules are ordered alphabetically, regardless of capitalization ({pull}130105[#130105]).
...
* Updates the Case table so that all tags assigned to a case are displayed when users go to the case and hover over the *Tags* column ({pull}132023[#132023).
* Adds Oauth support to the {sn} ITSM, SecOps, and ITOM connectors ({pull}131248[#131248).
* Adds a setting that specifies the list of allowed email domains which can be used with the email connector ({pull}129001[#129001).
