...
* Fixes a bug where the Elastic Managed LLM connector was missing from the AI settings **Connector*s* dropdown ({kibana-pull}273423[#273423]).
* Fixes alert action error toasts (assign, tag, open, close, acknowledge) so they show the full error message from {{es}} instead of a blank body({kibana-pull}269592[#269592]).
* Fixes an issue where auto-refresh on the **Alerts** page with grouping applied closed open flyouts, case creation dialogs, and expanded accordions ({kibana-pull}264518[#264518]).
