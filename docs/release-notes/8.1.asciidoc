...
* If {elastic-endpoint} is used with other products that monitor or manage network traffic (such as antivirus programs, firewalls, or VPNs) users might experience network connection issues. This issue is only present on macOS; upgrading to macOS 12.4 resolves it.
