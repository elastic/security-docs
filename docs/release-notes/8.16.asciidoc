...
// tag::known-issue-53[]
[discrete]
.Alerts page crashes if you upgrade to 8.16 and access it in a non-default space  
[%collapsible]
====
*Details* +
On November 14, 2024, it was discovered that the Alerts page would crash if you upgraded to 8.16.0 and accessed the page in a non-default space.
...
. Search for the `siem.<space_name>.pageFilters` key and right-click on the value. 
. In the menu, click *Delete*, then refresh the Alerts page. The Alerts table re-renders correctly.
