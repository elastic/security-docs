...
[[bug-fixes-8.16.6]]
==== Bug fixes
* Fixes a bug that affected column widths in Timeline ({kibana-pull}214178[#214178]).
...
* Allows data view selections to persist when you save a Timeline ({kibana-pull}211343[#211343]).
* Improves trusted applications performance by dropping Event Tracing for Windows (ETW) events earlier.
* Improves ETW-TI performance by not enabling local executable virtual memory events for sandboxed and Antimalware-PPL processes.
* Fixes the low probability of Tamper Protected {elastic-defend} upgrade failure.
* Fixes a bug in the scan response action that can crash {elastic-defend}.
* Improves the `call_stack_final_user_module` and `call_stack_final_hook_module` logic.
