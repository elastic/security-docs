...
[[bug-fixes-8.16.5]]
==== Bug fixes
* Fixes timeline column width bug ({kibana-pull}214178[#214178]).
* Fixes event filters cannot be saved bug ({kibana-pull}213805[#213805]).
* Fixes structured log flow to handle multiple types of structured logs ({kibana-pull}212611[#212611]).
* Reliably persist dataview selections for timeline ({kibana-pull}211343[#211343]).
* Drop ETW events logged by trusted applications earlier.
* Improve ETW-TI performance by not enabling local executable virtual memory events for sandboxed and Antimalware-PPL processes.
* Fixes low probability Tamper Protected Endpoint upgrade failure.
* Fixed a bug in the scan response action that can crash the Endpoint.
* Defend: Improve `call_stack_final_user_module` and `call_stack_final_hook_module` logic.
