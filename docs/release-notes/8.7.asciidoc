...
=====
If you are upgrading {agent} to 8.7.1 from 8.6.2 to 8.7.0, the {agent} upgrade might fail for {agents} running the Osquery Manager integration. To prevent this, delete the Osquery Manager integration from your {agents} before upgrading them to 8.7.0, then re-add it after upgrading. 
...
. Search for `compression_algorithm: none` in the policy details. This field and value pair will appear under the affected endpoint policy artifacts.
