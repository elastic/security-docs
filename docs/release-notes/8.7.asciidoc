...
* The **Add exceptions flyout** loads indefinitely and an out of memory error displays when a rule has a large number of unmapped fields in multiple indices. To avoid this issue, use the {api-kibana}/operation/group/endpoint-security-exceptions-api[Exceptions API] to manage exceptions. 
* If you modify an exception item using the {api-kibana}/operation/operation-updateexceptionlistitem[Update an exception list item] API and _only_ specify its `item_id`, the exception item is erroneously duplicated. To avoid this issue, you can either:

** <<manage-exception,Update exception items>> through the {security-app} UI. 
** Specify an exception item's `item_id` _and_ its `id` when modifying an exception through the {api-kibana}/operation/operation-updateexceptionlistitem[Update an exception list item] API. 
