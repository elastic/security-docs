...
+
**Case #1:** The Alerts table does not render while attempting to display threat indicator match alerts with nested `file.name` fields. To re-render the Alert table, manually edit your browser's local storage:
