...
+
**Case #1:** The Alerts table does not render as it attempts to display threat indicator match alerts with nested `file.name` fields. Re-render the Alerts table by manually editing your browser's local storage:
...
. Search for the `detection-engine-alert-table-securitySolution-rule-details-gridView` key and copy its value.
+
NOTE: The value you copied is the JSON blob that's used to persist the Alert table's state, including the table's selected columns. 
...
+
**Case #2:** The Alerts table renders with empty rows as it attempts to display threat indicator match alerts with nested `file.name` fields and any other alert types. To resolve this issue: 
...
[[enhancements-8.9.0]]
==== Enhancements
* Makes it easier to set up exceptions by auto-populating exception conditions and values with relevant alert data  ({pull}159075[#159075]).  159029
