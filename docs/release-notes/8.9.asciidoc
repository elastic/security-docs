...
* An UI bug can affect the Alerts table's height and make it difficult to view alerts. View the alerts in Timeline or choose one of the following workarounds to readjust the Alerts table:
...
+
**Case #1:** The Alerts table does not render during its attempt to display threat indicator match alerts with nested `file.name` fields. To re-render the Alert table, manually edit your browser's local storage:
...
. Go back DevTools and paste the edited JSON into the value for the `detection-engine-alert-table-securitySolution-rule-details-gridView` key.
