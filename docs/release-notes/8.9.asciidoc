...
[[features-8.9.0]]
==== New features
* Enables the `top` command for MacOS and Linux, which shows active processes sorted by much CPU they are causing {elastic-endpoint} to consume (www.github.com/elastic/endpoint/blob/main/EndpointTopCommand.md[`top` Readme]).
