...
[[enhancements-8.9.1]]
==== Enhancements
* Event correlation rules now support negative correlation. 
...
* Fixes rounding issues in the the *Top alerts by* table on the Alerts table ({pull}162647[#162647]).
* Fixes bug that prevented controls from being rendered in {elastic-sec} dashboards ({pull}162514[#162514]).
* Fixes exceptions in investigate in timeline ({pull}162190[#162190]).
* Fixes a bug that prevented rule changes from being saved if the rule's action frequency was shorter than the rule's run interval ({pull}160798[#160798]).
