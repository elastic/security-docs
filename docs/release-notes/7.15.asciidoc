...
* Fixes the bug that caused the live query search to return irrelevant results for agents ({pull}116332[#116332]).
* Improves Osquery SQL query parser logic.({pull}114932[#114932]).
...
* Updates the queries called from Security solution to use the `ALERT_WORKFLOW_STATUS` constant and removes the `replaceAll` reference in the Timelines plugin ({pull}114481[#114481]).
