...
* Ensures {endpoint-sec} can continue to run on all supported Windows versions by changing the primary signer of the `elastic-endpoint.exe` file from ELASTICSEARCH B.V. to Elasticsearch, Inc. (https://github.com/elastic/endpoint/issues/15[#15]).
