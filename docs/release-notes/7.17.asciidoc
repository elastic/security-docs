...
* The bug that prevented Endpoint Security from being run on Windows 8.1 and Server 2012 R2 has been fixed. With this change, the primary signer for `elastic-endpoint.exe` has changed from ELASTICSEARCH B.V. to Elasticsearch, Inc (https://github.com/elastic/endpoint/issues/15[#15]).
...
* Endpoint Security cannot be run on Windows 8.1 and Server 2012 R2 (https://github.com/elastic/endpoint/issues/15[#15]).
