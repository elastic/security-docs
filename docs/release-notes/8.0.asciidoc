...
Before upgrading, use the <<rules-api-find, Find rules>> API to retrieve a list of enabled detection rules in your environment. You will be referencing this list after you upgrade and need to re-enable rules.

NOTE: A more straightforward way of identifying rules that have been disabled during the upgrade process is being developed for {stack} version 8.0.1.

We recommend using curl or another HTTP tool to securely run {elastic-sec} APIs. Below is an example curl command that you can run to find enabled rules:
...
Alternatively, you use the <<bulk-actions-rules-api, Bulk rule actions>> API to re-enable rules.
