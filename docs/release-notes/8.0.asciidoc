...
Comments on rule exceptions will cause the import process to fail as the following system-generated attributes cannot be validated for exception comments ({pull}124742[#124742]):
...
. Search the exported `.ndjson` file for exceptions with comments. Exception comments are stored within the `exceptionItem` object (`exceptionItem.comments`).
. Edit the exception comment's attributes:
** To preserve the comment during the import process, only delete the `created_at`, `created_by`, `updated_at`, `updated_by`, and `id` attributes.
** If you don't want to preserve the comment, remove the comment and its attributes.
