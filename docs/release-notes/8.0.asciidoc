...
[[deprecations-8.0.0]]
==== Deprecations
* The `output_index` parameter is no longer supported for the APIs that create and update rules. In addition, the `output_index` field is not included in the exported rule file.
