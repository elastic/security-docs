...
*The import process fails if rule exceptions have comments*

Comments on rule exceptions will cause the import process to fail as the following system-generated attributes are not validated for rule exceptions ({pull}124742[#124742]):
...
To complete the import process successfully, update the exported `.ndjson` file and re-import it:

. Search the exported `.ndjson` file for exceptions with comments. Exception comments are stored within the `exceptionItem` object.
. Edit the exception comment's attributes:
** To preserve the comment during the import process, only remove metadata about the comment. Metadata is stored in the `created_at`, `created_by`, `updated_at`, and `updated_by` attributes.
** If you don't want to preserve the comment, remove it entirely.
