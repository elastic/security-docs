...
Before upgrading, use the <<rules-api-find, Find rules>> API to retrieve a list of enabled detection rules in your environment. We recommend using curl or another HTTP tool to securely run {elastic-sec} APIs.
...
Below is an example curl command that you can run to find enabled rules:
