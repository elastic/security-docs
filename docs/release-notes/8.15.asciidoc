...
[discrete]
.Alerts wrongfully inheriting previous tags
[%collapsible]
====
*Details* +

When you add tags to alerts from the Alerts table, the previously selected tag is incorrectly applied, in addition to new tags that you select. 

*Workaround* +

When adding tags to an alert, click the previously applied tags to re-apply them, then click them again to remove them. Save your changes by clicking *Apply tags*. This removes the wrongfully inherited tags from the alert.
