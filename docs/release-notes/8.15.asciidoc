...
[[features-8.15.2]]
==== New features
* Allows Automatic Import to identify log format type based on the sample input logs ({kibana-pull}190407[#190407]).

[discrete]
[[enhancements-8.15.2]]
==== Enhancements
* {elastic-defend} now supports Ubuntu 24.04.
* {elastic-defend} now supports proxy configuration with Logstash output.
* {elastic-defend} has improved support for call stack module stomp detection in Windows 11 24H2 ({kibana-pull}192490[#192490]).
* Automatic Import now supports Google Gemini, OpenAI, and Azure OpenAI connectors ({kibana-pull}191577[#191577]).
* Allows Automatic Import to support unstructured system logs ({kibana-pull}192817[#192817]).
* Allows Automatic Import to perform reproducible sampling from the list of log entries instead of truncating them ({kibana-pull}191598[#191598]).
* Displays error messages in Automatic Import when logs sample files fail to upload ({kibana-pull}191310[#191310]).

[discrete]
[[bug-fixes-8.15.2]]
==== Bug fixes
* Updates Gemini connector to ignore unknown properties in responses ({kibana-pull}192915[#192915]).
...
* Fixes an issue that prevented the `http_endpoint` input configuration from loading correctly in an Automatic Import workflow ({kibana-pull}191964[#191964]).
* Fixes a bug that prevented the `enable` field from being respected when you imported rules ({kibana-pull}192302[#192302]).
