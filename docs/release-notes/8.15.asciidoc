...
[[features-8.15.2]]
==== New features
* Automatic Import can now use sample input logs to identify log format types ({kibana-pull}190407[#190407]).
...
* Automatic Import now supports the Google Gemini, OpenAI, and Azure OpenAI connectors ({kibana-pull}191577[#191577]).
* Automatic Import can take unstructured system logs ({kibana-pull}192817[#192817]).
* Automatic Import displays error messages when logs sample files don't upload ({kibana-pull}191310[#191310]).
* Automatic Import performs reproducible sampling from a list of log entries instead of truncating them ({kibana-pull}191598[#191598]).

[discrete]
[[bug-fixes-8.15.2]]
==== Bug fixes
* Ensures the Gemini connector ignores unknown properties in responses ({kibana-pull}192915[#192915]).
