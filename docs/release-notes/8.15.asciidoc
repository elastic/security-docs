...
* Adds `process.Ext.protection` to image load events.
* Enables JAMF data in Analyzer ({kibana-pull}190965[#190965]).

[discrete]
[[bug-fixes-8.15.1]]
==== Bug fixes
* Fixes prebuilt rule duplication logic to copy related integrations and required fields from the original rule ({kibana-pull}191065[#191065]).
* Updates alert assignment to disable unassign action if no assignees exist ({kibana-pull}190937[#190937]).
* Copies over settings from timeline template ({kibana-pull}190511[#190511]).
* Fixes the select a connector placeholder issue ({kibana-pull}189944[#189944]).
* `Scorpion::GetLongFilePath` Caching.
