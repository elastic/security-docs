...
[[bug-fixes-8.15.3]]
==== Bug fixes
* Re-do the iterator and skip unsupported file systems.
* Automatic Import does not ask the LLM to map the fields to the reserved ECS fields anymore ({kibana-pull}195168[#195168]).
* Automatic Import is more forgiving if an LLM returns an ECS mapping in a slightly unexpected format ({kibana-pull}195167[#195167]).
* Fixes a bug to resolve non-ecs compatible fields in Structured / Unstructured syslog processing in Automatic Import ({kibana-pull}194727[#194727]).
* Fixes a bug when uploading an existing EPR integration with a new version ({kibana-pull}194298[#194298]).
* Fixes a bug that is causing the deploy step to fail after a pipeline edit/save ({kibana-pull}194203[#194203]).
* Includes the `user.target.name` field in the default Anonymization allow list to improve Attack discoveries ({kibana-pull}193496[#193496]).
* Fixes a display issue when an entity is repeated ({kibana-pull}193428[#193428]).
