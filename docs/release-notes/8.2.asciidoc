...
[[deprecations-8.2.0]]
==== Deprecations
The following endpoints are deprecated ({pull}129448[#129448]) and will be removed in a future release but no earlier than 18 months:
...
To avoid breakage, we recommend using the <<bulk-actions-rules-api,bulk rule actions>> endpoint, which performs the same actions. You can also use the regular create, update and delete endpoints.
