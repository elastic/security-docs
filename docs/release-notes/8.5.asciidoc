...
+
Upgrading to 8.6 or higher resolves this issue. If you’d prefer to stay on 8.5, use the {api-kibana}/operation/operation-updaterule[update rule API] to remove the `saved_id` field from the non-functioning `query`, `eql`, `machine_learning`, `threat_match`, `threshold`, or `new_terms` rule.
