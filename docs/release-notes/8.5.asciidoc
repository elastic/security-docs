...
* Fixes bug that caused incorrect alerts to display in Timeline when investigating alerts from the Detection & Response dashboard ({pull}144319[#144319]).
...
* Turns off the option to edit machine learning rules if users don’t have have the Machine Learning privilege in Kibana set to `All` ({pull}143260[#143260]).
* Removes the ability from the UI to enable and disable machine learning rules if users don’t have have the Machine Learning privilege in Kibana set to `All` ({pull}143252[#143252]).
