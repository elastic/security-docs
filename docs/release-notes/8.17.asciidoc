...
* Makes minor improvements to {elastic-defend}'s' CPU usage for systems with very high event volumes.
...
* Ensures that {elastic-defend} prevents unbounded kernel non-paged memory growth during extremely high event load situations on Windows.
* Allows {elastic-defend} to detect and recover from a corrupt persistent cache database.  Previously, such databases would be unusable, effectively disabling the 
