...
* In 8.16.2 and 8.17.0, we refactored a portion of our Windows kernel driver to work around an incompatibility with CrowdStrike Falcon which could result in a `CRITICAL_PROCESS_DIED` bugcheck. We discovered that this incompatibility could also be triggered by Memory Protection and refactored a portion of our kernel driver to avoid this conflict.
+
Affected users who are unable to upgrade should set one or both of following in their {elastic-defend} advanced policy, depending on their version:

** `windows.advanced.events.process.creation_flags: false` (verions 8.13.0 - 8.16.1)
** `windows.advanced.memory_protection.shellcode_trampoline_detection: false` (versions 8.12.0 - 8.16.2)
