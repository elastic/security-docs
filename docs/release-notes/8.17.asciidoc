...
* Fix a bug in {elastic-defend} where environment variables were not collected on macOS according to the `advanced.capture_env_vars` field
* Fix a bug where aarch64 {elastic-defend} won't gracefully detect being executed in emulation on amd64 hardware.
