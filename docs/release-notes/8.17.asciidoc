...
[[enhancements-8.17.6]]
==== Enhancements
* {elastic-defend} users can now opt out of event-driven Memory Protection scanning via advanced policy ({kibana-pull}218354[#218354]).
* Minor {elastic-defend} CPU usage improvements for systems with very high event volumes.
* {elastic-defend} users can now opt out of event-driven Memory Protection scanning via advanced policy.
...
* Avoids an `IRQL_NOT_LESS_EQUAL` [bugcheck](https://learn.microsoft.com/en-us/windows-hardware/drivers/debugger/bug-checks--blue-screens-) in the {elastic-defend} driver due to an interaction with Trellix Access Protection (`mfehidk.sys`). This issue can occur when `elastic-endpoint-driver.sys` calls [`FwpmTransactionBegin0`](https://learn.microsoft.com/en-us/windows/win32/api/fwpmu/nf-fwpmu-fwpmtransactionbegin0) to initialize its network driver. `FwpmTransactionBegin0` performs a synchronous RPC call to the user-mode Base Filtering Engine service. Trellix's driver intercepts this service's operations, causing `FwpmTransactionBegin0` to hang. This hang prevents Defend's driver from properly initializing in a timely manner. Subsequent system activity can invoke {elastic-defend}'s driver before it has fully initialized, leading to a `IRQL_NOT_LESS_EQUAL` bugcheck.
* {elastic-defend} will now prevent unbounded kernel non-paged memory growth during extremely high event load situations on Windows.
* {elastic-defend} will now detect and recover from a corrupt persistent cache database.  Previously, such databases would be unusable, effectively disabling the 
* {elastic-defend} now reduces CPU usage for registry events.
