...
[[bug-fixes-8.17.3]]
==== Bug fixes
* Reduces the prebuilt rule's package size to prevent {kib} from crashing with `Out Of Memory` errors when the package is installed.  
...
*Workaround* +
To resolve this issue, upgrade to 8.17.3. Alternatively, increase {kib}'s RAM to 2 GB.
