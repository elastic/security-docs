...
* In some situations, {elastic-endpoint} might change to a non-running state on Windows endpoints and then can't be restarted. This failure might be also missed by {agent}, which would automatically re-install {elastic-endpoint}. To determine whether {elastic-endpoint} has stopped running, run the following PowerShell command as an administrator:
...
If {elastic-endpoint} is not running on your Windows endpoint, there are several workarounds you can take:

** **Manually invoke {elastic-endpoint} uninstall command**: When you manually invoke {elastic-endpoint}'s' uninstall command on the host, this clears out the invalid {elastic-endpoint} install so that {elastic-endpoint} can be installed again. Once the uninstallation has finished,  restart {agent} so it can with the command: `c:\Program Files\Elastic\Agent\elastic-agent.exe restart`. {agent} will then automatically reinstall {elastic-endpoint}.
** **Uninstall the {endpoint-cloud-sec} integration from affected hosts, then re-install it**: Uninstalling and re-installing the {endpoint-cloud-sec} integration will also trigger an uninstall and re-install of {elastic-endpoint} on affected hosts.
+
NOTE: Uninstalling the {endpoint-cloud-sec} integration may put {agent}  into an `Unhealthy` state. This is temporary and the state will change to `Healthy` once the integration is re-installed.
