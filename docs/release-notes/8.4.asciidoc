...
* In some situations, {elastic-endpoint} might change to a non-running state on Windows endpoints and then can't be restarted. This failure might be also missed by {agent}, which would automatically re-install {elastic-endpoint} (https://github.com/elastic/endpoint/issues/29[#29]).
+
