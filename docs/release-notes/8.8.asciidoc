...
* Introduces the {kibana-ref}/gen-ai-action-type.html[Generative AI connector] and Security Assistant for {elastic-sec} ({pull}157228[#157228] {pull}156933[#156933]).

[discrete]
[[bug-fixes-8.8.1]]
==== Bug fixes and enhancements
* Fixes an Anomalies table bug that left {ml} jobs greyed out after they were installed ({pull}158821[#158821]).
* Fixes an Anomalies table bug that stopped some job counts from being shown ({pull}158739[#158739]).
* Fixes a bug that caused all field types in the **Fields** browser appear as `unknown` ({pull}158594[#158594]).
...
* Changes the way large error messages are displayed on the rule details page. Users can now scroll through long error messages ({pull}157271[#157271]).
