...
* The rule level `throttle` field, which defines how often rule actions are run, is deprecated. The field will remain active for at least the next 12 months. In {elastic-sec} 8.8 and later, we strongly recommend using the action level `frequency` field to set frequencies for individual rule actions.
...
* Updates the Findings page to include a Vulnerabilities tab that displays data from the new Cloud native vulnerability management integration ({pull}154388[#154388], {pull}154873[#154873], {pull}155045[#155045]).
