...
* Elastic Security Assistant ({pull}156933[#156933]).
...
* Generative AI Connector ({pull}157228[#157228]).

[discrete]
[[bug-fixes-8.8.1]]
==== Bug fixes and enhancements
* GetDataViewStateFromIndexFields was using wrong type as part of a cast ({pull}158594[#158594]).
* Failed `getFieldsForIndexPattern` calls can result in Exception Flyout getting stuck in loading state ({pull}158371[#158371]).
* Adds support for multiple values in cell actions ({pull}158060[#158060]).
* Fixes wildcard in field names issue for query language in detection rules ({pull}157981[#157981]).
* Fixes incorrectly applied emotion styles ({pull}157935[#157935]).
* Alert Table column metadata does not have the columns data type ({pull}157653[#157653]).
* Fixes error messages on Rule Details page taking too much space ({pull}157271[#157271]).
* Adding null checks when iterating through index template list ({pull}158742[#158742]).
* Throwing a mustache error when validating action message for warnings ({pull}158668[#158668]).
* Fixes a bug that caused rendering issues with ML embeddables, Osquery, and indicator attachments in cases ({pull}158441[#158441]).
