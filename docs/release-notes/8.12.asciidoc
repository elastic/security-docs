...
// tag::known-issue-173958[]
[discrete]
.[Action frequency filters hidden in the UI when creating and editing a rule]
[%collapsible]
====
*Details* +
Action frequency filters in the rule creation and edit flows are hidden in the UI. Rules configured with these filters still run correctly.

*Workaround* +
Use the <<rules-api-update,update rule>> API to change the action frequency filters. Alternatively, you can export the rule, update the action frequency filters, then re-import the rule to update it.
