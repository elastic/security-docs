...
[[bug-fixes-8.12.2]]
==== Bug fixes
* Fixes long running queries in Timeline and in Events tables within Explore pages ({pull}176838[#176838]).
...
* Fixes a bug that prevented numbers from being used for Osquery pack IDs ({pull}176507[#176507]).
* Fixes bugs that affected the rule details page layout if rule filters were extremely long or had aliases ({pull}176590[#176590]).
* Fixes a bug that prevented rules from being imported successfully if the rule referenced a preconfigured connector ({pull}176284[#176284]).
* Fixes a bug that prevented rules from being successfully exported if you were exporting more than 1000 rules ({pull}175979[#175979]).
* Turns off the option to install rules if users don't have the appropriate privileges ({pull}176598[#176598]).
* Fixes a bug that caused data to be lost when you upgraded a prebuilt rule to a new version that had a different rule's type ({pull}176421[#176421]).
