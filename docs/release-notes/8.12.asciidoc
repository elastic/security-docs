...
*Workaround* +
Use the {api-kibana}/operation/operation-updaterule[update rule API] to change a rule's action frequency settings. Alternatively, export a rule, update its action frequency settings, and then re-import the rule.
====
