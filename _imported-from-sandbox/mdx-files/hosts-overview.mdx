---
id: enSecurityHostsOverview
slug: /en/security/hosts-overview
title: Hosts page
...
* **Host risk**: The latest recorded host risk score for each host, and its host risk classification. This feature requires a [Platinum subscription](https://www.elastic.co/pricing) or higher and must be enabled to display the data. Click **Enable** on the **Host risk** tab to get started. To learn more, refer to our <DocLink id="enSecurityHostRiskScore">host risk score documentation</DocLink>.  
* **Sessions**: Linux process events that you can open in <DocLink id="enSecuritySessionView">Session View</DocLink>, an investigation tool that allows you to examine Linux process data at a hierarchal level.
