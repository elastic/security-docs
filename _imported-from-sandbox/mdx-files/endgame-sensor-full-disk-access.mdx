---
id: enSecurityEndgameSensorFullDiskAccess
slug: /en/security/endgame-sensor-full-disk-access
...
- `10.13, 10.14, 10.15`: You cannot install the sensor without allowing it to load a <DocLink id="enSecurityEndgameSensorFullDiskAccess" section="approve-the-kernel-extension-for-the-endgame-sensor">kernel extension</DocLink>. During installation, you'll be prompted to go to System Preferences and approve it. Upon approval, installation proceeds.

- `11.0`, `12.0`: You cannot install the sensor without allowing it to load a <DocLink id="enSecurityEndgameSensorFullDiskAccess" section="approve-the-system-extension-for-the-endgame-sensor">system extension</DocLink>. During installation, you'll be prompted to go to System Preferences and approve it. Upon approval, a second prompt appears to enable Network Filtering. Approve this final prompt to proceed.

You must also grant Full Disk Access to `com.endgame.systemextension`.

- `10.14.6+, 10.15, 11.0, 12.0`: Grant the <DocLink id="enSecurityEndgameSensorFullDiskAccess" section="enable-full-disk-access-for-the-endgame-sensor">esensor file</DocLink> Full Disk Access.

<DocCallOut title="Note">
The following instructions apply to the Endgame sensor only. To see requirements for the ((elastic-endpoint)), refer to <DocLink id="enSecurityElasticEndpointDeployReqs">((elastic-endpoint)) requirements</DocLink>.
</DocCallOut>
...
1. Enter `kextload /Library/Extension/kendpoint.kext`. Prepend the command with `sudo` if necessary.
...
1. In the lower-left corner of the pane, click the **Lock button**, then enter your credentials to authenticate.
1. Click the *+* button to view **Finder**. Find  `com.endgame.systemextension` and select it.
1. Continue below to enable Full Disk Access for the Endgame sensor.
...
1. In the lower-left corner of the pane, click the **Lock button**, then enter your credentials to authenticate. You can now add the `elastic-endpoint` or `esensor` file.
1. Click the *+* button to view **Finder**.
1. Navigate to `/Library/Endgame`, then select the `esensor` file.
1. Click **Open**.
1. In the **Privacy** tab, confirm that the `elastic-agent` or `esensor` file appears in the list of applications with Full Disk Access permissions.
