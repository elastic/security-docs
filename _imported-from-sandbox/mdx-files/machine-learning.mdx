---
id: enSecurityMachineLearning
slug: /en/security/machine-learning
title: Anomaly detection with ((ml))
description: Use the power of machine learning to detect outliers and suspicious events.
tags: ["security","overview","manage"]
---
...
or are testing out a **Free Trial**. Refer to <DocLink id="enSecurityMlRequirements">Machine learning job and rule requirements</DocLink>.
...
* On the **Rules** page, the **Last response** column displays the rule's current <DocLink id="enSecurityRulesUiManagement" section="check-the-current-status-of-rules">status</DocLink>. An indicator icon (<DocIcon type="alert" title="Error" />) also appears if a required ((ml)) job isn't running. Click the icon to list the affected jobs, then click **Visit rule details page to investigate** to open the rule's details page.
...
* You ship data using [Beats](https://www.elastic.co/products/beats) or the
    <DocLink id="enSecurityInstallEndpoint">((agent))</DocLink>, and ((kib)) is configured with the required index
...
<DocLink id="enSecurityPrebuiltMlJobs">Prebuilt job reference</DocLink> describes all available ((ml)) jobs and lists which ECS
...
number of false positives, see <DocLink id="enSecurityTuningAnomalyResults">Optimizing anomaly results</DocLink>.
