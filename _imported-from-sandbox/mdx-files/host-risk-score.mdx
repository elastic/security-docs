---
id: enSecurityHostRiskScore
slug: /en/security/host-risk-score
...
This feature is available for ((stack)) versions 7.16.0 and newer and requires a [Platinum subscription](https://www.elastic.co/pricing) or higher.
</DocCallOut>
...
<DocCallOut title="Note">
To enable the host risk score feature, you must have alerts in your environment. If you previously enabled host risk score and are upgrading the ((stack)) to 8.5 or newer, refer to <DocLink id="enSecurityHostRiskScore" section="upgrade-host-risk-score">Upgrade host risk score </DocLink>. 
</DocCallOut>
...
1. In the ((security-app)), go to **Dashboards** -> **Entity Analytics**. 
...
<div id="upgrade-host-risk-score"></div>
...
* Since older data is not preserved, previous host risk scores will be deleted, and new scores will be created. However, if you want to retain old host risk scores, you can reindex them _before_ upgrading. To learn how, refer to [Reindex API](((ref))/docs-reindex.html). New data will be stored in the `ml_host_risk_score_<space-id>` and `ml_host_risk_score_latest_<space-id>` indices. 

* You must edit your [((kib)) user settings](((cloud))/ec-manage-kibana-settings.html#ec-manage-kibana-settings) and remove the `xpack.securitySolution.enableExperimental:['riskyHostsEnabled']` feature flag. 
...
<DocCallOut title="Note">
After you enable or upgrade host risk score, you might get a message that says, "No host risk score data available to display." To verify that the transform that installs the host risk score module is picking up data, refer to <DocLink id="enSecurityVerifyHostRiskScore">Verify that host risk score data installed successfully</DocLink>. 
</DocCallOut>
...
1. In ((kib)), go to **Analytics** -> **Dashboard**, then search for `risk score`.
